...
With Fused, users find, reuse, and share [User Defined Functions (UDFs)](/guide/working-with-udfs/why-fused) in the Fused vibrant community. Fused UDFs are building blocks of serverless geospatial operations that integrate across the stack - with Planetary Computer, Google Earth Engine, Big Query, Snowflake, DuckDB, and more. They load datasets from the cloud ecosystem such as NASA, NOAA, US Census, and Overture. Fused serverless API turns these UDFs into live HTTPS endpoints that load their output into any tools.
