...
To overcome these challenges, Pachama turned to Fused to generate maps on the fly with [serverless API endpoints](/tutorials/Geospatial%20with%20Fused/filetile/#tile-https-call). Fused now provides them an elegant way to [write custom workflows](/core-concepts/write/) to [crunch data](/core-concepts/run-udfs/) with Python and serve it behind [tile endpoints](/tutorials/Geospatial%20with%20Fused/filetile/#tile-https-call) that natively integrate with map tile layers. This makes it possible to process and visualize any dataset with manageable operation costs.
