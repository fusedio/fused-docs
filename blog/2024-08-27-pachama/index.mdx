...
<iframe 
    ...
    src="https://www.youtube.com/embed/wG4wWHL5rN0" 
...
To overcome these challenges, Pachama turned to Fused to generate maps on the fly with [serverless API endpoints](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#tile-https-call). Fused now provides them an elegant way to [write custom workflows](/guide/working-with-udfs/writing-udfs) to [crunch data](/guide/working-with-udfs/udf-best-practices/realtime) with Python and serve it behind [tile endpoints](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#tile-https-call) that natively integrate with map tile layers. This makes it possible to process and visualize any dataset with manageable operation costs.
...
With Fused, Pachama replaced a complex pre-processing pipeline with a single User-Defined Function ([UDF](/guide/working-with-udfs/why-fused)) that directly loads data to the frontend. By dynamically generating tiles from the analytical data based on user requests, Pachama can provide national-level coverage without processing unnecessary data, streamlining their workflow and enabling more efficient and cost-effective operations.
...
This UDF can be called via [HTTPS request](/guide/data-input-outputs/export-api/tokens-endpoints) with the following URL structure:
