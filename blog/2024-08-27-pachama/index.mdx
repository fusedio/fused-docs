...
To overcome these challenges, Pachama turned to Fused to generate maps on the fly with [serverless API endpoints](/guide/working-with-udfs/geospatial/single-vs-tile#tile-https-call). Fused now provides them an elegant way to [write custom workflows](/guide/working-with-udfs/writing-udfs) to [crunch data](/guide/getting-started/udf-best-practices/realtime) with Python and serve it behind [tile endpoints](/guide/working-with-udfs/geospatial/single-vs-tile#tile-https-call) that natively integrate with map tile layers. This makes it possible to process and visualize any dataset with manageable operation costs.
