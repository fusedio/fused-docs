...
🎥 Watch the Webinar recording associated with this blog post [here](https://www.youtube.com/watch?v=jAoBr_qD63I).
...
Now that everything is running fine in the workbench, it is time to extract the layers and include them in a webpage. Fused provides an easy way to integrate layers in external applications via [HTTP requests](/core-concepts/run-udfs/run-single-udfs/#shared-token). You just need to go to Settings, click Share and copy the provided URL.
