...
<iframe 
    ...
    src="https://www.youtube.com/embed/jAoBr_qD63I" 
...
In this article I show how to create an object detection layer on 50cm imagery in realtime. It explains how to create a Fused User Defined Function ([UDF](/guide/working-with-udfs/writing-udfs)) to load satellite image tiles to call an inference model, then publish it as an interactive map app.
...
Basically, I just have to write the piece of code that generate the content of a tile and Fused takes care of running the code and providing the urls to share the layer in any application. The Python function that I have to [write](/guide/working-with-udfs/writing-udfs) is called a UDF and it has at least one parameter which contains the [bounding box (bounds)](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) on which I need to generate the tile.
...
The Fused [Map Builder](/workbench/udf-builder/canvas) enables one to view the result and logs while coding.
...
Now that everything is running fine in the workbench, it is time to extract the layers and include them in a webpage. Fused provides an easy way to integrate layers in external applications via [HTTPS requests](/guide/data-input-outputs/export-api/tokens-endpoints). You just need to go to Settings, click Share and copy the provided URL.

Then, you can integrate this URL as the tile source in any mapping application. I am not diving into that here, but you can read how to do this in the [DeckGL Fused docs](/guide/data-input-outputs/export-api/geospatial-export). You can check the code source of the demonstration below. Here is the extract of the JavaScript [Deck.gl](http://deck.gl/) code where the URL is integrated.
