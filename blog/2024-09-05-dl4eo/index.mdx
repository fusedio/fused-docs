...
Now that everything is running fine in the workbench, it is time to extract the layers and include them in a webpage. Fused provides an easy way to integrate layers in external applications via [HTTPS requests](/core-concepts/run-udfs/run-small-udfs/#shared-token). You just need to go to Settings, click Share and copy the provided URL.
