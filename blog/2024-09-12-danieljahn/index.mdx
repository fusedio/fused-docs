...
With its quickly growing list of integrations (getting data [in](/tutorials/others/in/) and [out](/tutorials/others/out/)) Fused is becoming a powerful glue layer for all of geospatial.

![](https://fused-magic.s3.us-west-2.amazonaws.com/docs_assets/ecosystem_diagram.png)

As an example, I've recently been able to [embed](/tutorials/others/out/notion/#2-embed-the-map-into-notion) an interactive map to a Notion page. The map downloads Landsat data from [Element 84's STAC catalog](https://earth-search.aws.element84.com/v1), computes a 6-month NDVI composite and displays it to the user. This all happens on the fly, as the user pans around.
...
Since any Fused UDF automatically serves raster and vector tiles, Fused can also be [used with Mapbox GL JS](/tutorials/others/out/mapbox/).
