...
Unlike Github, Fused will also run these functions for you. At a click of a button, you can get your function as an HTTPS endpoint, visualise it on an interactive map, or [wrap it in a Streamlit app](#5-geospatial-streamlit).
...
With its quickly growing list of integrations (getting data [in](/guide/data-input-outputs/read-write/geospatial/geospatial-reading) and [out](/guide/data-input-outputs/export-api/geospatial-export)) Fused is becoming a powerful glue layer for all of geospatial.
...
With its [caching layer](/guide/working-with-udfs/udf-best-practices/caching), the user can get the results of previously computed locations almost instantaneously.

Since any Fused UDF automatically serves raster and vector tiles, Fused can also be [used with Mapbox GL JS](/guide/data-input-outputs/export-api/geospatial-export).
