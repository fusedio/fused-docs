...
## Overview of Overture and GERS

Overture is an open data project that publishes interoperable map datasets. It aims to foster an ecosystem of developers creating downstream map services around its data products. Fused emerged as a solution to enrich datasets on the fly and serve them behind XYZ Tile endpoints.

### Why this matters

Datasets often lack the structure required to support the creation of reliable map services and products. A dataset may reference real-world entities using its own conventions, making it challenging to integrate with other datasets. Overture promotes interoperability with GERS (Global Entity Reference System) IDs, which link dataset entities to the same real-world features. This simplifies the process of conflating multiple datasets, ensuring consistent and accurate integration.

The Buildings theme provides a comprehensive collection of global building footprints, but different use cases might call for additional building-information. This could include risk assessment, census information, zoning regulations, property management details, and more. By enriching datasets with GERS IDs, users can easily link external data sources to power analytical applications.
...
## Step-by-step guide
In this post, we’ll show how to enrich your dataset by matching it with the Overture Buildings theme using GERS IDs. This makes your dataset easier to visually inspect and more accessible for others to consume and integrate with their own data.

For this example, we’ll create a Fused UDF to load the NSI Structures dataset and overlay it on the Overture Buildings dataset. This will let us visualize buildings based on attributes from NSI such as structure value or height. We’ll then turn the UDF into an XYZ Tile server to create an app.


### 1. Load Buildings data

Fused UDFs let us load, process, and export Overture dataset using spatial filters. This means we can load only a subset of columns for the areas we care about, without needing to first download the entire dataset.
...
While developing the UDF, the Fused UDF Editor allows us to dynamically visualize the output on the map.
...
def udf(bbox: fused.types.TileGDF=None):
    ...
    cols = ['id', 'ftprntsrc', 'bid', 'fd_id', 'geometry', 'class', 'ground_elv', 'min_height','roof_height','ground_elv_m', 'has_parts', 'height', 'val_struct']
...
Fused UDFs can be invoked via HTTP endpoints. The UDF we created returns a GeoDataFrame of the spatially joined tables, and Fused automatically provides an XYZ Tile endpoint. This allows seamless integration with mapping clients, enabling us to generate an enriched dataset in real-time and simultaneous serve it - without the need for additional processing layers.
...
This walkthrough provided a practical overview of how to create a UDF enrich a dataset with GERS IDs from Overture Buildings. We also saw how to use the UDF as a tile server to dynamically enrich and serve the dataset on the fly - directly on the browser.
