...
The NSI API provides access to NSI structures as a feature collection of points, which can be queried using GeoJSON. We'll use the same bounding box ([`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds)) that we used to load Overture data to query NSI, ensuring that each call to the UDF fetches data from both datasets for the spatially overlapping area.
