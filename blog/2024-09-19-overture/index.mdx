...
<iframe 
    ...
    src="https://www.youtube.com/embed/7x480zFrRr8" 
...
For this example, we created a Fused User Defined Function ([UDF](/guide/working-with-udfs/writing-udfs)) to load the [NSI Structures](https://www.hec.usace.army.mil/confluence/nsi/technicalreferences/latest) dataset and overlay it on the Overture Buildings dataset. This will let us visualize buildings based on attributes from NSI such as structure value or height. We'll then turn the UDF into an XYZ Tile server to create an app.
...
The NSI API provides access to NSI structures as a feature collection of points, which can be queried using GeoJSON. We'll use the same bounding box ([`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds)) that we used to load Overture data to query NSI, ensuring that each call to the UDF fetches data from both datasets for the spatially overlapping area.
...
While developing the UDF, the Fused Map Builder allows us to visualize the output on the map dynamically.
...
Fused UDFs can be invoked via HTTPS endpoints. The UDF we created returns a GeoDataFrame of the spatially joined tables, and Fused automatically provides an XYZ Tile endpoint. This allows seamless integration with mapping clients, enabling us to generate an enriched dataset in real-time and simultaneously serve it - without the need for additional processing layers.
