...
Overture is an open data project that publishes interoperable map datasets. It aims to foster an ecosystem of developers creating downstream map services around its data products. Fused emerged as a solution to enrich Overture datasets on the fly and serve them with [XYZ Tile endpoints](/core-concepts/filetile/#tile-endpoint).


import ReactPlayer from 'react-player'

<ReactPlayer className="video__player" playing={true} muted={true} controls height="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/blog-assets/jennings_video.mp4" width="100%" />



_This clip shows how coverage expands in (top right) Astoria when I add heights from the NSI dataset to Overture buildings._
...
The NSI API provides access to NSI structures as a feature collection of points, which can be queried using GeoJSON. We'll use the same bounding box ([\`bbox\`](/core-concepts/filetile/#the-bbox-object)) that we used to load Overture data to query NSI, ensuring that each call to the UDF fetches data from both datasets for the spatially overlapping area.
