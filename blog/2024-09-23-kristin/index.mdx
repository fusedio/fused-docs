...
Starting with the NetCDF predictor variable, which is notoriously tricky file type, Fused simplified the process. By using Fused's open source [utility modules](/python-sdk/top-level-functions/#utils), I loaded necessary functions directly into my UDF. I used them to clip the array, convert it into an image, and add a colormap.
...
Next, I merged the predictor variable with its geometry using nested UDFs in Fused. Then came the actual variable, the coarsest dataset, as a CSV paired with county shapefiles. I soon realized Fused has data assets that can quickly turn in county data to a udf. Here's where you find it.

**[TODO: Image of where to find data assets]**
