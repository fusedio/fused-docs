...
Now, let's get into the predictor variable—a NetCDF file from 5 degrees off the equator, covering around 25 square kilometers. NetCDF files can be a bit tricky to work with due to their complex formats, but Fused's [utility modules](/python-sdk/api-reference/fused-udf) make it easier. I imported some key functions directly into my UDF to clip the array, convert it into an image, and add a colormap.
...
```
Once I saved the UDF and created an [HTTPS endpoint](/guide/data-input-outputs/export-api/tokens-endpoints), I visualized the data interactively in the [App Builder](/workbench/app-builder/).
