...
This UDF queries the LP DAAC STAC catalog for data that matches the time and location of interest. This UDF then calls the previous one in parallel [asynchronously](/guide/working-with-udfs/udf-best-practices/realtime#async-execution) to fetch each cloud-free image in parallel. It then combines the outputs, taking the median of each band to create a cloud-free composite.
