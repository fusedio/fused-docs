...
_Example scenes highlight how cloud cover in the Sentinel-2 product can result in annual gaps as fewer valid observations lead to incomplete coverage. The right panel shows a cloudy image of Winter 2021 contrasted with a multi-year composite image of the same location._
...
async def udf():
    ...
    # Combine the arrays
    return np.mean(arrs, axis=0).astype(np.uint8)
```
