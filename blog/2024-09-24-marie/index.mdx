...
_Example scenes highlight how the HLS product can have annual gaps, as fewer valid observations, which lead to incomplete coverage. The lower panels compare median composites of 2020 scenes with those from dry seasons between 2015 and 2020, which show a more uniform, cloud-free view compared to annual medians._

## Obstacles to create custom HLS imagery
...
### 2. Call the UDF asynchronously and combine array of images
...
The best part about Fused's UDF Editor is that our Data Science team can design UDF while looking at a specific area, and to run it for a different region by simply changing the input bounding box (bbox). The team can easily experiment and generate composites for different date ranges by adjusting the input parameters. Once we are happy with the UDF algorithm, we can easily create individual image tiles for any location worldwide.
