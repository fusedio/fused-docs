...
This UDF queries the LP DAAC STAC catalog for data that matches the time and location of interest. This UDF then calls the previous one in parallel [asynchronously](/guide/getting-started/udf-best-practices/realtime#async-execution) to fetch each cloud-free image in parallel. It then combines the outputs, taking the median of each band to create a cloud-free composite.
...
- Invoke UDF and load its data into a Jupyter Notebook with [`fused.run`](/guide/getting-started/udf-best-practices/realtime) for downstream analysis
