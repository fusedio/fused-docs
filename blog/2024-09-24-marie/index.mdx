...
This UDF queries the LP DAAC STAC catalog for data that matches the time and location of interest. This UDF then calls the previous one in parallel [asynchronously](/python-sdk/api-reference/fused-submit) to fetch each cloud-free image in parallel. It then combines the outputs, taking the median of each band to create a cloud-free composite.
...
- Iterate on both UDFs in the same code editor with the [Map Builder](/workbench/udf-builder/canvas)
- Instant feedback during algorithm development, no need to wait for pipelines to run
- Invoke UDF and load its data into a Jupyter Notebook with [`fused.run`](/guide/working-with-udfs/udf-best-practices/realtime) for downstream analysis
