...
Using Fused, we found a solution that bridged the gap between our client applications and our data warehouse seamlessly. Fused's [User Defined Functions](/guide/working-with-udfs/writing-udfs) (UDFs) enabled us to read from Snowflake and deliver data behind remote Parquet file endpoints for DuckDB to consume directly and render on the Honeycomb map.
