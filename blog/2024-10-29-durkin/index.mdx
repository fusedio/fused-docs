...
The rate of prototyping the [Fused App Builder](/workbench/app-builder/) unlocks is unrivaled. I used it to create a fairly sophisticated prototype app to run Fused User Defined Functions ([UDFs](/core-concepts/write/)) with [Streamlit](https://streamlit.io/), a tool to build Python apps I was introduced to just three weeks ago. In this blog post I explain the problem I experienced as a Data Scientist to create training datasets for a machine learning model, and how I solved it with Fused. You can follow along by using the ["Cube Factory" app](https://www.fused.io/workbench/apps#app/s/i/fa_ya5NILRuwOVfqa79A1GJF) I built.
...
The field of AI stands to revolutionize the field of Earth observation, unlocking unprecedented insights from satellite imagery. This article shows how you can leverage multiple layered Fused components, from the serverless compute underpinning everything else, to the highly customizable python-based Fused UDFs to the top layer of the new Fused app platform. We will harness these resources to curate training datasets that can streamline deep learning workflows. As a working example, we will demonstrate land use inference and crop detection with multispectral satellite imagery.
...
- [Sentinel 2](https://sentiwiki.copernicus.eu/web/s2-mission) offers 13-bands of 10 meter resolution imagery with broad temporal and spatial coverage. We chose the Glacier index as the Pseudo Red Channel and Channels 8 (NIR) and 11 as Green and Blue, respectively:
...
7. Finally hit Export to create a parquet record of your chosen datacube collection along with survey time period and all relevant metadata. This will be saved to your account's S3 bucket in the Fused File System - if you have one.
...
<ReactPlayer className="video__player" playing={true} muted={true} controls height="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/blog-assets/gabe_slider_HD.mp4" width="100%" />
...
To replicate this work for your own AOIs, you can [try out the Cube Factory app](https://www.fused.io/workbench/apps#app/s/i/fa_ya5NILRuwOVfqa79A1GJF) yourself or run the underlying UDFs on your own Fused Workbench environment. Please feel free to contact me on [LinkedIn](https://www.linkedin.com/in/gabrieldurkin/).
