...
<iframe 
    ...
    src="https://www.youtube.com/embed/cyX75eS6s1M" 
...
- **Python User Defined Functions ([UDFs](/guide/working-with-udfs/writing-udfs)):** UDFs define transformations on data and can easily be called with parameters and across different areas of interest.
- **Remote Accessibility:** UDFs can be called from anywhere via [HTTPS requests](/guide/data-input-outputs/export-api/tokens-endpoints), delivering data exactly where it's needed without the need to store or transfer large datasets.
- **Parallel Execution:** UDFs can [run in parallel](/python-sdk/api-reference/fused-submit), processing thousands of data chips per minute for efficient scaling.
- **UDF Workbench:** The [Map Builder](/workbench/udf-builder/canvas) provides instant feedback during algorithm development, allowing users to visually inspect the resulting data chips on a dynamic map. Any changes are immediately deployed upon saving.
