...
Traditionally, we've used QGIS with GeoParquet as our file storage for visualizations. However, the sheer amount of data generated from each site at H3 resolution 15 has made it difficult to filter outputs interactively in QGIS. Fortunately, Fused [HTTP endpoints](/core-concepts/run-udfs/run-single-udfs/#http-requests) make it easy to dynamically [integrate UDFs with QGIS](/user-guide/out/qgis/).
