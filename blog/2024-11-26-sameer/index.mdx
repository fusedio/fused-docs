...
Traditionally, we've used QGIS with GeoParquet as our file storage for visualizations. However, the sheer amount of data generated from each site at H3 resolution 15 has made it difficult to filter outputs interactively in QGIS. Fortunately, Fused [HTTPS endpoints](/guide/data-input-outputs/export-api/tokens-endpoints) make it easy to dynamically [integrate UDFs with QGIS](/guide/data-input-outputs/export-api/geospatial-export).
