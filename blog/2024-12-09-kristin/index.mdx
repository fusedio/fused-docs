...
And that's just for one time period. If we run a model on 2–4 time periods, we're looking at nearly **100 million data points**. Now, building a model on 100 million data points isn't trivial, but with the compute technology at Fused, this process becomes a breeze.
...
| Date  | State | County | GEOID | SIF_Corn | Percent_Corn | Number of Acres/Pixels | Yield  (bu/acre) (function of “date” and “GeoID” |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
Now the really fun part. I get to run all sorts of different models. Auto Regressive model because it predicts the value of a variable based on its own past values, which I think will work well for crop yields. But for that….you will have to wait on the next blog post. See you next ime!
