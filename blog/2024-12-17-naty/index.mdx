...
I created a UDF to read Parquet files stored on S3 and filter them it based on a given bounding box and column values. It first executes a query lazily directly on the S3 dataset using DuckDB and Ibis, filters by infrastructure subtypes (e.g., pedestrian and water), and further filters it by specific infrastructure classes like toilets, ATMs, and information signs.

The UDF also prints the count of each infrastructure class within the filtered results and renames columns to show as output.
