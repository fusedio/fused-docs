...
Using Ibis with DuckDB as the backend, we can access the Overture Maps data directly from the cloud. These queries will only fetch the attributes and rows that meet specified conditions, minimizing the data processed locally. You can learn more about it in this DuckDB [blog](https://duckdb.org/2021/06/25/querying-parquet.html).
