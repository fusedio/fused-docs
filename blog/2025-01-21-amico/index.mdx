...
<ReactPlayer playsinline={true} className="video__player" playing={true} muted={true} controls height="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/blog-assets/fire_buffer_h3.mp4" width="100%" />
...
- [Fire Proximity Buffer UDF](https://www.fused.io/workbench/catalog/Fire_Proximity_Buffer-4e1fbf84-ea8b-4758-8b09-a32d35b545ca)
...
4. Roll-up categories by H3, create categories primary set
...
As next steps, we could create apps or API services that deliver derivative products, such as GERS lookups to categorize "fire risk" based on buffer proximity. We could provision HTTP endpoints from the UDF to return a CSV with the GERS and "Fire Risk" score for buildings within a defined bounding box, as specified by a query parameter. Additionally, we could also create HTTP endpoints to automatically enrich the "Fire Risk" column of an arbitrary dataset for any given Building GERS. Users could apply it to any row of their table, with the functionality powered by a "GERS lookup" endpoint using a GERS ID query parameter.
