...
- [Overture Maps Docs "Examples" Page](https://docs.overturemaps.org/tutorials/others/fused/)
...
This UDF loads the Overture and NSI datasets, performs a spatial join to enrich the building polygons with hazard metadata, and returns the enriched GeoDataFrame. It can be used within a larger analysis workflow to enrich building polygons to calculate risk indices. You can read more about performing a point in polygon operation to enrich Overture Buildings with NSI [here](/tutorials/others/transform/geospatial/point-in-polygon/).
