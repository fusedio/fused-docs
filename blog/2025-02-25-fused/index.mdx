...
- Moving away from Fused having [many different `fused.types`](/tutorials/Geospatial%20with%20Fused/filetile/#legacy-types) to only having 2 simple types: [`fused.types.Bounds`](/tutorials/Geospatial%20with%20Fused/filetile/#fusedtypesbounds) and [`fused.types.Tile`](/tutorials/Geospatial%20with%20Fused/filetile/#tile).
- Replacing `bbox` object with [`bounds`](/tutorials/Geospatial%20with%20Fused/filetile/#the-bounds-object): a more generic term to pass geometries to UDFs

#### [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#running-jobs-in-parallel-fusedsubmit) for simple, multi job runs
