...
- Moving away from Fused having [many different `fused.types`](/tutorials/Geospatial%20with%20Fused/filetile/#legacy-types) to only having 2 simple types: [`fused.types.Bounds`](/tutorials/Geospatial%20with%20Fused/filetile/#bounds) and [`fused.types.Tile`](/tutorials/Geospatial%20with%20Fused/filetile/#tile-udf).
- Replacing `bbox` object with [`bounds`](/tutorials/Geospatial%20with%20Fused/filetile/#bounds): a more generic term to pass geometries to UDFs
