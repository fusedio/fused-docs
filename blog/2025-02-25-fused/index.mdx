...
- Moving away from Fused having [many different `fused.types`](/guide/working-with-udfs/geospatial/single-vs-tile#legacy-types) to only having 2 simple types: [`fused.types.Bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds) and [`fused.types.Tile`](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf).
- Replacing `bbox` object with [`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds): a more generic term to pass geometries to UDFs
...
- Editing the cache duration with the new [`cache_max_age` argument](/guide/working-with-udfs/caching)

[Read more about Caching](/guide/working-with-udfs/caching)
