...
UDFs can be called via HTTP endpoints, allowing them to execute and return results.
...
Workbench by default renders `gpd.GeoDataFrame`,  `gpd.GeoSeries`, and `shapely geometry` on the map.

Fused also renders on the map array (raster) objects with spatial metadata, like `xarray.DataArray`. Arrays without spatial metadata, like `numpy.ndarray`, can be can have their spatial extent specified with a geometry object or an array bounds as `[xmin, ymin, xmax, ymax]`. If the bounds are not present, they default to `(-180, -90, 180, 90)`. For example: 
