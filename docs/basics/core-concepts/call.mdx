...
Each UDF in Fused can be called as an API endpoint. When a client application invokes this endpoint, Fused executes the UDF and returns its output.

## File & Tile

The endpoint can return data as if it were a single remote `File`. The same endpoint can also be called dynamically as a `Tile`. To understand the different behavior and advantages between the two, consider this diagram. 

When a UDF is called as a single remote File, every polygon would be returned in the same response. However, processing large-scale spatial data, it's advantageous to handle partitions individually to efficiently use memory and network resources. 

When a UDF is called as a Tile, the geometry can be returned across one or more File calls, each containing data for one cell of the grid. These tiles are based on predefined [tile grid](https://wiki.openstreetmap.org/wiki/Slippy_map).
...
When an endpoint is called as a `File`, the UDF runs only once and returns all output data in a single response. This behaves like the standard access pattern for a remote file URL (such as with an S3 endpoint).
...
When and endpoint is called as a `Tile`, Fused provides a `bbox` as the first argument, which the UDF can use to filter the dataset spatially. The `bbox` contains spatial information about the Tile, which makes it interoperable with Tiled web map clients. The endpoint is called as a `File` for every `Tile` requested. 
...
One of Fused's most powerful features is its first-class support for [cloud-optimized data formats](https://guide.cloudnativegeo.org/). Fused can efficiently access geospatial data on the fly, which reduces latency and transfer costs. 

A UDF can use the `bbox` parameter to spatially filter a datasets and distributes compute across multiple workers that scale automatically. Tile-level spatial filtering supercharges UDFs to process only specific parts of a dataset - based on specified geographic or logical partitions.
