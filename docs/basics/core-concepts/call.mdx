...
Each UDF in Fused can be called as an API endpoint. When invoked by a client application, the UDF executes and returns its output.

## File & Tile

UDFs can be called in two modes: `File` and `Tile`.
- `File`: The UDF runs and returns the entire result data in one HTTP response. This is efficient for smaller datasets where processing the data in a single request is feasible.
- `Tile`: When working with geospatial datasets, the UDF can break the data into grid-based tiles that align with a predefined [web map tiling schema](https://wiki.openstreetmap.org/wiki/Slippy_map). Each tile corresponds to a segment of the data, which allows more efficient handling of large-scale geospatial datasets. 
