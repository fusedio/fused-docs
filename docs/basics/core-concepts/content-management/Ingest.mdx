...
The [`fused.ingest`](/python-sdk/top-level-functions/#ingest) method uploads
the specified dataset with [fused.upload](/python-sdk/top-level-functions/#upload),
geopartitions it, and writes a parquet table to S3. Each parquet file has one or more chunks, which are a further level of spatial partitioning. The Fused utility function `table_to_tile` is can read the table and load only relevant chunks that fall within a specified polygon.

An ingested dataset contains a `main` table with the original input columns and a `fused` table containing spatial metadata.

The `ingest()` method has many configuration options, which the API documentation
explains. The following sections cover common ingestion use cases.
