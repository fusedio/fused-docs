...
TODO: Fused supports geospatial rendering, but since map file table is geodataframe, already contains geometry and can be rendered directly. in the case of raster, if the raster does not containt spatial info, need to pass bounds to render it. at fused workbench only accept uint8 to be able to render on the map. 
todo: It's possible to pass bounds, represented by xmin, ymin, xmax, ymax, represented with red box.
...
The `bbox` object defines the spatial bounds of the Tile, which can be represented as a geometry object or XYZ index. For convenience, a user writing a UDF can select from 3 `bbox` object structures with a [parameter type](/basics/core-concepts/write/#typed-parameters).
