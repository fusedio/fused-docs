...
Every UDF has a secure HTTP endpoint which can be called via a [web request](/basics/core-concepts/run/#via-http-requests) or [`fused.run`](/basics/core-concepts/run/#fusedrun). This guide covers both approaches and how they can be configured. These endpoints can be accessed with a [shared token](/basics/core-concepts/run/#shared-token) or [private token](/basics/core-concepts/run/#private-token). The shared tokens can be created and revoked [via workbench] and [Fused Python SDK]. 


## `fused.run`

The `fused.run` function runs the UDF specified in its first argument, and passes subsequent arguments to the UDF. The UDF can be specified by name, using a shared token, or directly passing a UDF object. The UDF object can be loaded using `fused.load` or declared in your code. 

### Ways to run a UDF

**Using public id:** Run a public UDF by specifying its public id. This does not require authentication.

```python
fused.run('fsh_PC_File_Example')
```

**Using shared token:** Run using a [shared token](...). This does not require authentication and runs on the UDF account that provisioned the shared token. Read more about [shared tokens](/).
...
**Using name with private token:** Run by specifying the UDF's name and owner's user account. This requires authentication and access to the referenced UDF.

```python
fused.run("user@fused.io", "Overture_Maps")
```

**Using GitHub with private token:** Run a UDF from a GitHub repository by specifying the UDF's URL in a GitHub repo. This requires authentication and access to the referenced UDF.
...
**Using UDF object:** Declare or load the UDF then run it by passing the udf object. This requires authentication unless the UDF is ran locally.
