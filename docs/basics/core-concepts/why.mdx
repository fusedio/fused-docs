...
Fused UDFs are reusable and modular Python functions that can anywhere.
...
UDFs serve as modular building blocks, abstracting key operations to power workflows and applications — without the need for devops or infrastructure management. 
...
UDFs also function like reusable and composable readers for different file formats. The File Explorer can list, preview, and create UDFs from objects in S3 buckets - without the need to move or copy entire datasets. This facilitates data discovery and exploration of cloud datasets. 
## Composability: Reusable Operations

UDFs are modular building blocks that can be assembled into complex workflows. They function as reusable tasks in analysis pipelines that process data on-demand. They run and load data across applications via simple HTTP requests. 
