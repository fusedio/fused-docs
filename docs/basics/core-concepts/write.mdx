...
To write UDFs that can be called as either [`File` or `Tile`](/basics/core-concepts/filetile/), Fused recommends setting `bbox` as the first parameter, defaulted to `None`. This enables the UDF to be invoked successfully both as `File` (when `bbox` isn’t passed) and as `Tile`. For example:
...
UDFs called with [`fused.run`](/basics/core-concepts/run/#fusedrun) run as Tile only if a parameter specifies the `Tile` geometry. 
:::
...
A common approach to debug UDFs in the [Map builder](/workbench/map-builder) is to introduce `print` statements along the code to show intermediate results in the [results](/workbench/map-builder/results/) panel. 
