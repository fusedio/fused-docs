...
First decorate a Python function with `@fused.udf`. This makes the function run in the Fused cloud, which can provision and scale compute. 
...
UDFs can optionally accept parameters and execute code based on those parameters. Applications calling the UDF can pass arguments to the UDF to change its behavior. When writing a UDF, you can explicitly type the parameters to ensure the UDF receives the correct data type.
