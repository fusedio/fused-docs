...
Fused resolves each argument as the types specified in the function signature. For example this function has typed parameters.
...
The returned raster object must have a geospatial component. This tells Fused where on the map to render it as an image. 
...
For example, this UDF returns an `xarray` `DataArray`, which inherently contains coordinates that tell Fused where on the map to place it. Verify this by printing the object and its type.
...
The print statements above should display the following in the stdout box, which shows the layer value and coordinates of the `DataArray` object.
