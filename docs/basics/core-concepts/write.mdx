...
- [Declare the function](/basics/core-concepts/write/#function-declaration)
...
- Set a vector table or raster as a [return object](/basics/core-concepts/write/#return-object)
- How to [save UDFs](/basics/core-concepts/write/#save-udfs) 
...
When writing a UDF you can use the [@fused.cache](/basics/core-concepts/content-management/cache/) decorator to persist a function's output across runs, helping with faster code development.
...
Fused also supports the geospatial types `fused.types.Bbox`, `fused.types.TileXYZ`, and `fused.types.TileGDF`. When one of these types is assigned to the first parameter of the function ([read more](/basics/core-concepts/filetile/#the-bbox-object)) Fused Workbench will handle the UDF as a "Tile" otherwise as a "File". 
