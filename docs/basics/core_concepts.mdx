...
When an endpoint is called as a `File`, the UDF runs only once and returns a all output data in a single batch. This behaves like the access pattern for a remote file URL.
...
##### `fused.types.TileGDF` 
This is a `geopandas.geodataframe.GeoDataFrame` with `x`, `y`, `z`, and `geometry` columns.
...
##### `fused.types.TileXYZ` 
This is a `mercantile.Tile` object with values for the the `x`, `y`, and `z` Tile indices.
