...
This method involves dividing GeoTiffs into a grid of manageable cell sections that fit in memory, then asynchronously processing each cell by extracting the relevant portion of the GeoTIFF, performing zonal stats, stitching the results, and re-computing aggregate statistics based on the combined results. This approach allows for efficient processing of any size datasets.
