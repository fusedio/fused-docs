...
| Flag | Description |
|---|---|
| `--minute INT_LIST` | Minute (0-59); exactly one value. Repeat the flag and/or use comma-separated values. (default: `Sentinel.UNSET`) |
| `--hour INT_LIST` | Hour (0-23). Repeat the flag and/or use comma-separated values. (default: `Sentinel.UNSET`) |
| `--day-of-month INT_LIST` | Day of month (1-31); omit for every day. Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--month INT_LIST` | Month (1-12); omit for every month. Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--day-of-week INT_LIST` | Day of week (0=Sunday); omit for every day. Repeat the flag and/or use comma-separated values. (default: `()`) |
...
|---|---|
| `--id` | Treat the first argument as a cronjob UUID instead of CANVAS. |
| `--day-of-week INT_LIST` | Day of week (0=Sunday, 6=Saturday). Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--month INT_LIST` | Month (1-12). Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--day-of-month INT_LIST` | Day of month (1-31). Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--hour INT_LIST` | Hour (0-23). Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--minute INT_LIST` | Minute (0-59); exactly one value. Repeat the flag and/or use comma-separated values. (default: `()`) |
| `--udf-args TEXT` | JSON object of arguments passed to the UDF when the schedule runs. |
| `--disabled` | Disable the schedule. |
| `--enabled` | Enable the schedule. |
