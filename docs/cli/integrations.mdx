...
|---|---|
| `list` | Show all providers and their connection status |
| `<provider> connect` | Start an OAuth flow |
| `<provider> token` | Print a short-lived access token |
| `<provider> revoke` | Disconnect and remove stored tokens |
...
- [Using integrations inside UDFs](/guide/working-with-udfs/udf-best-practices/ai-coding-tools) — connect to third-party services from UDF code
