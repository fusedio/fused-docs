...
Asynchronous execution can be used to call UDFs concurrently using the [async/await](https://docs.python.org/3/library/asyncio.html) syntax.

A common use case is to call a UDF multiple times in parallel with different parameters then combine the results. For example, the following UDF calls `udf_to_run_async` asynchronously with the [`run_pool`](https://github.com/fusedio/udfs/blob/9bfb5d0/public/common/utils.py#L522) utility function 10 times and concatenates the results.
