...
:::note
    Setting sync=False in fused.run is intended for asynchronous calls when running in the cloud with engine='realtime'. The parameter has no effect if the UDF is ran in the local environment with engine='realtime'.
:::
