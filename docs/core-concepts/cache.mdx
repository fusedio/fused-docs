...
- Cached for 90 days by default (can be changed with [`cache_max_age`](/core-concepts/cache/#defining-your-cache-lifetime-cache_max_age))
...
However, be wary of relying on [`@fused.cache`](/python-sdk/top-level-functions/#fusedcache) to load very large (>10Gb) datasets as cache is only stored for a limited amount of time and is over-written each time you change the cached function or inputs.
