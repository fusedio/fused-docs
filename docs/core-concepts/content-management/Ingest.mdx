...
To start an ingestion job, call [`run_remote`](/reference/fused/experimental/#jobrun_remote) on the job object returned by [`fused.ingest`](/python-sdk/top-level-functions/#ingest).
...
View the ingestion job status & logs in [fused.io/profile#jobs](https://www.fused.io/profile#jobs). Monitor and manage the job with the following methods:
