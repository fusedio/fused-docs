...
{/* TODO: Need to introduce `fused.ingest()` first as the main tool:
- ingesting vector data specifically
- using Fused algorithm to geo-partition vector data (share Chris Holmes article showing merits of our approach: https://github.com/opengeospatial/geoparquet/discussions/251) 
- Outcome is geoparquet files on S3 (accessible even outside of Fused)
// */}
...
We've built our own ingestion pipeline at Fused that partitions data based on dataset size & location. This is defined with [`fused.ingest()`](/python-sdk/top-level-functions/#ingest). Our ingestion process:
1. Uploads the `input`
...
Ingestion jobs often take more than a few seconds and require a lot of RAM (we open the whole dataset & re-partition it), which makes [this a large run](/core-concepts/run-udfs/run_large/#defining-large-jobs) so we're going to use `run_remote()` to run it:
...
{/* TODO: This needs to be moved to `run_remote()` section. This isn't about ingestion, it's about running large jobs */}
View the ingestion job status and logs in [fused.io/workbench/jobs](https://www.fused.io/workbench/jobs). Monitor and manage the job with the following methods:
