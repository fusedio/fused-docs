...
This `bbox` is purposefully small (`print(smaller_bbox.iloc[0].geometry.area)` returns `0.02`) to highlight loading a very large dataset and recover only a small portion of data.
...
{/* TODO: Write another UDF to save CSV as parquet to another file */}
{/* NOTE:
@fused.udf
def ais_to_parquet():
    import pandas as pd
    csv_df = pd.read_csv("s3://fused-users/fused/demo_reading_ais/AIS_2024_01_01.csv")
    csv_df.to_parquet("s3://fused-users/fused/demo_reading_ais/AIS_2024_01_01.parquet")
...
def from_parquet_udf(
    bbox: fused.types.ViewportGDF,
    path: str="fd://demo_reading_ais/AIS_2024_01_01.parquet"
):
    import geopandas as gpd
    import pandas as pd
    # Sticking to pyarrow over fastparquet as that's not installed by default on fused it seems
...
Now, we're going to ingest the parquet file we have to create geo-partitioned files, using `fused.ingest()`. We'll go in more details on how to ingest your own data in [the following section](/core-concepts/data%20ingestion/ingestion-your-data/).
...
```python
job.run_remote(
    instance_type='r5.8xlarge', # We're using a machine with a large amounts of CPUs to do the partitioning in parallel
)
```
...
Our ingestion process broke down our dataset into smaller files (making each file easier to access) and a `_sample.parquet` file containing the bounding box of each individual file. This allows us to first intersect our `bbox` with `_sample` and then only open the smaller `.parquet` files we need:

```python showLineNumbers
@fused.udf
def read_ingested_parquet_udf(
    bbox: fused.types.TileGDF,
    path: str = "s3://fused-users/fused/file_format_demo/ingested/",
    preview: bool = False,
):
    import fused
    import pandas as pd
...
{/* TODO: Could also mention using @fused.cache to show how to make a CSV read a lot faster when wanting to do a quick visual analysis of a CSV for 1 time. With caveat of /mnt/cache (dissapears after a day & cache has to rerun on each change and slow the first time) */}
