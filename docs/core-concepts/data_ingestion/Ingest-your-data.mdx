...
Ingest using a [partition schema](/python-sdk/top-level-functions/#ingest) that has been defined from a previous ingestion. This may be useful if you are analyzing data across a time series and want to keep the bounding boxes consistent throughout your analysis.
...
def read_ingested_parquet_udf(path: str = "s3://sample-bucket/ingested_data/first_set/"):
    ...
    # Since we want our `partition_schema_input` specified in `ingest()` to be a link to a parquet file containing bbox coordinates, we will save this metadata
...
<details>
  <summary>Differences in bounding boxes from ingested AIS data on different days </summary>
