...
def read_ingested_parquet_udf(path: str = "s3://sample-bucket/ingested_data/first_set/"):
    ...
    # Since we want our `partition_schema_input` specified in `ingest()` to be a link to a parquet file containing bbox coordinates, we will save this metadata as a parquet file
