...
You can read more about ingestion and when it's required [here](/core-concepts/data_ingestion/why-ingestion/#when-is-ingestion-needed)
</details>
...
To configure the integration, [connect your GitHub repository](/core-concepts/content-management/git/#connect-your-github-repository) and provide us with the repository name and details. We'll activate it for you.
</details>
...
Save environment variables to an `.env` file as shown [here](/core-concepts/content-management/environment-variables/) to make them available to UDFs as environment variables. Secrets can be stored in the [secrets manager](/workbench/configuration/#secrets-management).
</details>
...
<details>
<summary>How can I change the basemap in the UDF Builder?</summary>

Set it in the map style settings, located at the top right of the [UDF Builder map](/workbench/udf-builder/map/). Currently, light, dark, satellite, and blank basemaps are supported.

import ImgChangeBasemap from '@site/static/img/change_basemap.png';

<img src={ImgChangeBasemap} alt="File" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
...
<details>
<summary>What is "Zoom to layer" and when would I use it?</summary>

If a [default view state](/workbench/udf-builder/code-editor/#default-view-state) is set for the UDF, the viewport will zoom to that predefined extent whenever the UDF first loads or when the user clicks the "Zoom to layer" button. This feature is useful for centering the map on a specific area of interest when users open the UDF.
...
<details>
<summary>How can I run a UDF asynchronously?</summary>

To run a UDF asynchronously, use Python's async/await syntax as shown [here](/core-concepts/async/). This approach allows multiple UDF calls to run in parallel, each with different parameters, and enables you to combine the results once all calls are complete.
...
Fused batch jobs, which are initiated with [run_remote](/python-sdk/top-level-functions/#jobrun_remote), require a server quota to be enabled for your account. These include data [ingestion](/core-concepts/data_ingestion/) jobs. If you encounter this error, please contact the Fused team to request an increase in the quota allotted to your account.
</details>
