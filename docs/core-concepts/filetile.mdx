...
A UDF called via an [HTTP endpoint](/core-concepts/run-udfs/run-single-udfs/#http-requests) is invoked as `File` or `Tile`, depending on the URL structure.
...
A UDF called with [`fused.run`](/core-concepts/run-udfs/run-single-udfs/#fusedrun) runs as `Tile` when the `Tile` geometry is specified with [reserved parameters](/core-concepts/write/#reserved-parameters) in the following ways. Otherwise, it runs as `File`.
