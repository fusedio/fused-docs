...
A UDF called with [`fused.run`](/core-concepts/run/#fusedrun) runs as `Tile` when the `Tile` geometry is specified with [reserved parameters](/core-concepts/write/#reserved-parameters) the following ways. Otherwise, it runs as `File`.
