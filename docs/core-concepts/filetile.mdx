...
This endpoint structure runs a UDF as a `Single`. See implementation examples with [Felt](/examples/out/felt/#vector) and [Google Sheets](/examples/out/googlesheets/#2-create-a-url-for-the-udf) for vector.
...
This endpoint structure runs a UDF as a `Tile`. The `{z}/{x}/{y}` templated path parameters correspond to the Tile's XYZ index, which Tiled web map clients dynamically populate. See implementation examples for Raster Tiles with [Felt](/examples/out/felt/#raster-tiles) and [DeckGL](/examples/out/deckgl/#raster-tile-layers), and for Vector Tiles with [DeckGL](/examples/out/deckgl/#vector-tile-layers) and [Mapbox](/examples/out/mapbox/#a-vector-tile-layers).
