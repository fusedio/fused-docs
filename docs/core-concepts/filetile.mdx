...
- Simple explanation of `Tile` & `Bounds`
    - `TileGDF` is now just `Tile`
    - `Bounds` is a simple bbox
...
A UDF may use the `bbox` parameter to spatially filter datasets and load into memory only the data that corresponds to the `bbox` spatial bounds. This reduces latency and data transfer costs. 
[Cloud-optimized formats](https://guide.cloudnativegeo.org/) are particularly suited for these operations - they include [Cloud Optimized GeoTiff](https://www.cogeo.org/), [Geoparquet](https://geoparquet.org/), and [GeoArrow](https://geoarrow.org/format.html).
...
{/* TODO: Rename input to `bounds` here? If so then change this doc section away from `bbox`, and called something else */}
```python showLineNumbers
@fused.udf
