...
Some jobs require more resources than a few Gb of RAM or run in more than a few seconds. This section will show how to run such larger jobs
...
- Require large resources (more than a few Gb of storage or RAM)
...
As mentioned in the [Small UDF job section](/core-concepts/run-udfs/run-single-udfs/), to call it 1 time we can do:
...
To run UDFs on a larger instance, we need to define the UDFs to run and then send them to a remote instance:
...
- `instance_type`: Decide which type of machine to run your job on (based on some of the [AWS General Purpose instance types](https://aws.amazon.com/ec2/instance-types/))
...
<details>
  ...
  Fused `run_remote()` doesn't support all of the AWS General Purpose instances types. We do support:
