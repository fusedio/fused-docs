...
## Running a large job: `job.run_batch()`
...
As mentioned [above](/core-concepts/run-udfs/run_large/#running-a-large-job-jobrun_batch) to pass UDF arguments to a remote job, use `arg_list` to specify a `list` of inputs to run your job over:
...
To get data back from your `run_batch()` jobs is a bit more complicated than for ["real-time"](/core-concepts/run-udfs/run-small-udfs/#real-time-logs). Our recommendation is to have your UDF write data directly to shared storage `/mount/` or cloud storage and access it after
