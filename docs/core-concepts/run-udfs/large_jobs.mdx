...
- `instance_type`: Decide which type of machine to run your job on (see [below](/core-concepts/run-udfs/run_large#currently-supported-instance_type) for which ones we support)
...
### Currently supported `instance_type`
  ...
  gcp_instance_types = [
    ...
    "c2-standard-4": 2.0,  # 4 vCPUs
    "c2-standard-8": 4.0,  # 8 vCPUs
    "c2-standard-16": 8.0,  # 16 vCPUs
    "c2-standard-30": 16.0,  # 30 vCPUs
    "c2-standard-60": 32.0,  # 60 vCPUs
    # Memory optimized - M3 family
    "m3-ultramem-32": 24.0,  # 32 vCPUs
    "m3-ultramem-64": 48.0,  # 64 vCPUs
  ]
  ```
...
To run a job from workbench, you can simply add an [`instance_type`](/core-concepts/run-udfs/run_large#currently-supported-instance_type) parameter and run the UDF as usual from the udf builder page:
