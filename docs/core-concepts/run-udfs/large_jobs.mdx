...
```python showLineNumbers
job = udf(arg_list=[0,1,2,3])
job.run_remote()
```

**Multiple Parameters**

Currently `arg_list` can only be used to pass a single parameter to the UDF we'll run. We can work around this by aggregating multiple variables into a `dict` and having a UDF take only a `dict` as input:
...
We can then confirm this worked by viewing our results by browsing [File Explorer](/workbench/file-explorer/):
