...
At the moment UDFs can only be passed to `run_remote()` jobs with 1 parameter through `arg_list`. We can work around this by passing a dictionary to `arg_list`:
...
We can then see our results by browsing [File Explorer](/workbench/file-explorer/):
