...
We'll use the same UDF as in the [running multiple UDFs section](/core-concepts/run-udfs/run/#running-jobs-in-parallel-fusedsubmit):
...
As mentioned in the [Run UDFs section](/core-concepts/run-udfs/run/#fusedrun), to call it 1 time we can use `fused.run()`:
...
## Running a job: `job.run_batch()`

Running a UDF as a job is done in 2 steps:
...
To get data back from your `run_batch()` jobs is a bit more complicated than for ["regular UDF runs"](/core-concepts/run-udfs/run/#real-time-logs). Our recommendation is to have your UDF write data directly to shared storage `/mount/` or cloud storage and access it after
