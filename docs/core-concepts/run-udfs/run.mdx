...
```python showLineNumbers
# It's recommend to pin to a specific commit
output = fused.run("https://github.com/fusedio/udfs/tree/main/community/sina/Eiffel_Tower_Point")
```

:::warning pinning to git commit

We do STRONGLY recommend you use [git commit hash](/core-concepts/run-udfs/run-small-udfs/#git-commit-hash-recommended-for-most-stable-use-cases) rather than using `main` to pin your UDF to a specific version
...
Aim for a single UDF that takes 30-45s. This gives you a "safety" margin as UDFs will [timeout after 120s](/core-concepts/run-udfs/run-small-udfs/#running-udfs). So they can still take a bit longer and not time out.
:::
...
[`fused.submit()`](/python-sdk/top-level-functions/#fusedsubmit) is used in many places across our docs, here are some examples:

- 📈 Processing 20Tb of data in minutes: [Making a Climate Dashboard](/tutorials/Geospatial%20with%20Fused/use-cases/Climate%20Dashboard/) of 20 years of data
- ⛴️ [Retrieving 30 days](/tutorials/Geospatial%20with%20Fused/use-cases/dark-vessel-detection/#33---run-this-udf-over-a-month-of-ais-data) of AIS boat transponder data around the United States to detect illegal fishing
