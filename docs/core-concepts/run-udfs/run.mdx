...
Sometimes you want to run a UDF over a list of inputs (for example running a [UDF that unzips a file over a list of files](/examples/dark-vessel-detection/#33---run-this-udf-over-a-year-of-ais-data)). If each run itself is quite small, than you can run all a batch of UDFs over a list of inputs.
...
- ⛴️ In the Dark Vessel Detection example to [scale retrieving](/examples/dark-vessel-detection/#33---run-this-udf-over-a-month-of-ais-data) daily AIS `.zip` files from NOAA over 30 days.
- 🛰️ [Retrieving all of Maxar's Open Data STAC Catalogs](/examples/exploring_maxar_data/#preparing-fusedsubmit-to-run-in-parallel) across every events they have imagery for.
