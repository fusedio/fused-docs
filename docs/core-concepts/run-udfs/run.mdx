...
1.  [`fused.run()`](/python-sdk/top-level-functions/#fusedrun) in Python. All you need is the [`fused` Python](/python-sdk/#install) package installed
...
2.  [**HTTP call**](/core-concepts/run-udfs/run-small-udfs/#http-requests) from *anywhere*
    - Useful when you want to call a UDF outside of Python. For example receiving a [dataframe into Google Sheets](/tutorials/others/out/googlesheets/) or plotting points and images in a [Felt map](/tutorials/others/out/felt/)
...
Sometimes you want to run a UDF over a list of inputs (for example running a [UDF that unzips a file over a list of files](/use-cases/dark-vessel-detection/#33---run-this-udf-over-a-year-of-ais-data)). If each run itself is quite small, than you can run all a batch of UDFs over a list of inputs.
...
Fused is built to help you scale your processing to huge datasets and the core of this ability is [`fused.submit()`](/python-sdk/top-level-functions/#fusedsubmit). You can run 1 UDF over a large amount of arguments:
...
By default we've setup `fused.submit()` to be a blocking, meaning it will wait for all the jobs to finish before returning the results.
...
`job_pool` then becomes an object you can query to get status of your jobs:

- `job_pool.get_status_df()` -> Returns a dataframe of the status of each job_pool:
...
- `job_pool.collect()` -> Returns a dataframe of the results of all the jobs once they're all finished (equivalent to default `fused.submit()` behaviour):
...
- ⛴️ In the Dark Vessel Detection example to [scale retrieving](/use-cases/dark-vessel-detection/#33---run-this-udf-over-a-month-of-ais-data) daily AIS `.zip` files from NOAA over 30 days.
- 🛰️ [Retrieving all of Maxar's Open Data STAC Catalogs](/use-cases/exploring_maxar_data/#preparing-fusedsubmit-to-run-in-parallel) across every events they have imagery for.
...
To integrate with a tiling service, structure the URL with the `tiles` path parameter, followed by templated `/{z}/{x}/{y}` path parameters. See [Lonboard](/tutorials/others/out/lonboard/) for an example.
...
- The supported types for vector tables are `parquet`, [`geojson`](/tutorials/others/out/deckgl/#vector-tile-layers), [`json`](/tutorials/others/out/deckgl/#h3hexagonlayer), `feather`, [`csv`](/tutorials/others/out/googlesheets/), [`mvt`](/tutorials/others/out/mapbox/#2-create-a-mapbox-html-map), `html`, `excel`, and `xml`.
- For raster array: [`png`](/tutorials/others/out/deckgl/#raster-tile-layers), `gif`, `jpg`, `jpeg`, `webp`, `tif`, and `tiff`.

```
https://www.fused.io/server/v1/realtime-shared/****/run/file?dtype_out_raster=png
```

Read how to structure HTTP endpoints to call the UDF as a [Map Tile & File](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#call-http-endpoints).
