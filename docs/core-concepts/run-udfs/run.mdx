...
2.  [**HTTP call**](/core-concepts/run-udfs/run-small-udfs/#http-requests) from *anywhere*
    - Useful when you want to call a UDF outside of Python. For example receiving a [dataframe into Google Sheets](/examples/out/googlesheets/) or plotting points and images in a [Felt map](/examples/out/felt/)
...
<Tabs className="unique-tabs">
  <TabItem value="python" label="Python" default>
    ...
    ```python showLineNumbers
    token = my_super_duper_private_udf.get_access_token()
    print(token)
    ```
...
To integrate with a tiling service, structure the URL with the `tiles` path parameter, followed by templated `/{z}/{x}/{y}` path parameters. See [Lonboard](/examples/out/lonboard/) for an example.
...
- The supported types for vector tables are `parquet`, [`geojson`](/examples/out/deckgl/#vector-tile-layers), [`json`](/examples/out/deckgl/#h3hexagonlayer), `feather`, [`csv`](/examples/out/googlesheets/), [`mvt`](/examples/out/mapbox/#2-create-a-mapbox-html-map), `html`, `excel`, and `xml`.
- For raster array: [`png`](/examples/out/deckgl/#raster-tile-layers), `gif`, `jpg`, `jpeg`, `webp`, `tif`, and `tiff`.
