...
Simply use `basic-tier` as your environment name.

**With environment**

Figure out your environment's client_id by running:

```python showLineNumbers
import fused
# Figure out your client_id: (basic-tier below)
fused.api.FusedAPI()._automatic_realtime_client_id() # <<<
```

This returns your environment's client_id:
```bash
'your-environment-name'
```

Here is an example, substitute in the code in the code field, naturally (note if you are using a subscribed account you may need to adjust basic-tier to fit your environment):
...
```bash
https://www.fused.io/server/v1/realtime/<your-environment>/api/v1/run/udf
```
