...
You can read more about `fused.run()` in the dedicated [API docs page](/python-sdk/top-level-functions/#run).
...
1. [Mandatory] The first argument needs ot be the UDF to run (name, object, token, etc [as seen above](/core-concepts/run-udfs/run-small-udfs/#fusedrun))
2. [Optional] Any arguments of the UDF itself (if it has any). In the example above that's `inputs` because `my_udf` takes `inputs` as arguments.
...
<details>
  <summary>Simple `PoolRunner` Benchmark</summary>
    ...
    `PoolRunner` runs all the UDF calls in parallel, making it a helpful tool to run multiple UDFs all at once.
