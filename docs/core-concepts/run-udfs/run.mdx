...
### Advanced [`fused.submit()`](/python-sdk/top-level-functions/#submit) options
...
[`fused.submit()`](/python-sdk/top-level-functions/#submit) also have parameters giving you more control over the execution. See the Python SDK [docs page](/python-sdk/top-level-functions/#submit) for more details:
...
- ⛴️ In the Dark Vessel Detection example to [scale retrieving](/user-guide/examples/dark-vessel-detection/#33---run-this-udf-over-a-month-of-ais-data) daily AIS `.zip` files from NOAA over 30 days.
- 🛰️ [Retrieving all of Maxar's Open Data STAC Catalogs](/user-guide/examples/exploring_maxar_data/#preparing-fusedsubmit-to-run-in-parallel) across every events they have imagery for.
...
<ReactPlayer playsinline className="video__player" loop playing={false} muted controls height="100%" width="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/shared_token_edit2.mp4" />
