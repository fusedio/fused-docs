...
Sometimes you want to run a UDF over a list of inputs (for example running a [UDF that unzips a file over a list of files](/use-cases/dark-vessel-detection/#33---run-this-udf-over-a-month-of-ais-data)). If each run itself is quite small, then you can run a batch of UDFs over a list of inputs.
