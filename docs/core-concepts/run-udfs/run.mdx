...
Sometimes you want to run a UDF over a list of inputs (for example running a [UDF that unzips a file over a list of files](/use-cases/dark-vessel-detection/#33---run-this-udf-over-a-year-of-ais-data)). If each run itself is quite small, then you can run a batch of UDFs over a list of inputs.
...
Aim for a single UDF job to takes 30-45s. This gives you a "safety" margin as UDFs will [timeout after 120s](/core-concepts/run-udfs/run-small-udfs/#defining-small-job). So jobs can still take a bit longer and not time out. 
:::
