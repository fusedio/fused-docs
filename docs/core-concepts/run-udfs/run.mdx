...
By default we've setup `fused.submit()` to be a blocking call, meaning it will wait for all the jobs to finish before returning the results. 

You might want to just kick off these jobs and continue executing the rest of your Python code:
...
Sometimes you want to run a _really_ large number of jobs, but you want to make sure your code is running correctly. That's what Debug Mode allows you do to:

```python showLineNumbers
job = fused.submit(udf, inputs, debug_mode=True)
```

This will run the first item in `inputs` directly using `fused.run()` and then return the results:
...
- `engine`: The engine to use for the job pool.
