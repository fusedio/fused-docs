...
{/* TODO: Update link to `fused.submit()` page in Python SDK once we've released it */}
### `fused.submit()`
...
A few seconds after, once all our jobs are run we can get our results as a `gpd.GeoDataFrame`, using the same example as above:
...
{/* TODO: Need to add link to this section once we have it */}
:::tip
