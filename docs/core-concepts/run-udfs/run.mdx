...
In the UDF Builder, you can create an HTTPS endpoint for a UDF in the ["Snippets"](/workbench/udf-builder/navigation/#share-snippets) section. This generates a unique URL to call the UDF via HTTPS requests. The URL is scoped to that UDF only and it can be revoked to disable access. The same can be done with the [Fused Python SDK](/python-sdk/).

### Shared token

To run a UDF via HTTPS request, generate a [shared token](/workbench/udf-builder/navigation/#share) and use the provided URL. Manage your account's shared tokens in [fused.io/profile#tokens](https://www.fused.io/workbench/tokens).
...
When UDF endpoints are called via HTTPS requests, argument values are specified with [query parameters](https://www.branch.io/glossary/query-parameters/), which require input parameters to be serializable. As such, the UDF should specify the types to cast them to. Read more about [supported types for UDF parameters](/core-concepts/write/#supported-types).
