...
All UDFs have secure HTTP endpoints callable via a [web request](/core-concepts/run/#http-requests) or [`fused.run`](/core-concepts/run/#fusedrun). An endpoint is accessed with a [shared token](/core-concepts/run/#shared-token) or [private token](/core-concepts/run/#private-token). You can [cache](/core-concepts/run/#caching-responses) and [specify the type](/core-concepts/run/#response-data-types) of the response.

## `fused.run`


The first argument of fused.run specifies the udf and the remaining inputs are passed to it. The UDF can be specified with a string identifier, shared token, or a UDF object. The UDF object can be defined in the code or loaded with `fused.load`.
...
**With a [shared token](/core-concepts/run/#shared-token).** Use this for you or others others to run your shared UDFs, or to run UDFs that others shared with you. This does not require authentication and runs on the account that owns the token.
...
Fused can also run UDFs asynchronously, which you can read more about [here](/core-concepts/advanced-execution/async/).



## HTTP requests

In Workbench, you can create an HTTP endpoint for a UDF using the "Snippets" button located in the [UDF settings tab](/workbench/udf-builder/udf-editor/#share--snippets). This button generates a unique URL to call the UDF via HTTP requests. The URL is scoped to that UDF only and, if necessary, it can be revoked to disable access. The same can be done with the [Fused Python SDK](/python-sdk/).
...
To run a UDF via HTTP request, generate a [signed UDF endpoint](/workbench/udf-builder/udf-editor/#share--snippets) then modify the provided URL.
...
To integrate with a tiling service, structure the URL with the `tiles` path paramater, followed by templated `/{z}/{x}/{y}` path parameters. See [Lonboard](/user-guide/out/lonboard/) for an example.
...
- The supported types for table are [`parquet`](/user-guide/out/duckdb/#3-query-using-read_parquet), [`geojson`](/user-guide/out/deckgl/#vector-tile-layers), [`json`](/user-guide/out/deckgl/#h3hexagonlayer), `feather`, [`csv`](/user-guide/out/googlesheets/), `mvt`, `html`, `excel`, and `xml`.
...
## Caching responses
If enabled in the [UDF settings](/workbench/udf-builder/udf-editor/#cache), UDF endpoints will cache outputs for each unique combination of code and parameters. The first call runs and caches the UDF and subsequent calls return the cache.
