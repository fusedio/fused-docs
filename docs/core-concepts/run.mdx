...
The simplest way to call a UDF is using a [public UDF name](/core-concepts/run/#public-udf-name) and simply calling it by `UDF_` + name. Let's take this simple UDF that returns [the location of the Eiffel Tower](https://github.com/fusedio/udfs/tree/main/public/Single_point_Eiffel_Tower) in a `GeoDataFrame`:
...
- [By public UDF name](/core-concepts/run/#public-udf-name)
- [Using a token](/core-concepts/run/#token)
- [Using a `udf` object](/core-concepts/run/#udf-object)
- [From Github URL](/core-concepts/run/#github-url)
- [From git commit hash (most recommended for teams)](/core-concepts/run/#git-commit-hash-recommended-for-most-use-cases)

### _Name_ (from your account)

_When to use: When calling a UDF you made, from your own account._

You can call any UDFs you have made simply by calling them by the name you have given your UDF when saving it
...
This UDF can then be run in, say a notebook locally granted that you have authenticated:
...
Let's use the [same public Eiffel Tower UDF](https://github.com/fusedio/udfs/tree/main/public/Single_point_Eiffel_Tower) as an example:

```python showLineNumbers
from fused.api import FusedAPI
api = FusedAPI()
token = api.create_udf_access_token("Single_point_Eiffel_Tower").token
print(token)
```

This would return something like: `'fsh_**********K3b'` (You can recognise this to be a shared token because it starts with `fsh_`)
...
:::warning
    ...
    For that reason we recommend using [git commit hash](/core-concepts/run/#git-commit-hash-recommended-for-teams) instead
:::

### Git commit hash (recommended for most use cases)

_When to use: In most cases. This is the safest way to use a UDF. Since you're pointing to a specific git commit hash you won't end up with changes breaking your URL._
