...
The UDF Builder runs the UDF as a [Map Tile](/core-concepts/filetile/) if the [first parameter](/core-concepts/filetile/#the-bounds-object) is typed as `fused.types.Bounds`, or `fused.types.Tile`.
