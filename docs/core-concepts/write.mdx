...
This example shows the [`bounds` parameter](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#bounds-object-types) typed as `fused.types.Bounds`
...
To write UDFs that run successfully as both [`File` and `Tile`](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/), set `bounds` as the first parameter, with `None` as its default value. This enables the UDF to be invoked successfully both as `File` (when `bounds` isn't passed) and as `Tile`. For example:
...
The UDF Builder runs the UDF as a [Map Tile](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/) if the [first parameter](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#the-bounds-object) is typed as `fused.types.Bounds`.
...
When running a UDF with `fused.run`, it's possible to specify the [map tile](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#tile) Fused will use to structure the `bounds` object by using the following reserved parameters.
...
- Arrays: `numpy.ndarray`, `xarray.Dataset`, `xarray.DataArray`, and `io.BytesIO`. Fused Workbench only supports the rendering of `uint8` arrays. Rasters without spatial metadata should indicate their [tile bounds](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#map-tiles).
