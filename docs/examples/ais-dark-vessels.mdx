---
id: ais-dark-vessels
title: AIS Dark Vessel Detection
sidebar_label: AIS Dark Vessels
sidebar_position: 9
---
...
### Requirements
- [Access to Fused](/guide/advanced-setup/local-installation)
...
<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/examples/dark-vessel-detection/analysis_walkthrough_dark_vessel_detection.mp4" />
...
Since our AIS dataset is not in a geo-partitioned, cloud native format, our first step is to [ingest](/guide/data-input-outputs/read-write/geospatial/ingestion) it into a tiled format and put it on a cloud bucket. Thankfully we can [do all of this in Fused](/guide/data-input-outputs/read-write/geospatial/ingestion).
...
[`fused.file_path()`](/python-sdk/api-reference/fused-file-path) returns the mount path of any file we'd like to create. When run locally, it returns the local `/tmp/fused/` directory on your machine.
If this function is called inside a UDF, and that UDF is run on Fused servers, the returned path will be of the shared mounted storage `/mount/`. Specifying [`engine='local'`](/python-sdk/api-reference/fused-run) runs the UDF on your local machine.
...
<Tabs className="unique-tabs">
  ...
  <TabItem value="udf" label="In a UDF" default>
        ...
        >>> /mount/AIS/2024_09
        ```
        You can see this directory in the [File Explorer](/workbench/file-explorer/) in [Workbench](/workbench/overview).
...
The rest of the logic is [write a UDF](/guide/working-with-udfs/writing-udfs/) to open each file, read it as a CSV and write it to parquet.
...
Since each UDF takes a few seconds to run per date, we're going to use [`fused.submit()`](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution) to call a large numbers of UDFs all at once that will each run over a single date.
...
`debug_mode=True` runs the 1st value inside `date_ranges` with [`fused.run()`](/python-sdk/api-reference/fused-run). This allows you to make sure your UDF + arg_list combo is working properly. 
...
:::tip
[`fused.submit()`](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution) has more parameters you can control allowing you to change the number of `max_workers`, `engine` or the retry policies. Also take a look at the [technical docs](/python-sdk/api-reference/fused-submit) for more details.
:::
...
These individual parquet files are now store on our mount disk. We could save them directly onto cloud storage but before that we can geo-partition them to make them even faster to read. This will make us reduce the time it takes to access our data from [minutes to seconds](/guide/data-input-outputs/read-write/geospatial/ingestion).
Fused provides a simple way to do this with the [ingestion process](/guide/data-input-outputs/read-write/geospatial/ingestion).
...
We'll send this job to a [large instance using `job.run_batch()`](/guide/working-with-udfs/udf-best-practices/scaling-out#dedicated-instances) as we latency doesn't matter much (we can wait a few extra seconds) and we'd rather have a larger machine & a larger storage:
...
<LazyReactPlayer wrapperStyle={{ width: '80%', margin: '0 auto' }} url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/examples/dark-vessel-detection/geopartioned_AIS_file_explorer.mp4" />
...
:::tip
    Most of the following section was written in Workbench's [Map Builder](/workbench/udf-builder/) rather than in Jupyter Notebooks.

    We'll have the code in code blocks, you can run these anywhere but as we're looking at images, it's helpful to have Map Builders' [live map](/workbench/udf-builder/map/) updated as you write your code.

    {/* TODO: Should also add a link to the best practices for writing UDFs in Workbench once we have that section */}
:::

:::warning Map Builder is a legacy feature
Map Builder is deprecated and hidden by default. To follow along you can re-enable it under [Workbench Preferences](/workbench/preferences/) → **Deprecated features** → **Enable map view**. For new work, use the [Fused Map widget](/widget-api/fused-map) instead.
:::
...
This UDF simply returns our [Map viewport](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#legacy-fusedtypesviewportgdf) as a `gpd.GeoDataFrame`, this is a good starting point for our UDF returning Sentinel 1 images
...
- Add a [cache decorator](/guide/working-with-udfs/udf-best-practices/caching/) to code functions that retrieve data to speed up the UDF & reduce costs.
...
If we save and call this UDF with a [token](/guide/data-input-outputs/export-api/tokens-endpoints#shared-token) we can call it from anywhere, from a Jupyter Notebook or from another UDF. Let's create a new UDF in Workbench:
...
We're going to leverage code from the community for this part, namely reading the AIS data from a geo-partitioned GeoParquet. Fused allows us to easily re-use any code we want and freeze it to a specific commit so it doesn't break our pipelines ([read more about this here](/guide/working-with-udfs/udf-best-practices/realtime#pin-to-commit-hash-for-production))
...
We can expand the UDF we had started in [section 6.](/examples/ais-dark-vessels#6-retrieving-ais-data-for-our-time-of-interest) to call our AIS UDF by passing a bounding box + `s1_acquisition_month_day_hour_min`.
...
<LazyReactPlayer wrapperStyle={{ width: '80%', margin: '0 auto' }} url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/examples/dark-vessel-detection/false_positive_wind_mills.mp4" />
...
- Running [UDFs at scale with `run_batch()`](/guide/working-with-udfs/udf-best-practices/scaling-out#dedicated-instances). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
- More about Fused core concepts like chosing between [running UDFs based on Tile or File](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile)
