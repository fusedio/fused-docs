...
[Try it out on the Canvas →](https://unstable.fused.io/canvas/fc_fused/Standardized_text_to_image)
...
![Canvas showing the "Create an image" interface with a prompt input on the left and the generated branded diagram output on the right](/img/examples/branded-t2i/canvas-text-to-image.png)
...
- **[Response caching](/python-sdk/api-reference/cache)** — identical prompt + style combinations return instantly without re-calling the API. Running the same prompt twice costs nothing extra.
...
- [`@fused.cache`](/python-sdk/api-reference/cache) — avoid duplicate API charges
