...
Read more about [`fused.file_path()`](/python-sdk/top-level-functions/#fusedfile_path) and [mount](/guide/advanced-setup/file-system/#mntcache-disk)
...
Fused allows us to run a [UDF in parallel](/guide/working-with-udfs/execution/realtime#running-jobs-in-parallel-fusedsubmit). So we'll process 1 month of data across hundreds of jobs:
