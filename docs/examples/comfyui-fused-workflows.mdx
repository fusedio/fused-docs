...
1. **Export the API JSON** from ComfyUI (**Graph → Export (API)**). You can start from an existing shared workflow — for example, the [Images to Video graph](https://cloud.comfy.org/?share=77000df905db) or the [Text to Image graph](https://cloud.comfy.org/?share=b457ccdd9d91) on Comfy Cloud. Open it in ComfyUI, then export via **Graph → Export (API)**.
