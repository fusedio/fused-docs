...
This example walks through two pipelines — **Images to Video** and **Text to Image** — both living on the same canvas. The same pattern applies to any ComfyUI workflow you want to bring into Fused.
...
The underlying graph is [shared on Comfy Cloud](https://cloud.comfy.org/?share=77000df905db) — inspect and fork it directly. To export it as JSON: **Graph → Export (API)**. That JSON is what Fused loads from S3 and patches with your parameters at runtime.

![ComfyUI Graph menu with Export (API) highlighted](/img/examples/comfyui-fused/comfyui_export_api.png)

### The Fused UDF

The UDF is split into two parts: a [`@fused.cache`](/python-sdk/api-reference/fused-cache) helper that runs the expensive GPU job, and a thin `@fused.udf` wrapper that refreshes the signed URL. The wrapper uses `engine='small'`, which runs it as a [batch job](/guide/working-with-udfs/udf-best-practices/scaling-out) — necessary because video generation can take several minutes, well beyond the 120-second limit of realtime UDFs.
...
`@fused.cache` on `run_comfy_job` stores the S3 path of the generated video, keyed by every argument. The next call with the same inputs returns that path immediately — no Comfy Cloud call, no GPU time.

The outer `udf` is intentionally not cached: it re-signs the S3 URL on every call. Signed URLs expire; the video doesn't.
...
1. **Export the API JSON** from ComfyUI (**Graph → Export (API)**).
...
4. **Write a [`@fused.cache`](/python-sdk/api-reference/fused-cache) helper** that loads the workflow, patches those nodes, submits, polls, and uploads the result.
...
- **[`@fused.cache`](/python-sdk/api-reference/fused-cache)** — identical inputs return instantly; no GPU charges for repeat runs.
- **[`fused.secrets`](/python-sdk/api-reference/fused-secrets)** — your Comfy Cloud API key lives in Fused secrets, never in code.
...
2. Click **Add secret**, name it `COMFY_API_KEY`, paste your key
