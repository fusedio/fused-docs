...
Since each UDF takes a few seconds to run per date, we're going to use [`fused.submit()`](/guide/working-with-udfs/execution/parallel) to call a large numbers of UDFs all at once that will each run over a single date.
...
[`fused.submit()`](/guide/working-with-udfs/execution/parallel) requires inputs as a list or `DataFrame`, in which case columns should have the argument names that our UDF expects, in this case `datestr`. We also recommend you always do a first run with `debug_mode=True` to [test your submit job](/guide/working-with-udfs/execution/parallel#debug-mode):
...
- Running [UDFs at scale with `run_batch()`](/guide/working-with-udfs/execution/batch-jobs#running-jobrun_batch-from-notebook). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
