...
Since each UDF takes a few seconds to run per date, we're going to use [`fused.submit()`](/guide/working-with-udfs/execution/realtime#running-jobs-in-parallel-fusedsubmit) to call a large numbers of UDFs all at once that will each run over a single date.
...
[`fused.submit()`](/guide/working-with-udfs/execution/realtime#running-jobs-in-parallel-fusedsubmit) requires inputs as a list or `DataFrame`, in which case columns should have the argument names that our UDF expects, in this case `datestr`. We also recommend you always do a first run with `debug_mode=True` to [test your submit job](/guide/working-with-udfs/execution/realtime#debug-mode):
...
:::tip
[`fused.submit()`](/guide/working-with-udfs/execution/realtime#running-jobs-in-parallel-fusedsubmit) has [more parameters](/guide/working-with-udfs/execution/realtime#execution-parameters) you can control allowing you to change the number of `max_workers`, `engine` or the retry policies. Also take a look at the [technical docs](/python-sdk/top-level-functions/#fusedsubmit) for more details.
:::
...
We'll send this job to a [large instance using `job.run_batch()`](/guide/working-with-udfs/execution/batch-jobs) as we latency doesn't matter much (we can wait a few extra seconds) and we'd rather have a larger machine & a larger storage:
...
This UDF simply returns our [Map viewport](/guide/working-with-udfs/geospatial/single-vs-tile#legacy-fusedtypesviewportgdf) as a `gpd.GeoDataFrame`, this is a good starting point for our UDF returning Sentinel 1 images
...
- Add a [cache decorator](/guide/working-with-udfs/caching/) to code functions that retrieve data to speed up the UDF & reduce costs.
...
If we save and call this UDF with a [token](/guide/working-with-udfs/execution/realtime#token) we can call it from anywhere, from a Jupyter Notebook or from another UDF. Let's create a new UDF in Workbench:
...
- Running [UDFs at scale with `run_batch()`](/guide/working-with-udfs/execution/batch-jobs#running-a-large-job-jobrun_batch). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
- More about Fused core concepts like chosing between [running UDFs based on Tile or File](/guide/working-with-udfs/geospatial/single-vs-tile)
