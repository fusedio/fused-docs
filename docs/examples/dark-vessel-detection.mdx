...
Since each UDF takes a few seconds to run per date, we're going to use [`fused.submit()`](/guide/getting-started/udf-best-practices/parallel) to call a large numbers of UDFs all at once that will each run over a single date.
...
[`fused.submit()`](/guide/getting-started/udf-best-practices/parallel) requires inputs as a list or `DataFrame`, in which case columns should have the argument names that our UDF expects, in this case `datestr`. We also recommend you always do a first run with `debug_mode=True` to [test your submit job](/guide/getting-started/udf-best-practices/parallel#tips):
...
:::tip
[`fused.submit()`](/guide/getting-started/udf-best-practices/parallel) has [more parameters](/guide/getting-started/udf-best-practices/parallel#execution-parameters) you can control allowing you to change the number of `max_workers`, `engine` or the retry policies. Also take a look at the [technical docs](/python-sdk/top-level-functions/#fusedsubmit) for more details.
:::
...
We'll send this job to a [large instance using `job.run_batch()`](/guide/getting-started/udf-best-practices/batch-jobs) as we latency doesn't matter much (we can wait a few extra seconds) and we'd rather have a larger machine & a larger storage:
...
We're going to leverage code from the community for this part, namely reading the AIS data from a geo-partitioned GeoParquet. Fused allows us to easily re-use any code we want and freeze it to a specific commit so it doesn't break our pipelines ([read more about this here](/guide/getting-started/udf-best-practices/realtime#ways-to-reference-a-udf))
...
- Running [UDFs at scale with `run_batch()`](/guide/getting-started/udf-best-practices/batch-jobs#advanced-run_batch-with-arg_list). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
