...
- Running [UDFs at scale with `run_batch()`](/guide/working-with-udfs/execution/batch-jobs#advanced-run_batch-with-arg_list). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
