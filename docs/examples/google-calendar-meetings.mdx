...
**Geocoding** — The `gcal_events_geocode` UDF uses [`fused.load()`](/python-sdk/api-reference/load) to call `gcal_today_events`, then geocodes each event's location using the [Nominatim API](https://nominatim.org/). The [`@fused.cache`](/guide/working-with-udfs/writing-udfs/#fusedcache-decorator) decorator on the geocoding function means locations are cached — if you move a meeting's time but not its location, the next run skips the geocoding call and only re-fetches the calendar data.
