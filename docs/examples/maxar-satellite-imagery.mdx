...
- Show how to use [`fused.submit()`](/python-sdk/api-reference/fused-submit) to fetch all the available STAC Catalogs all at once in parallel in just a few seconds
...
- Use [`fused.submit()`](/python-sdk/api-reference/fused-submit) to fetch all the STAC Collections for each event name
...
- We're using [`@fused.cache`](/python-sdk/api-reference/fused-cache) to cache the Catalog request so our UDF doesn't need to do this request each time we execute it. It prevents being rate limited and doing the same request over and over against the same endpoint
...
We're going to use [`fused.submit()`](/python-sdk/api-reference/fused-submit) to run our first UDF in parallel. To do this we need a few things:
...
Unpacking this UDF:
- This UDF takes :
    - a [`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) object. This allows us to pass the [current Workbench Map Viewport](/workbench/udf-builder/canvas) to our UDF
...
- Use [`fused.submit()`](/python-sdk/api-reference/fused-submit) to run a [UDF in parallel](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution)
- Use [`@fused.cache`](/python-sdk/api-reference/fused-cache) to cache requests to reduce costs and improve performance
- Use Workbench to [display images](/workbench/udf-builder/canvas) in [different modes](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile)
