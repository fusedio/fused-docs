...
You can explore it yourself by directly [reading the code in Github](https://github.com/fusedio/udfs/blob/main/public/common). If you see any functions you'd like to use, we strongly recommend you use `fused.load()` and pass the latest commit hash at the time you want to use it (see [ways to reference a UDF](/guide/working-with-udfs/udf-best-practices/realtime#ways-to-reference-a-udf)):
...
:::tip
You can easily rename your UDFs in Workbench. Rename this UDF to `Maxar_Open_Data_STAC_single_catalog` so we can call it later [directly by name](/guide/working-with-udfs/udf-best-practices/realtime#ways-to-reference-a-udf).
