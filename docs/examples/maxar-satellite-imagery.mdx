---
id: maxar-satellite-imagery
title: Maxar Satellite Imagery
sidebar_label: Maxar Satellite Imagery
sidebar_position: 8
---
...
<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench/walkthrough-videos/docs_rewrite/exploring_all_maxar_open_data2.mp4" />
...
- Show how to use [`fused.submit()`](/python-sdk/api-reference/submit) to fetch all the available STAC Catalogs all at once in parallel in just a few seconds
...
You can explore it yourself by directly [reading the code in Github](https://github.com/fusedio/udfs/blob/main/public/common). If you see any functions you'd like to use, we strongly recommend you use `fused.load()` and pass the latest commit hash at the time you want to use it (see [pinning to commit hash](/guide/working-with-udfs/udf-best-practices/realtime#pin-to-commit-hash-for-production)):
...
<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench/walkthrough-videos/docs_rewrite/maxar_catalog.mp4" />
:::
...
- Use [`fused.submit()`](/python-sdk/api-reference/submit) to fetch all the STAC Collections for each event name
...
- We're using [`@fused.cache`](/python-sdk/api-reference/cache) to cache the Catalog request so our UDF doesn't need to do this request each time we execute it. It prevents being rate limited and doing the same request over and over against the same endpoint
- We're returning a list (`collections`) so if you run this in Workbench you'll notice nothing shows up on the map! That's also [why we print](https://docs.fused.io/guide/working-with-udfs/writing-udfs#use-print) the first 5 rows. 

:::tip
Read through [the Best Practices](/guide/working-with-udfs/writing-udfs) for more handy tips on how to write efficient and easy to debug UDFs
:::
...
We're going to use [`fused.submit()`](/python-sdk/api-reference/submit) to run our first UDF in parallel. To do this we need a few things:
...
:::tip
`fused.submit()` allows you to run a single over a list / dataframe of inputs in parallel. Under the hood Fused spins up many realtime instances (see [technical docs for details](/python-sdk/api-reference/submit)) that will each run the given UDF (in this case `"Maxar_Open_Data_STAC_single_catalog"`) all at the same time.

This is a powerful way to scale a process with just a single function call.

Read [the dedicated Docs section](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution) on `fused.submit()` for more
:::
...
First we can use the [Runtime Tab](/workbench/udf-builder/runtime/) to find the URL of an image we'd like to display:

<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench/walkthrough-videos/docs_rewrite/maxar_choosing_image2.mp4" />
...
Unpacking this UDF:
- This UDF takes :
    - a [`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) object. This allows us to pass the [current Workbench Map Viewport](/workbench/udf-builder/map/) to our UDF
...
:::note
These images can be loaded using `bounds` and [Tile mode](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#tile-udf) because Maxar has provided these images as [Cloud Optimized Geotiffs](/guide/data-input-outputs/read-write/geospatial/ingestion). This allows us to leverage their tiles & overviews and only load the data we need as we pan around the map
...
This allows us to introduce 2 concepts in [Workbench](/workbench/overview):
- 1. [Zoom to layer](/workbench/udf-builder/navigation/#zoom-to-layer)
- 2. [Tile / File modes](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile)
...
<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench/walkthrough-videos/docs_rewrite/zoom_to_layer_default_view2.mp4" />
...
This change returns the image instead of the extent, but it returns the image all at once, because Workbench is set to ["Single" mode](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#single-udf) by default
...
We can change this by setting Workbench to ["Tile" mode](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#tile-udf):

<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench/walkthrough-videos/docs_rewrite/workbench_image_switching_to_tile_video2.mp4" />
...
- Use [`fused.submit()`](/python-sdk/api-reference/submit) to run a [UDF in parallel](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution)
- Use [`@fused.cache`](/python-sdk/api-reference/cache) to cache requests to reduce costs and improve performance
- Use Workbench to [display images](/workbench/udf-builder/map/) in [different modes](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile)

If you want to go a bit further you could:
- Explore the Best Practices to [make the most of UDFs](/guide/working-with-udfs/writing-udfs) or learn handy tips to use [Workbench](/guide/getting-started/workbench-intro) as its fullest
- Go more in depth with ["File" & "Tile" modes](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile) in Workbench
- Dive into the different ways Fused allows you to use [caching](/guide/working-with-udfs/udf-best-practices/caching/) to improve performance
