...
The `all_suppliers` UDF reads supplier records (name, address, phone, account balance) from Snowflake's TPCH_SF1 sample dataset. Store your Snowflake credentials using `fused.secrets[]` so the UDF can authenticate. See the [Snowflake guide](/guide/data-input-outputs/import-connection/databases/#snowflake) for setup details.
...
def udf():
    ...
    path = 's3://fused-asset/demos/supplier_customer_joining/supplier_locations.csv'  # replace with your S3 path
    df = pd.read_csv(path)
...
<div style={{textAlign: "center", marginBottom: "20px"}}>
  <img
    src="/img/messy-data-agents-claude-code.gif"
