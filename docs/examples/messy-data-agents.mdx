...
A downstream UDF (`join_store_infos`) uses [`fused.load()`](/python-sdk/api-reference/load) — which loads a UDF by name so you can call it from another UDF — to pull data from the three sources:
