...
This post shows how to turn Fused UDF pipelines into agentic workflows: ask questions over massive datasets while compute stays behind the scenes and agents see only the scoped outputs needed to query, compare, and visualize results.
...
*AI chatbot helping answer queries related to dataset. [Try it out](https://unstable.fused.io/canvas/fc_1OKBYKbEo1nab5A4aE2ezb?bounds=4640%2C-804%2C10148%2C2903)*
...
[![IoU visualization](/img/tutorials/agents/iou_visualization.gif)](/img/tutorials/agents/iou_visualization.gif)

*A building-level IoU visualization highlighting where Overture building footprints closely match (green) and where gaps or mismatches still exist (yellow-red). IoU represents overlap percentage between Philadelphia's official building and Overture Building footprint.*
...
These tools, which are also exposed as MCP tools, can be called from any client.
