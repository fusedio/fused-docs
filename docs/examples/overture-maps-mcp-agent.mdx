...
This example shows how to wrap [Overture Maps Foundation](https://overturemaps.org/) open data as MCP tools on a Fused Canvas, so an AI agent can query buildings, places, and more by location or GERS ID without pulling raw geospatial data into the chat context. [Explore the Canvas](https://www.fused.io/canvas/fc_fused/Overture_UDFs).
