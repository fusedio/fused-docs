...
- **Subsequent runs with the same inputs** (near-instant): results are served from [`fused.cache`](/guide/working-with-udfs/udf-best-practices/caching/) so the dashboard loads immediately
