...
[![POI Site Selection Dashboard](/img/tutorials/poi-dashboard/dashboard_hero.gif)](https://unstable.fused.io/workbench/max/Dashboard_Overture_Census_Isochrone)
...
The dashboard chains multiple UDFs together using [`fused.load()`](/guide/working-with-udfs/run-udfs-in-python#load-and-run-saved-udfs) — geocoding an address, computing an [isochrone](https://valhalla.github.io/valhalla/api/isochrone/api-reference/) (travel-time boundary), querying [Overture Maps](https://docs.overturemaps.org/) POIs, pulling [Census ACS](https://www.census.gov/programs-surveys/acs) income data, and aggregating everything into [H3 hexagons](/guide/h3-analytics/h3-overview) for visualization.
...
The `isochrone_chosen_site_overture` UDF chains the geocoder with the Valhalla isochrone API. It loads the geocode UDF with [`fused.load()`](/guide/working-with-udfs/run-udfs-in-python#load-and-run-saved-udfs), extracts the lat/lng, and passes them to `latlng_isochrone_simplified`:
...
{/* TODO: GIF showing isochrone appearing on map after typing an address */}
...
Interact with the POI map below — hexagons are colored by POI density, with the isochrone boundary and geocoded point overlaid:
...
- Swap the Census variable to population, education, or any other ACS metric
- Add competitor analysis by loading a second POI category — see the [Site Selection Analysis](/examples/site-selection-analysis) example for inspiration
...
- Scale the analysis to multiple addresses using [`fused.submit()`](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution)
