...
You can explore it yourself by directly [reading the code in Github](https://github.com/fusedio/udfs/blob/main/public/common). If you see any functions you'd like to use, we strongly recommend you use `fused.load()` and pass the latest commit hash at the time you want to use it (see [ways to reference a UDF](/guide/getting-started/udf-best-practices/realtime#ways-to-reference-a-udf)):
...
:::tip
You can easily rename your UDFs in Workbench. Rename this UDF to `Maxar_Open_Data_STAC_single_catalog` so we can call it later [directly by name](/guide/getting-started/udf-best-practices/realtime#ways-to-reference-a-udf).
...
- We use `fused.submit(..., debug_mode = True)` to run only the 1st value from `collections_df`. This allows us to test that our `fused.submit()` job is [written correctly](/guide/getting-started/udf-best-practices/parallel#tips). 
...
Read [the dedicated Docs section](/guide/getting-started/udf-best-practices/parallel) on `fused.submit()` for more
:::
...
Unpacking this UDF:
- This UDF takes :
    - a [`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds) object. This allows us to pass the [current Workbench Map Viewport](/workbench/udf-builder/map/) to our UDF
...
- Use [`fused.submit()`](/python-sdk/top-level-functions/#fusedsubmit) to run a [UDF in parallel](/guide/getting-started/udf-best-practices/parallel)
