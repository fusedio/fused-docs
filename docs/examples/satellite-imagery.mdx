...
You can explore it yourself by directly [reading the code in Github](https://github.com/fusedio/udfs/blob/main/public/common). If you see any functions you'd like to use, we [strongly recommend](/guide/working-with-udfs/execution/realtime#git-commit-hash-recommended-for-most-stable-use-cases) you use `fused.load()` and pass the latest commit hash at the time you want to use it:
...
:::tip
You can easily rename your UDFs in Workbench. Rename this UDF to `Maxar_Open_Data_STAC_single_catalog` so we can call it later [directly by name](/guide/working-with-udfs/execution/realtime#name-from-your-account).
...
- We use `fused.submit(..., debug_mode = True)` to run only the 1st value from `collections_df`. This allows us to test that our `fused.submit()` job is [written correctly](/guide/working-with-udfs/writing-udfs#test-out-your-udfs-before-running-them-in-parallel). 
...
Read [the dedicated Docs section](/guide/working-with-udfs/execution/realtime#running-jobs-in-parallel-fusedsubmit) on `fused.submit()` for more
:::
...
Unpacking this UDF:
- This UDF takes :
    - a [`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#the-bounds-object) object. This allows us to pass the [current Workbench Map Viewport](/workbench/udf-builder/map/) to our UDF
...
:::note
These images can be loaded using `bounds` and [Tile mode](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf) because Maxar has provided these images as [Cloud Optimized Geotiffs](/guide/data-input-outputs/read-write/geospatial/ingestion#for-rasters-images-cloud-optimized-geotiff). This allows us to leverage their tiles & overviews and only load the data we need as we pan around the map
...
- 2. [Tile / File modes](/guide/working-with-udfs/geospatial/single-vs-tile)
...
This change returns the image instead of the extent, but it returns the image all at once, because Workbench is set to ["Single" mode](/guide/working-with-udfs/geospatial/single-vs-tile#single-udf) by default
...
We can change this by setting Workbench to ["Tile" mode](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf):
...
- Use [`fused.submit()`](/python-sdk/top-level-functions/#fusedsubmit) to run a [UDF in parallel](/guide/working-with-udfs/execution/realtime#running-jobs-in-parallel-fusedsubmit)
- Use [`@fused.cache`](/python-sdk/top-level-functions/#fusedcache) to cache requests to reduce costs and improve performance
- Use Workbench to [display images](/workbench/udf-builder/map/) in [different modes](/guide/working-with-udfs/geospatial/single-vs-tile)
...
- Go more in depth with ["File" & "Tile" modes](/guide/working-with-udfs/geospatial/single-vs-tile) in Workbench
- Dive into the different ways Fused allows you to use [caching](/guide/working-with-udfs/caching/) to improve performance
