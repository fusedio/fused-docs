...
Every time we host a [LinkedIn Live webinar](https://www.linkedin.com/feed/update/urn:li:activity:7462432559440977920/), we need a thumbnail — speaker photos, title, brand colors, consistent layout. The first one took an afternoon of manual Figma work. By the third, that felt unreasonable. This example shows how we replaced that workflow with a Fused Canvas that anyone on the team can open, fill in, and download in under a minute.
...
```bash
fused canvas push ./thumbnail_generator --canvas thumbnail_generator
```
...
That's it. The canvas is live at `fused.io/workbench/<team>/thumbnail_generator` and the whole team can open it immediately.
...
Open the [live canvas](https://www.fused.io/workbench/max/thumbnail_generator), swap in your own speakers and title, and hit **Download PNG**.
