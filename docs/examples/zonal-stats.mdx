...
To see the data as we process it, we will write a UDF in the [Fused Workbench](https://www.fused.io/workbench). As you write code in the [Code Editor](/workbench/udf-builder/code-editor) you'll see how visualization results, logs, and errors show up immediately.
...
Try running the UDF in [Workbench](/workbench/overview) and visually inspect the output on the map. See what happens when you change `year`. Try introducing print statements such as `print(arr)` and `print(gdf)` to show logs in the console.
