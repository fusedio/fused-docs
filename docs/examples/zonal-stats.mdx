...
To see the data as we process it, we will write a UDF in the [Fused Workbench](https://www.fused.io/workbench). As you write code in the [Map Builder](/workbench/udf-builder/canvas) you'll see how visualization results, logs, and errors show up immediately.
...
Try running the UDF in the [Map Builder](/workbench/udf-builder/canvas) and visually inspect the output on the map. See what happens when you change `year`. Try introducing print statements such as `print(arr)` and `print(gdf)` to show logs in the console.
...
<details>
  <summary>You can also style the map layer by setting this JSON in the Visualization tab</summary>
