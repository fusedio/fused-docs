...
You'll first upload your own vector table with `fused.ingest`. This spatially partitions the table and writes it in your specified S3 bucket as a [GeoParquet](/guide/data-input-outputs/read-write/geospatial/ingestion#for-vectors-tables-geoparquet) file. You'll then calculate zonal stats over a raster array of alfalfa crops in the [USDA Cropland Data Layer](https://catalog.data.gov/dataset/cropscape-cropland-data-layer) (CDL) dataset.
...
Invoke the UDF in parallel with `run_pool` to increase performance when you want to run it repeatedly with different input parameters. In this case, we'll use it to call the UDF across a set of years. For a deeper dive, read how to [Call UDFs asynchronously](/guide/working-with-udfs/execution/realtime#async).
...
We hope this overview gives you a glimpse of what you can build with Fused. You can continue to learn how to [read data](/guide/data-input-outputs/read-write/geospatial/reading), [process data](/guide/working-with-udfs/caching), and [integrate](/guide/data-input-outputs/export-api/geospatial) with other applications.
