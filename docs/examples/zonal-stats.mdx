...
- the following Python packages installed locally:
  - [`fused`](/guide/advanced-setup/local-installation)
...
You'll first upload your own vector table with `fused.ingest`. This spatially partitions the table and writes it in your specified S3 bucket as a [GeoParquet](/guide/data-input-outputs/read-write/geospatial/ingestion#ingest-geospatial-table-data) file. You'll then calculate zonal stats over a raster array of alfalfa crops in the [USDA Cropland Data Layer](https://catalog.data.gov/dataset/cropscape-cropland-data-layer) (CDL) dataset.

This example shows how to geo partition polygons of [Census Block Groups](https://www.census.gov/geographies/mapping-files/time-series/geo/tiger-line-file.html) for Utah, which is a Parquet table with a `geometry` column. You can follow along with this file or any vector table you'd like. Read about other supported formats in [Ingest your own data](/guide/data-input-outputs/read-write/geospatial/ingestion).

First, set up a local Python environment, install the latest Fused SDK with `pip install fused`, and [authenticate](/guide/advanced-setup/local-installation).
...
To see the data as we process it, we will write a UDF in the [Fused Workbench](https://www.fused.io/workbench). As you write code in the [Map Builder](/workbench/udf-builder/) you'll see how visualization results, logs, and errors show up immediately.
...
To [write a UDF](/guide/working-with-udfs/writing-udfs/) simply wrap a Python function with the decorator `@fused.udf`.

The first parameter of this UDF, `bounds`. It is reserved for Fused to pass a `GeoDataFrame` which the UDF may use to spatially filter datasets, and usually corresponds to a web map [tile](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile). This enables Fused to run the UDF for each tile in the viewport to distribute processing across multiple workers.
...
Try running the UDF in the [Map Builder](/workbench/udf-builder/) and visually inspect the output on the map. See what happens when you change `year`. Try introducing print statements such as `print(arr)` and `print(gdf)` to show logs in the console.
...
You might receive a [timeout error](/guide/working-with-udfs/udf-best-practices/realtime#limits) in the `Results` tab. Try zooming into Utah on the map where the zonal areas are highlighted, to reduce the [size of the tile](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#tile-udf) being passed into the UDF from the map.
...
Now that you have a UDF, let's see three ways you can [Run UDFs](/guide/working-with-udfs/udf-best-practices/realtime) remotely:
- via an HTTPS endpoint,
- in a Python application,
- in parallel.

### a. HTTPS endpoint
Save the UDF, then click the 'Share' button located below the UDF name on the left side. In the dialog, click 'Share' again to generate a [token](/guide/data-input-outputs/export-api/tokens-endpoints#shared-token). Once the token is created, you'll see an [HTTPS endpoint](/guide/data-input-outputs/export-api/tokens-endpoints) that you can use to call the UDF from anywhere. Try it out by changing the endpoint's query parameters — notice here `?format=geojson`, meaning the output will be a GeoJSON file.

:::note
This example uses the legacy per-UDF token (`fsh_`). You can also [share your canvas](/workbench/udf-builder/canvas#share-modal) and use `udf.ai/fc_<token>/<udf_name>.<ext>` URLs.
:::
...
When you called the HTTPS endpoint, Fused ran the UDF then sent back the output table and [debug logs](/guide/working-with-udfs/writing-udfs/#https-requests). In the URL above, `/12/778/1548` specifies the ZXY tile index to structure `bounds` and `format=geojson` specifies the output format. Fused passes the `year` & `crop_type` parameters to the UDF as `int` based on their types in the function definition.
...
Invoke the UDF in parallel with `run_pool` to increase performance when you want to run it repeatedly with different input parameters. In this case, we'll use it to call the UDF across a set of years. For a deeper dive, read how to [run UDFs in parallel](/guide/working-with-udfs/udf-best-practices/scaling-out#parallel-execution).
...
App Builder is deprecated and hidden by default. You can re-enable it under [Workbench Preferences](/workbench/preferences/) → **Deprecated features**. There is currently no direct replacement.
:::
...
We hope this overview gives you a glimpse of what you can build with Fused. You can continue to learn how to [read data](/guide/data-input-outputs/read-write/geospatial/geospatial-reading), [process data](/guide/working-with-udfs/udf-best-practices/caching), and [integrate](/guide/data-input-outputs/export-api/geospatial-export) with other applications.
