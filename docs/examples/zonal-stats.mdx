...
You might receive a [timeout error](/guide/working-with-udfs/execution/realtime#defining-realtime-udf-runs) in the `Results` tab. Try zooming into Utah on the map where the zonal areas are highlighted, to reduce the [size of the tile](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf) being passed into the UDF from the map.
...
### a. HTTPS endpoint
Save the UDF, then click the 'Share' button located below the UDF name on the left side. In the dialog, click 'Share' again to generate a [token](/guide/working-with-udfs/execution/realtime#token). Once the token is created, you'll see an [HTTPS endpoint](/guide/working-with-udfs/execution/realtime) that you can use to call the UDF from anywhere. Try it out by changing the endpoint's query parameters — notice here `?format=geojson`, meaning the output will be a GeoJSON file.
