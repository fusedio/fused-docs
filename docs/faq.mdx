...
This error happens when your UDF returns a `GeoDataFrame` but your local env doesn't have the `geopandas` package installed to manipulate it.

The standalone `fused` package doesn't have `geopandas` or other optional dependencies installed to keep it light weight. 

This can be fixed in 2 ways:

1. When running fused locally

You can [install](/python-sdk/#install) `fused` with all the optional dependencies:
