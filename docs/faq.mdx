...
Use cases like creating chips may call for running a UDF across a set of tiles that fall within a given geometry. This can be done by creating a list of tiles with the [mercantile](https://github.com/mapbox/mercantile) library then calling the UDF [in parallel](/examples/zonal-stats/#c-parallelization).
