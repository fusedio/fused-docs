...
Download remote files to the local system to make them available to UDFs across runs. Files are written to a [disk](/guide/content-management/file-system#mntcache-disk) shared across all UDFs in an organization.

## [`fused.download`](/reference/python-sdk/top-level-functions/#fuseddownload)
