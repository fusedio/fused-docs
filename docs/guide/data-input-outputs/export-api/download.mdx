...
Download remote files to the local system to make them available to UDFs across runs. Files are written to a [disk](/guide/working-with-udfs/udf-best-practices/storage/#mntcache-disk) shared across all UDFs in an organization.

## [`fused.download`](/python-sdk/api-reference/fused-download)
