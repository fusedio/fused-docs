...
Integrate Fused UDFs with popular mapping and visualization tools. See [UDFs as API](/guide/working-with-udfs/run-udfs-as-api) for more details.
...
1. Create a UDF, [share your canvas](/workbench/udf-builder/canvas#share-modal) to get a canvas token, call your UDF through the [HTTPS endpoint](/guide/data-input-outputs/export-api/tokens-endpoints).
...
2. [Share your canvas](/workbench/udf-builder/canvas#share-modal) and use your canvas token and UDF name:
...
[Kepler](https://kepler.gl/) is an open source tool for visualizing large geospatial datasets. The Fused Map Builder provides direct integration with Kepler.

### Usage

1. Create a UDF that returns vector data
2. In the Map Builder, click "Open in Kepler.gl" on the top-right menu
...
tile_layer = ipyleaflet.TileLayer(
    url="https://udf.ai/fc_<CANVAS_TOKEN>/my_udf/run/tiles/{z}/{x}/{y}?=png",
    tile_size=512,
    zoom_offset=-1,
)
...
vector_layer = ipyleaflet.VectorTileLayer(
    url="https://udf.ai/fc_<CANVAS_TOKEN>/my_udf/run/tiles/{z}/{x}/{y}?=mvt"
)
