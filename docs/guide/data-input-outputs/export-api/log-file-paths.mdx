...
Output cache for [`@fused.cache`](/python-sdk/api-reference/cache). Each subdirectory corresponds to a UDF by name; entries within are keyed by a hash of the function arguments.
...
S3-backed output cache for [`@fused.cache`](/python-sdk/api-reference/cache). Stores results keyed by a hash of the function arguments, persisted per user and per UDF.
