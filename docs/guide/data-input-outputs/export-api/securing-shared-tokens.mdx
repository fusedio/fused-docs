...
## Creating a Fused Service Account (required for session tokens) {#fused-service-accounts}

A Fused Service Account is used to generate a session token. You can generate a service account in the Fused Workbench under Preferences.
...
[`fused.api.session_token()`](/python-sdk/api-reference/api#session_token) can be called from a UDF running in Workbench. It returns a short-lived token (default `"1h"`) you can pass as `fused_session_token` on UDF HTTPS URLs for that canvas—useful when you do not want to set up a service account yet.

To mint session tokens from **your own backend** (for example for embeds in a product), use a [Fused Service Account](#fused-service-accounts) and the HTTP API below.
