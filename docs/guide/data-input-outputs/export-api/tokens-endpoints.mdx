...
<Tabs className="unique-tabs">
  <TabItem value="udf-editor" label="UDF Editor" default>
    ...
    2. Optionally: Edit the URL to return the data in your preferred file format. (See [Calling UDFs as API](/guide/working-with-udfs/run-udfs-as-api) for more details)
    
    For example changing the format from `parquet` to `json`:

    ```bash
    # Returning as JSON (canvas token + UDF name)
  ...
  <TabItem value="advanced-udf-editor" label="Advanced UDF Editor">
    ...
    1. [Share your canvas](/workbench/udf-builder/canvas#share-modal) to get a canvas token
    2. [Save your UDF](/workbench/udf-builder/code-editor/#save-a-udf) with `Cmd + S` (MacOS) / `Ctrl + S` (Win / Linux)
    3. Click the 3 dots and open "Settings", then HTTPS URL to open the UDF in its default file format
...
### Canvas token (recommended)

After you [share your canvas](/workbench/udf-builder/canvas#share-modal), you get a **canvas token** (e.g. `fc_**********`). Every UDF on that canvas is then available at:

```
https://udf.ai/fc_<CANVAS_TOKEN>/<udf_name>.<ext>
```
...
Manage your canvas tokens and view shared links [here](https://www.fused.io/workbench/tokens).
