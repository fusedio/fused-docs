...
Read and write Airtable records from Fused UDFs using OAuth-managed tokens. No API keys need to be saved or hardcoded — Fused handles the token refresh automatically.

## Prerequisites

- A Fused account on a paid plan (execution environment required) and an Airtable account.
