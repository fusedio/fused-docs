...
For datasets from external sources like [Zenodo](https://zenodo.org/) or [Humanitarian Data Exchange](https://data.humdata.org/) that take longer than 120s to download, run as a [batch job](/guide/working-with-udfs/execution/batch-jobs):
