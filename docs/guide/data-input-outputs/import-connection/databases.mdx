...
If your Snowflake account has a [network policy](https://docs.snowflake.com/en/user-guide/network-policies), you must allowlist the Fused IPs before connections will work:
- `44.230.70.117/32`
- `52.42.164.181/32`
...
### 2. Create a Snowflake user for Fused

Create a dedicated Snowflake user with the permissions your UDFs need:
...
Add `SNOWFLAKE_USER` and `SNOWFLAKE_PASSWORD` in the Fused [secrets management UI](/workbench/preferences#secrets-management).
...
Read more about [Snowflake's Python connector](https://docs.snowflake.com/en/developer-guide/python-connector/python-connector-connect).
...
Create a UDF to set your Google [Service Account credentials](https://cloud.google.com/bigquery/docs/use-service-accounts) in your Fused runtime [disk](/guide/working-with-udfs/udf-best-practices/storage/#mntcache-disk) in a file in the `/mnt/cache` directory.
