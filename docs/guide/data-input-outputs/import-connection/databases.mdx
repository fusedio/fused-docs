...
Add `SNOWFLAKE_USER` and `SNOWFLAKE_PASSWORD` in the Fused [secrets management UI](/workbench/preferences#secrets-management). Use the user’s password or a [programmatic access token (PAT)](https://docs.snowflake.com/en/user-guide/programmatic-access-tokens) for `SNOWFLAKE_PASSWORD`—the Python connector accepts either.
