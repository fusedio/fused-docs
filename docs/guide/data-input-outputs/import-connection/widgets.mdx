...
- **`fused-map`** — a Mapbox GL + deck.gl map with typed layers (`mvt`, `raster`, `geojson`, `h3`, `heatmap`, …). A `geojson` layer can read plain lat/lng columns from any UDF via SQL — no GeoDataFrame required.
...
Convert your lat/lon columns into a GeoDataFrame with `geopandas.points_from_xy` and return it:
...
Return a GeoDataFrame with polygon geometry — the default `GeoJsonLayer` fills and outlines it automatically. Here `vizConfig` colors each neighbourhood by its average price:
...
### `fused-map` with lat/lng columns

`fused-map` is more forgiving: a `geojson` layer builds points from `latColumn`/`lngColumn` returned by a SQL query, so the UDF can return a **plain DataFrame** — no GeoDataFrame needed. Each layer carries its own `style`, and the map can show controls, a scale bar, a basemap switcher, a legend, and a layer panel:
