...
Example (bar chart) — [Try it out](https://www.fused.io/canvas/fc_fused/Widget_UDFs):
...
*Prompting the AI panel to change the chart type to a line chart — the widget updates in real time.* [Try it out →](https://www.fused.io/canvas/fc_fused/Widget_UDFs)
...
The examples below are live on the [Map widget canvas →](https://www.fused.io/canvas/fc_fused/Map_Rendering)
...
:::tip
`layers` is an array — stack as many as you like, e.g. tiled building footprints beneath a direct polygon overlay. The [Map widget canvas](https://www.fused.io/canvas/fc_fused/Map_Rendering) has live examples of combined layers and raster tilesets (`fused-map` `raster`).
:::
...
When the slider changes, every widget using `$param` (here `$top_n`) updates instantly — the bar chart and SQL table re-render in sync with the slider value. [Try it out →](https://www.fused.io/canvas/fc_fused/Widget_UDFs)
