...
A `map` layer renders whatever geometry the UDF returns. For point data you'd build geometry with `gpd.points_from_xy(df["longitude"], df["latitude"])` and return a GeoDataFrame; for polygons you return the polygon geometries directly. The example demoed below uses **polygons** — one convex hull per SF neighbourhood, colored by average price. Paste this into a UDF node named `sf_neighborhoods`:
...
By default a `map` layer connects in **viewport mode** — one request fetches data for the whole current view. Set `tile: true` to connect in **tile mode** instead: the UDF runs once per map tile, so only the visible tiles load. This is how you serve large or pre-partitioned datasets — like the `table_to_tile` example below, which streams Microsoft building footprints and renders fast partition previews when zoomed out, swapping to individual footprints as you zoom in.

In tile mode, `vizConfig` takes **two** layers:

- `tileLayer` configures the `TileLayer` wrapper that fetches tiles (`minZoom`, `maxZoom`, `tileSize`).
- `vectorLayer` styles the data rendered inside each tile.

:::warning `tile: true` is what activates `tileLayer`
The `tileLayer` key only takes effect when the layer runs in tile mode. Without `"tile": true` on the layer object, the UDF runs in viewport mode and the `tileLayer` config is ignored.
:::
