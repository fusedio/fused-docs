...
You can set any canvas parameter directly in the shared widget URL by appending it as a query string. This lets you create pre-configured links that load with specific values — no interaction required.

Using the same `orders_widget` example above, the shared URL already includes `selected_option` as a query parameter:

```
https://fused.io/share/fc_.../orders_widget?selected_option=orders
```

Change `orders` to `revenue` in the URL and reload — the widget re-renders showing revenue data instead. You can chain multiple parameters:

```
https://fused.io/share/fc_.../orders_widget?selected_option=revenue&year=2024
```

This works because every input widget that sets a `param` can also receive its value from the URL at load time. Any parameter your canvas uses can be pre-set this way, making it easy to share specific views with others.
