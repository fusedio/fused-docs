...
Example (bar chart) — [Try it out](https://unstable.fused.io/canvas/fc_fused/Widgets_Docs_Guide):
...
*Prompting the AI panel to change the chart type to a line chart — the widget updates in real time.* [Try it out →](https://unstable.fused.io/canvas/fc_fused/Widgets_Docs_Guide)
...
All three examples below are live on the [Map widget canvas →](https://www.fused.io/canvas/fc_fused/map_widget_canvas)

:::tip Two rules for the `map` widget
1. UDF names must use `{{double-brace}}` syntax — a bare string like `"my_udf"` won't resolve.
2. The UDF must return a **GeoDataFrame**. A plain DataFrame with `latitude`/`longitude` columns renders nothing.
:::

### Point data

Convert your lat/lon columns into a GeoDataFrame with `geopandas.points_from_xy` and return it. Paste this into a UDF node named `sf_listings_points`:

<details>
<summary><code>sf_listings_points</code></summary>
...
def udf(path: str = "s3://fused-sample/demo_data/airbnb_listings_sf.parquet"):
    ...
    return gpd.GeoDataFrame(
        df[["name", "neighbourhood_cleansed", "room_type", "price_in_dollar"]],
...
The simplest widget is just the UDF name — the map applies a default style and `autoFit` zooms to the data:
...
```json
{
  ...
  "props": {
    ...
    "mapStyle": "dark",
    "autoFit": true,
    "layers": ["{{sf_listings_points}}"]
  }
}
```

To control the rendering, pass a layer object with a `vizConfig`. `vizConfig.vectorLayer` is a [deck.gl layer](https://deck.gl/docs/api-reference/layers) spec — here points are colored by `price_in_dollar` with a continuous color ramp:
...
Return a GeoDataFrame with polygon geometry — the default `GeoJsonLayer` fills and outlines it automatically. Here `vizConfig` colors each neighbourhood by its average price. Paste this into a UDF node named `sf_neighborhoods`:
...
def udf(path: str = "s3://fused-sample/demo_data/airbnb_listings_sf.parquet"):
    ...
    return out[out.geometry.geom_type == "Polygon"]
```
...
`fused-map` is the most flexible map widget. Its `geojson` layer runs SQL over a UDF and builds geometry from the result, so the same layer works whether the UDF returns a **plain DataFrame** (point coordinates in `latColumn`/`lngColumn`) **or** a **GeoDataFrame** of any geometry type — points, lines, or polygons. For data too large to query in one shot, an `mvt` or `raster` layer streams a **Fused-partitioned tileset** instead, which is the key difference from the `map` widget. Each layer carries its own `style`, and the map can show controls, a scale bar, a basemap switcher, a legend, and a layer panel.

The example below reads a **plain DataFrame** and plots its lat/lng columns. Paste this into a UDF node named `sf_listings`:
...
<iframe
  src="https://unstable.fused.io/canvas/fc_fused/Widget_Showcase"
  width="100%"
  height="600px"
  frameBorder="0"
  style={{borderRadius: '8px'}}
...
When the slider changes, every widget using `$param` (here `$top_n`) updates instantly — the bar chart and SQL table re-render in sync with the slider value. [Try it out →](https://unstable.fused.io/canvas/fc_fused/Widgets_Docs_Guide)
