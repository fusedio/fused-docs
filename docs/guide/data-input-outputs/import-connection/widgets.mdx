...
<ClickZoomImage
  src="/img/widgets/widgets-system-diagram.png"
  alt="Fused Widgets System diagram"
/>
...
_Changes in the input widget make all the downstream UDFs and output widgets re-run._
...
<ClickZoomImage
  src="/img/widgets/widget-url-anatomy.png"
  alt="Widget URL anatomy"
/>
...
<ClickZoomImage
  src="/img/widgets/parameterize-sql.png"
  alt="Input widget filtering bar chart by passing $name into udf_1 at query time"
/>
...
With `aiBuilderMode: "enabled"`, you can describe what you want in plain English (for example, _"switch this to a line chart"_ or _"add a filter for price > 100"_), and the AI rewrites the inner `{ type, props }` definition for you. Use `aiPanel` to place the panel (`"right"`, `"left"`, `"top"`, or `"bottom"`).

![AI panel rewriting widget-builder config from a prompt](/img/widgets/render-widget-ai-prompt.gif)

_Prompting the AI panel to change the chart type to a line chart — the widget updates in real time._ [Try it out →](https://unstable.fused.io/canvas/fc_15tsijBQ8tPM5T2X4r09vP?bounds=846%2C-408%2C2964%2C1270)
...
| -------------------- | ------------------------------------------- | ------------ | ---------------------------------------------------------- |
| `defaultValue`       | object or string                            | _(required)_ | The widget definition to render, or a `"$param"` reference |
...
_When you share a widget URL, Fused automatically appends all canvas parameters as query parameters so the shared view can be reproduced exactly._ [Try it out →](https://unstable.fused.io/share/fc_1bHh5W6D1Yk6smpCeUuWPy/orders_widget?selected_option=orders)
