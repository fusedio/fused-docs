...
In the `multi_widget` JSON below, the slider declares `"param": "top_n"`, and both the bar chart and the SQL table use `LIMIT $top_n` — that's the dependency. Click the widget icon, paste it in, then [connect the widget to the `my_udf` UDF](#step-2-connect-it-to-a-udf).
