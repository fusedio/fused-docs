...
This makes it fast to read for any UDF, so developing in [Map Builder](/workbench/udf-builder/) & [running UDFs](/guide/working-with-udfs/udf-best-practices/realtime) is a lot faster & responsive!
...
If you only need to store a file in cloud storage, use [`fused.api.upload()`](/python-sdk/api-reference/api#upload) or [drag and drop it in Workbench](/guide/data-input-outputs/import-connection/local-files#drag--drop). If you want fast spatial reads on large geospatial data, use [`fused.ingest()`](/python-sdk/api-reference/ingest).

| Topic | [`fused.api.upload`](/python-sdk/api-reference/api#upload) | [`fused.ingest`](/python-sdk/api-reference/ingest) |
|---|---|---|
...
This is defined with [`fused.ingest()`](/python-sdk/api-reference/ingest):
...
:::tip
Refer to the [dedicated documentation page for `fused.ingest()`](/python-sdk/api-reference/ingest) for more details on all parameters
:::
...
Use the `file_id` and `chunk_id` from Step 2 with [`fused.get_chunk_from_table()`](/python-sdk/api-reference/get-chunk-from-table) to fetch just the data you need. Chunks are bounding-box aligned, so `gpd.clip()` trims the result to the exact AOI boundary.
