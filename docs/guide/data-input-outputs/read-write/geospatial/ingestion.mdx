...
This makes it fast to read for any UDF, so developing in [Workbench UDF Builder](/workbench/udf-builder/) & [running UDFs](/core-concepts/run-udfs/) is a lot faster & responsive!
...
Ingestion jobs often take more than a few seconds and require a lot of RAM, making [this a large run](/core-concepts/run-udfs/run_large/#defining-large-jobs). Use `run_batch()` so your ingestion job can take as long as needed.
