...
This makes it fast to read for any UDF, so developing in [Workbench UDF Builder](/workbench/udf-builder/) & [running UDFs](/guide/getting-started/udf-best-practices/realtime) is a lot faster & responsive!
...
Ingestion jobs often take more than a few seconds and require a lot of RAM, making [this a large run](/guide/getting-started/udf-best-practices/batch-jobs#when-to-use-batch). Use `run_batch()` so your ingestion job can take as long as needed.
