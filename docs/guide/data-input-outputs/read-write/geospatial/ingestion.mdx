...
If you only need to store a file in cloud storage, use [`fused.api.upload()`](/python-sdk/api-reference/api#upload). If you want fast spatial reads on large geospatial data, use [`fused.ingest()`](/python-sdk/api-reference/fused-ingest).
...
| Storage | `fd://` S3 bucket | `fd://` or your own S3 bucket |
...
| Best for large data? | Useful for staging files, but not optimized for querying | Recommended for large geospatial datasets |
| Cost | Minimal upload cost | Runs on a dedicated batch instance |
