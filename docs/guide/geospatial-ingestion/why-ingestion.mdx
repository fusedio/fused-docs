...
id: why-ingestion
title: Why Ingestion
sidebar_label: Why Ingestion
tags: [ingestion, cloud native]
sidebar_position: 1
description: Understand when and why to ingest geospatial data
---
...
This makes it fast to read for any UDF (and any other cloud operation), so developing UDFs in [Workbench UDF Builder](/reference/workbench/udf-builder/) & [running UDFs](/guide/running-udfs/) is a lot faster & responsive!

## Benchmark & Demo

We're going to use a real-world example to show the impact of using different file formats & partitioning to make data a lot faster to access. For this demo, we'll be using AIS (Automatic Identification System) data as for our [Dark Vessel Detection example](/guide/use-cases/dark-vessel-detection).
...
We'll download 1 day and upload it as a CSV to Fused server with [`fused.api.upload()`](/reference/python-sdk/api-reference/api#fusedapiupload):
...
But opening it on its own doesn't do all that much for us. We're going to create 3 [UDFs](/guide/running-udfs/writing-udfs) to showcase a more real-world application: Opening the dataset and returning a subset inside a bounding box. We'll do this 3 different ways to compare their execution time:
...
Now, we're going to ingest the parquet file we have to create geo-partitioned files, using `fused.ingest()`. We'll go in more details on how to ingest your own data in [the following section](/guide/geospatial-ingestion/ingest-your-data).
...
Ingestion jobs are quite memory hungry so we'll run this on a remote machine as a [batch job](/guide/running-udfs/batch-jobs):
...
Again using [File Explorer](/reference/workbench/file-explorer) to inspect our data we can see `fused.ingest()` didn't create 1 file but rather multiple:
...
:::tip
    We've implemented a utils function in the [publicly available common UDF](https://github.com/fusedio/udfs/tree/main/public/common) that allows you to more simply read Fused ingested data: [table_to_tile](https://github.com/fusedio/udfs/blob/ca262bef10b2774c4b4b58fc04548863efff2364/public/common/utils.py#L272)
...
We could actually significantly speed up the above example where we loaded the AIS data as a CSV without running `fused.ingest()`, by using [cache](/guide/scaling-up/cache):
...
We're using [`@fused.cache`](/guide/scaling-up/cache) to cache the result of `load_csv()` which is the same regardless of our `bounds`, so this allows us to save an 'intermediate' result on disk.
...
This approach is only helpful if you want to 1 time explore a new dataset in [UDF Builder](/reference/workbench/udf-builder/) and don't want to wait around for the ingestion run to be done. Beyond that, this will end up being slower (and more frustrating)
