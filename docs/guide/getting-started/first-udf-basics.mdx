...
Any UDF can be connected together in [Fused Canvas](/workbench/udf-builder/canvas/) to create a workflow using [`fused.run()`](/guide/working-with-udfs/execution/realtime#fusedrun)
...
Any UDF calling another with [`fused.run()`](/guide/working-with-udfs/execution/realtime#fusedrun) will be re-run automatically when the upstream UDF is saved & re-run.
