...
Here, `fused.load("load_cities")` loads the saved UDF named `load_cities`. It returns a callable UDF object, so `load_cities(continent=continent, engine='local')` runs it like a normal Python function.
