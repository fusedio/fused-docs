...
UDFs (User Defined Functions) are Python functions with a [`@fused.udf`](/python-sdk/api-reference/udf-decorator) decorator that return data. Each UDF becomes its own API endpoint with a unique URL.
...
A UDF needs:
- The [`@fused.udf`](/python-sdk/api-reference/udf-decorator) decorator
...
- **[Databases](/guide/data-input-outputs/import-connection/databases)** — Read from [Snowflake](/workbench/integrations/snowflake), [BigQuery](/workbench/integrations/bigquery), or other databases
