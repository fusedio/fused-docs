...
These run with no start-up time, so are quick to run, but with limited resources. Need more? See [Batch Jobs](/guide/getting-started/udf-best-practices/batch-jobs).
...
| Inside a batch job | Runs in the [batch job](/guide/getting-started/udf-best-practices/batch-jobs)'s compute (shares instance resources) |
...
This lets you leverage the [batch job's](/guide/getting-started/udf-best-practices/batch-jobs) resources (more RAM, no time limit) for the inner UDF call.
...
| Need | Solution |
|------|----------|
| Run same UDF over multiple inputs | [`fused.submit()`](/guide/getting-started/udf-best-practices/parallel) |
| More than 120s execution time | [Batch Jobs](/guide/getting-started/udf-best-practices/batch-jobs) with `instance_type` |
| More than ~4GB RAM | [Batch Jobs](/guide/getting-started/udf-best-practices/batch-jobs) with `instance_type` |
