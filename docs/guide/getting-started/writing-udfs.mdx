...
UDFs can import functions from other UDFs with `fused.load` in the [UDFs GitHub repo](https://github.com/fusedio/udfs/tree/main) or [private GitHub repos](/core-concepts/content-management/git/). Here the commit SHA `05ba2ab` pins the UDF to specific commit [for version control](/core-concepts/run-udfs/run-small-udfs/#git-commit-hash-recommended-for-most-stable-use-cases).
