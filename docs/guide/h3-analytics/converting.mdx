...
- **`_sample` metadata file** with chunk/file bounding boxes for fast spatial queries
- **`/overview/` directory** with aggregated files at lower resolutions (`hex3.parquet`, `hex4.parquet`, etc.)

![Files created by the ingestion process](/img/tutorials/geospatial/raster_to_h3_nyc_dem.png)

Each file contains:
- `hex`: H3 cell IDs at the target resolution
- `data_<metric>`: Computed values (e.g., `data_avg`)
- `source_url`, `res`: Metadata columns

<details>
<summary>View data file structure</summary>
...
<details>
<summary>View overview file structure</summary>
...
For visualization options, see [H3 Visualization](/guide/h3-analytics/visualization).
