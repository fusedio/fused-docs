  ...
  <div 
    ...
    onClick={e => {
      if (e.target.tagName !== 'A' && e.target.tagName !== 'VIDEO') {
        window.open('https://www.fused.io/canvas/fc_fused/CDL_Variables_Dashboard', '_blank');
      }
    }}
  >
    <video
    ...
    />
    <a
      href="https://www.fused.io/canvas/fc_fused/CDL_Variables_Dashboard"
      target="_blank"
      style={{
        position: 'absolute',
        top: '10px',
...
[**Launch the interactive Crop Exploration Dashboard →**](https://www.fused.io/canvas/fc_fused/CDL_Variables_Dashboard)

We provide these UDFs for now as-is. You can duplicate them and modify them to work with your own data:

:::note Why the data is embedded, not fetched
Canvas node iframes run with an opaque origin, so a client-side `fetch()` from one node's HTML back to another node's URL is blocked by CORS. Each UDF below instead computes its data server-side with [`fused.load()`](/python-sdk/api-reference/udf#udf) and embeds the result directly in the page as JSON. Cross-component sync (the histogram brush filtering the map, the map's viewport filtering the histograms) still works over `postMessage`/`BroadcastChannel`, which isn't subject to CORS.
:::
...
# Pairs with join_era5_cdl_elevation.py -- bump this note if that UDF's schema changes,
# so this UDF's own cached result gets invalidated and re-executed too.
@fused.udf
def udf(
    bounds: list = [-125.0, 32.0, -114.0, 42.0],
    res: int = 5,
    year: int = 2024,
    month: str = "2024-05",
    ...
    # Compute the joined dataset server-side and embed it directly in the page as
    # plain JSON -- canvas node iframes run with an opaque origin, so a client-side
    # fetch() back to another node's URL is blocked by CORS with no way to allow it
    # from here. The dataset is small (a few thousand rows), so there's no need for
    # a client-side SQL engine either -- plain JS array filtering is enough and has
    # far fewer moving parts (no WASM, no Worker, no CORS, no Parquet compatibility
    # surface) to fail silently inside a sandboxed iframe.
    join_era5_cdl_elevation = fused.load("join_era5_cdl_elevation")
    df = join_era5_cdl_elevation(bounds=bounds, res=res, year=year, month=month)
    df = df.copy()
    df["hex"] = df["hex"].astype(str)  # preserve full 64-bit H3 id precision through JSON (JS numbers only hold 53 bits)
    ...
    <body>
        ...
        <div class="chart-container" id="chartContainer">
            <div id="loadingState" class="loading-state">
                ...
                <div style="margin-top: 8px; font-weight: 500;">Loading dataset</div>
            </div>
        ...
        <script>
            // --- config ---
            const componentId = 'histogram-v4-' + Math.random().toString(36).substr(2, 9);
            ...
            function applyAllFilters() {
                ...
                for (let [,f] of activeFilters.entries()) {
                    const next = applyFilter(filtered, f);
                    // The map's viewport is broadcast as a spatial filter on every pan/zoom,
                    // but this dataset is a fixed snapshot, not a live re-fetch -- zooming
                    // into any sub-area with no matching points would otherwise blank the
                    // chart with no way back except re-running the UDF. Ignore a spatial
                    // filter specifically when it would wipe out everything.
                    if (f.type === 'spatial' && next.length === 0 && filtered.length > 0) continue;
                    filtered = next;
                }
            ...
            function updateHistogram(animate=false) {
                ...
                data.forEach(f => {
                    ...
                    for (let b of dynamicBins) {
                        if (value >= b.min && (value < b.max || b === dynamicBins[dynamicBins.length - 1])) { b.count++; break; }
                    }
                });
            ...
            // --- init: data is embedded, so this is synchronous, no network/WASM involved ---
            try {
                fullDataset = processRawData(RECORDS);
                dynamicBins = calculateDynamicBins(fullDataset);
                applyAllFilters();
            } catch (e) {
                console.error(e);
                statusElement.textContent = `Init failed: ${e && e.message ? e.message : e}`;
            }
...
def udf(
    ...
    # USDA CDL classification codes -> crop name.
    crop_mapping = {
        ...
        226: "Dbl Crop Oats/Corn", 227: "Lettuce", 229: "Pumpkins",
...
<body>
  ...
  <script type="module">
    ...
    const MIN_ELEVATION   = {{ min_elevation }};
    ...
    // --- filtering (same model as the histogram/bar-chart panels) ----------
    ...
    function applyCategoricalFilter(data, filter) {
      const field = filter.field;
      const values = filter.values || [];
      return data.filter(f => values.includes(getFeatureValue(f, field)));
    }

    function applyAllFilters() {
      let filtered = fullDataset.filter(f => getFeatureValue(f, 'elevation') > MIN_ELEVATION);
      for (const [, f] of activeFilters.entries()) filtered = applyFilter(filtered, f);
      filteredDataset = filtered;
      renderLayer();
      updateStatus(`rendered ${filteredDataset.length.toLocaleString()} features`);
    }
    ...
    function rowsToFeatures(rows, confObj){
      ...
      return rows.map(p => {
        const hex = p.hex ?? p.h3 ?? p.index ?? p.id;
    ...
    function onMessage(msg) {
      ...
      if (msg.type === 'filter' && msg.filter && datasetOk) {
        ...
        if (f.type === 'range' && Array.isArray(f.values)) {
          ...
          if (!field) return;
          activeFilters.set(`range_${msg.fromComponent||'unknown'}_${field}`, { type: 'range', field, values: f.values });
          applyAllFilters();
        ...
        if (f.type === 'categorical' && Array.isArray(f.values)) {
          const field = sanitizeFieldName(f.field || 'value');
          if (!field) return;
          activeFilters.set(`categorical_${msg.fromComponent||'unknown'}_${field}`, { type: 'categorical', field, values: f.values });
          applyAllFilters();
          updateStatus(`incoming categorical ${field}`);
          return;
        }
        return;
      }
    ...
    function renderLayer() {
      if (!map) return;
      const data = rowsToFeatures(filteredDataset, currentConfObj);
      fitToDataOnce(data);
    ...
    async function main(){
      ...
      // data is embedded, so this is synchronous, no network/WASM involved
      fullDataset = RECORDS;
    ...
    main().catch(e=>{
      ...
      updateStatus('init error: ' + (e && e.message ? e.message : e));
    });
