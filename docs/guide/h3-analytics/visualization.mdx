...
def udf(
    ...
    # Compute the joined dataset server-side and embed it directly in the page --
    # canvas node iframes run with an opaque origin, so a client-side fetch() back
    # to another node's URL is blocked by CORS with no way to allow it from here.
    df = fused.run("join_era5_cdl_elevation", bounds=bounds, res=res, year=year, month=month)
    buf = io.BytesIO()
    df.to_parquet(buf)
    embedded_parquet_b64 = base64.b64encode(buf.getvalue()).decode()
    ...
    <body>
        ...
        <script>
            ...
            const EMBEDDED_PARQUET_B64 = \"""" + embedded_parquet_b64 + """\";
            ...
            async function initDuckDB() {
                ...
                try {
                    ...
                    statusElement.textContent = 'Loading embedded dataset…';
                    const binary = atob(EMBEDDED_PARQUET_B64);
...
def udf(
    ...
    default_query: str = "SELECT * FROM data WHERE elevation > 0",
...
<body>
  ...
  <script type="module">
    // --- UDF PARAMS ---------------------------------------------------------
    const EMBEDDED_PARQUET_B64 = {{ embedded_parquet_b64 | tojson }};
    ...
    async function initMapAndDuckDB(){
      ...
      updateStatus('loading embedded data…');
      const binary = atob(EMBEDDED_PARQUET_B64);
      const bytes = new Uint8Array(binary.length);
      for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
...
""").render(
        embedded_parquet_b64=embedded_parquet_b64,
