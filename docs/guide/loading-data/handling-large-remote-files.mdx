...
sidebar_label: Large Remote Files
description: Download and work with large files from remote sources
---
...
2. (Optional) If you estimate the file will take more than 100s to download, run the following UDF as a [batch job](/guide/running-udfs/batch-jobs)

Here we chose a `c2-standard-4` instance type, i.e. the [smallest instance type Fused supports](/guide/running-udfs/batch-jobs#supported-instance-types) as downloading a file doesn't require heavy resources we simply need a UDF that can run for minutes or hours while we download, beyond the [120s timeout](/guide/running-udfs/realtime#timeout) of realtime UDFs
...
(For smaller files) Check out [this realtime UDF](/guide/use-cases/dark-vessel-detection#32---writing-a-udf-to-open-each-ais-dataset) downloading individual ship transponder AIS signal to S3 directly from NOAA
...
- Using [`fused.submit()`](/reference/python-sdk/top-level-functions#fusedsubmit) to run the UDF in parallel.
...
Read more about UDF execution engines [here](/guide/running-udfs/parallel-processing#execution-engines)
...
- Ingest [raster files into H3 hexagons](/guide/h3-hexagons/when-to-use-h3#converting-datasets-to-h3-tiles)
- [Visualize your data in Fused](/guide/building-apps/visualization)
- Create a [standalone map](/guide/building-apps/standalone-maps) sharable outside of Fused
