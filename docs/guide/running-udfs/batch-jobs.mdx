...
We'll use the same UDF as in the [running multiple UDFs section](/guide/running-udfs/realtime#running-jobs-in-parallel-fusedsubmit):
...
As mentioned in the [Run UDFs section](/guide/running-udfs/realtime#fusedrun), to call it 1 time we can use `fused.run()`:
...
1. Creating a job with the UDF to run and [passing input parameters](/guide/running-udfs/batch-jobs#passing-udf-parameters-with-arg_list) to run the UDF over
2. Send the job to a remote instance and (optionally) [defining the instance arguments](/guide/running-udfs/batch-jobs#run_batch-instance-arguments)
...
As mentioned [above](/guide/running-udfs/batch-jobs#running-a-job-jobrun_batch) to pass UDF arguments to a remote job, use `arg_list` to specify a `list` of inputs to run your job over:
...
We can confirm this worked by viewing our results by browsing [File Explorer](/reference/workbench/file-explorer):
...
- `instance_type`: Decide which type of machine to run your job on (see [below](/guide/running-udfs/batch-jobs#currently-supported-instance_type) for which ones we support)
...
To run a job from workbench, you can simply add an [`instance_type`](/guide/running-udfs/batch-jobs#currently-supported-instance_type) parameter and run the UDF as usual from the udf builder page:
...
To get data back from your `run_batch()` jobs is a bit more complicated than for ["regular UDF runs"](/guide/running-udfs/realtime#real-time-logs). Our recommendation is to have your UDF write data directly to shared storage `/mount/` or cloud storage and access it after

<details>
  <summary>Example job: saving to shared storage `/mount/`</summary>

    A common use case for offline jobs is as a "pre-ingestion" process. You can find a real-life example of this in our [dark vessel detection example](/guide/use-cases/dark-vessel-detection#32---writing-a-udf-to-open-each-ais-dataset)
    ...
    :::tip
      You can use [File Explorer](/reference/workbench/file-explorer) to easily see your outputs! In this case of the above example typing `efs://AIS_{datestr}.csv` (and replacing `datestr` with your date) will show the results directly in File Explorer!
    :::
...
- [Scaling an ingestion job](/guide/use-cases/dark-vessel-detection#34---ingest-1-month-of-ais-data-into-a-geo-partitioned-format) of AIS point data
- [Ingesting a large amount of cropland data](/guide/use-cases/zonal-stats#2-bring-in-your-data) for zonal statistics.