...
sidebar_label: Workbench & Visualization
sidebar_position: 1
---

# Workbench best practices

_Tips & Tricks for making [Fused Workbench](/reference/workbench/overview) work for you_

import ReactPlayer from 'react-player';

![Workbench Overview](/img/user-guide/best-practices/new_workbench.png)

[Workbench](/reference/workbench/overview) is a web-IDE built to make working with Fused UDFs even faster! 

## Experimenting with UDFs, fast

In [UDF Builder](/reference/workbench/udf-builder/) you have access to a [Code Editor](/reference/workbench/udf-builder/code-editor) that runs your UDFs and outputs results directly on the [Map View](/reference/workbench/udf-builder/map) for you. As soon as you make changes they show up in Map View! 

### 💡 Leverage all the [UDF Best Practices](/guide/running-udfs/best-practices/udf-best-practices)

While this page is for Workbench, it builds on top of all the [Best Practices](/guide/running-udfs/best-practices/udf-best-practices) that make your UDF fast & efficient. So if you haven't yet, take a look at our [dedicated UDF Best Practices](/guide/running-udfs/best-practices/udf-best-practices).
...
While you might be tempted to explore a specific row of a `GeoDataFrame` by filtering it and printing it, sometimes it is faster to just click on it in [Map View](/reference/workbench/udf-builder/map). Once selected, use the tooltip copy icon to copy the object as JSON, which you can then inspect elsewhere.
...
Map View supports [multiple base maps](/reference/workbench/udf-builder/map#map-controls) that might be better suited for different uses cases.
...
:::tip
You can reset the view by running the "Reset 3D view to top-down" shortcut from [Command Palette](#using-keyboard-shortcuts-command-palette)
:::

### Using the Visualization panel

You can easily change the styling of your data in the [Visualization](/reference/workbench/udf-builder/styling) panel:
- [Changing the color of your data](/reference/workbench/udf-builder/styling#color-styling) (especially for vector files)
- Using Visualize presets under "Preset"
- [Creating your own custom `loadingLayer`](/reference/workbench/udf-builder/styling#custom-loadinglayer-and-errorlayer)
...
### Adjusting Opacity for Better Insights
...
<details>
<summary>Example: Vegetation Segmentation with Semi-Transparent Overlay</summary>
...
<details>
<summary>Example: Advanced Overlap Analysis with H3 Hexagons</summary>
...
Your team can load your own UDFs by calling it with a [team udf name](/guide/running-udfs/realtime#team-udf-names) so be sure to give it an explicit name!

### Using tags

You can add tags to your UDF in the [Share](/reference/workbench/udf-builder/navigation#share) page. This gives yet another way to find & search your UDFs. We recommend giving tags according to:
...
You can use [Canvas](/reference/workbench/udf-builder/canvas) to organise your UDFs into different projects. This allows you to:
...
<ReactPlayer className="video__player" playing={false} muted={true} controls height="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench/walkthrough-videos/docs_rewrite/refresh_edit5.mp4" width="100%" />
