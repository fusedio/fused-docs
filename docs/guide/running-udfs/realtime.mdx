...
1.  [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun) in Python. All you need is the [`fused` Python](/reference/python-sdk/#python-install) package installed
    - Useful when wanting to run a UDF as part of another pipeline, inside another UDF or anywhere in Python / code.
2.  [**HTTPS call**](/guide/running-udfs/realtime#https-requests) from *anywhere*
    - Useful when you want to call a UDF outside of Python. For example, receiving dataframes into Google Sheets or plotting points and images in a [Felt map](/reference/udf-patterns/integrations#felt)
...
## [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun)

[`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun) is the simplest & most common way to execute a UDF from any Python script or notebook.

The simplest way to call a public UDF is using a [public UDF name](/guide/running-udfs/realtime#public-udf-name) and calling it as: `UDF_` + name. Let's take this UDF that returns [the location of the Eiffel Tower](https://github.com/fusedio/udfs/tree/main/public/Single_point_Eiffel_Tower) in a `GeoDataFrame` as an example:
...
- [By name from your account](/guide/running-udfs/realtime#name-from-your-account)
- [By public UDF name](/guide/running-udfs/realtime#public-udf-name)
- [Using a token](/guide/running-udfs/realtime#token)
- [Using a `udf` object](/guide/running-udfs/realtime#udf-object)
- [From Github URL](/guide/running-udfs/realtime#github-url)
- [From git commit hash (most recommended for teams)](/guide/running-udfs/realtime#git-commit-hash-recommended-for-most-stable-use-cases)

### _Name_ (from your account)

_When to use: When calling a UDF you made, from your own account._

You can call any UDFs you have made simply by referencing it by name (given when you [save a UDF](/guide/running-udfs/writing-udfs)).
...
You can get the token from a UDF either in [Workbench](/reference/workbench/) (Save your UDF then click "Share") or by returning the token in Python.
...
We then need to [save this UDF to Fused server](/guide/running-udfs/writing-udfs) to make it accessible from anywhere.
...
We do NOT recommend you use this approach as your UDF might break if changes are done to it
    ...
    For that reason we recommend using a [git commit hash](/guide/running-udfs/realtime#git-commit-hash-recommended-for-most-stable-use-cases) instead
...
This can be helpful when collaborating with team members as this does **not** require [making a shared token](/guide/running-udfs/realtime#token)
...
Set `sync=False` to run a UDF [asynchronously](/guide/scaling-up/async).

## Passing arguments in [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun)

A typical [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun) call of a UDF looks like this:
...
A [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun) call will require the following arguments:

1. [Mandatory] The first argument needs to be the UDF to run (name, object, token, etc [as seen above](/guide/running-udfs/realtime#fusedrun))
...
3. [Optional] Any protected arguments as seen in the dedicated [API docs page](/reference/python-sdk/top-level-functions#fusedrun) (if applicable). These include for example:
   - `bounds` -> A geographical bounding box (as a list of 4 points: `[min_x, min_y, max_x, max_y]`) defining the area of interest.
   - `cache_max_age` -> The maximum [age of the UDF's cache](/guide/scaling-up/cache#defining-your-cache-lifetime-cache_max_age).
...
The same can be done with the [Fused Python SDK](/reference/python-sdk/).
...
- Your UDF is set to [Tile mode](/reference/udf-patterns/bounds-and-tiles#tile-udf)
- You properly pass `/{z}/{x}/{y}` instead of default value
- Depending on your server type, you might need to add the `format` parameter. For example [QGIS](/reference/udf-patterns/integrations#qgis) requires you to use `format=mvt` for [vector tile servers](/reference/udf-patterns/integrations#vector-tiles-1)

```
https://www.fused.io/server/v1/realtime-shared/******/run/tiles/{z}/{x}/{y}?format=png
```

See our [integration guides](/reference/udf-patterns/integrations) for examples.

### Private token

Calling UDFs with [Bearer authentication](https://swagger.io/docs/specification/authentication/bearer-authentication/) requires an [account's private token](/reference/python-sdk/authentication#get-bearer-access-token). The URL structure to run UDFs with the private token varies slightly, as the URL specifies the UDF's name and the owner's user account.
...
- The supported types for vector tables are `parquet`, [`geojson`](/reference/udf-patterns/integrations#deckgl), [`json`](/reference/udf-patterns/integrations#deckgl), `feather`, `csv`, [`mvt`](/reference/udf-patterns/integrations#mapbox), `html`, `excel`, and `xml`.
- For raster array: [`png`](/reference/udf-patterns/integrations#deckgl), `gif`, `jpg`, `jpeg`, `webp`, `tif`, and `tiff`.
...
Read how to structure HTTPS endpoints to call the UDF as a [Map Tile & File](/reference/udf-patterns/bounds-and-tiles#tile-https-call).
...
You first do need to retrieve your [bearer token](/reference/python-sdk/authentication#get-bearer-access-token) (this requires Python):
```python showLineNumbers
...
1. In [Workbench](/reference/workbench/), go to "Preferences"
...
If a UDF's [cache](/guide/scaling-up/cache) is enabled, its endpoints cache outputs for each combination of code and parameters. The first call runs and caches the UDF, and subsequent calls return cached data.
