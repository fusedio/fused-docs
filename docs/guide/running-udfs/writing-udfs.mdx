...
sidebar_position: 3
tags: [udf, "@fused.udf", "@fused.cache", typed parameters, utility functions]
description: Learn how to write User Defined Functions (UDFs) in Fused
---

![udf anatomy](/img/udfanatomy.png)

Follow these steps to write a [User Defined Function (UDF)](/guide/getting-started/).

- Decorate a function with [`@fused.udf`](/guide/running-udfs/writing-udfs#fusedudf-decorator)
- [Declare the function](/guide/running-udfs/writing-udfs#function-declaration) logic
- Optionally [cache](/guide/running-udfs/writing-udfs#fusedcache-decorator) parts of the function
- Set [typed parameters](/guide/running-udfs/writing-udfs#typed-parameters) to dynamically run based on inputs
- Import [utility modules](/guide/running-udfs/writing-udfs#utils-module) to keep your code organized
- [Return](/guide/running-udfs/writing-udfs#return-object) a vector table or raster
- [Save](/guide/running-udfs/writing-udfs#save-udfs) the UDF
...
```
:::info
The UDF Builder in Workbench imports the `fused` module automatically. To write UDFs outside of Workbench, install the [Fused Python SDK](/reference/python-sdk/) with `pip install fused` and import it with `import fused`.
:::
...
Use the [@fused.cache](/guide/scaling-up/cache) decorator to persist a function's output across runs so UDFs run faster.
...
This example shows the [`bounds` parameter](/reference/udf-patterns/bounds-and-tiles#bounds) typed as `fused.types.Bounds`
...
To write UDFs that run successfully as both [`File` and `Tile`](/reference/udf-patterns/bounds-and-tiles/), set `bounds` as the first parameter, with `None` as its default value. This enables the UDF to be invoked successfully both as `File` (when `bounds` isn't passed) and as `Tile`. For example:
...
The UDF Builder runs the UDF as a [Map Tile](/reference/udf-patterns/bounds-and-tiles/) if the [first parameter](/reference/udf-patterns/bounds-and-tiles#bounds) is typed as `fused.types.Bounds`.
...
When running a UDF with `fused.run`, it's possible to specify the [map tile](/reference/udf-patterns/bounds-and-tiles#tile-udf) Fused will use to structure the `bounds` object by using the following reserved parameters.
...
UDFs can import functions from other UDFs with `fused.load` in the [UDFs GitHub repo](https://github.com/fusedio/udfs/tree/main) or [private GitHub repos](/guide/content-management/git). Here the commit SHA `05ba2ab` pins the UDF to specific commit [for version control](/guide/running-udfs/realtime#git-commit-hash-recommended-for-most-stable-use-cases).
...
- Arrays: `numpy.ndarray`, `xarray.Dataset`, `xarray.DataArray`, and `io.BytesIO`. Fused Workbench only supports the rendering of `uint8` arrays. Rasters without spatial metadata should indicate their [tile bounds](/reference/udf-patterns/bounds-and-tiles#tile-udf).
...
UDFs exported from the [UDF Builder](/reference/workbench/udf-builder/) or saved locally are formatted as a `.zip` file containing associated files with the UDFs code, `utils` Module, metadata, and `README.md`.
...
This will allow you to access your UDF using a [token](/guide/running-udfs/realtime#token), from a [Github commit](/guide/running-udfs/realtime#git-commit-hash-recommended-for-most-stable-use-cases) or directly importing it in [Workbench from the Github URL](/reference/workbench/udf-catalog#add-from-github-url)

### In Workbench: Saving through Github

You can also save your UDFs directly through GitHub as personal, team or community UDF. Check out the [Contribute to Fused](/reference/workbench/udf-catalog#contribute-to-fused) to see more.
...
A common approach to debug UDFs is to show intermediate results in the [UDF Builder](/reference/workbench/udf-builder/) [runtime panel](/reference/workbench/udf-builder/results) with `print` statements.
