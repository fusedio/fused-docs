...
The goal of Fused is to make developing & running code faster for data scientists. This is done by using [efficient file formats](/guide/geospatial-ingestion/geospatial-file-formats) and making [UDFs simple to run](/guide/running-udfs/). On top of those, Fused relies heavily on caching to make recurring calls much faster.
...
## Caching any Python function: [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache)

### Locally

Any Python function, either inside a UDF or even locally on your machine can be cached using the [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache) decorator around it:
...
:::tip
Similar to how this works with [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun), you can overwrite [`cache_max_age`](#defining-your-cache-lifetime-cache_max_age) when executing your function directly:
...
This also works inside a UDF by passing [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache) decorator around any function:
...
A function cached with [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache) is:
- Cached for [12h by default](/reference/python-sdk/top-level-functions#fusedcache) (can be changed with [`cache_max_age`](#defining-your-cache-lifetime-cache_max_age))
- Stored as pickle file on `mount/`

### Benchmark: With / without [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache)

Using [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache) is mostly helpful to cache functions that have long, repetitive calls like for example loading data from slow file formats.
...
### Best Practices: [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache)
...
However, be wary of relying on [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache) to load very large (>10Gb) datasets as cache is only stored for a few hours by default and is over-written each time you change the cached function or inputs.

Look into [ingesting your data](/guide/geospatial-ingestion/) in partitioned [cloud native formats](/guide/geospatial-ingestion/geospatial-file-formats) if you're working with large datasets.

:::tip
The line between when to ingest your data or use `@fused.cache` to load data inside a UDF is a bit blurry. Check [this section](/guide/geospatial-ingestion/why-ingestion/#using-cache-as-a-single-use-ingester) for more
:::

### Example use cases

You can look at some real-world use cases in some of our Examples:
- [Caching a STAC catalog request](/guide/use-cases/dark-vessel-detection#41-cleaning-our-sentinel-1-udf) when fetching Sentinel 1 radar satellite image in our Dark Vessel Detection example
...
While [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache) allows you to cache functions locally or _inside_ UDFs, UDFs ran with [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun) are cached by default on Fused server.
...
Cached UDF are:
- Stored for 90d by default (see [Python SDK](/reference/python-sdk/top-level-functions#fusedudf) for more details)
- Stored on S3
- You can overwrite the cache age by passing [`cache_max_age`](#defining-your-cache-lifetime-cache_max_age) either when defining the UDF with [`@fused.udf(cache_max_age)`](/reference/python-sdk/top-level-functions#fusedudf) or when running the UDF with [`fused.run(udf, cache_max_age)`](/reference/python-sdk/top-level-functions#fusedrun)
...
### Caching & [`bounds`](/reference/udf-patterns/bounds-and-tiles#bounds)

Pass [`bounds`](/reference/udf-patterns/bounds-and-tiles#bounds) to make the output unique to each [Tile](/reference/udf-patterns/bounds-and-tiles#tile-udf).
...
This also works with [`@fused.udf()`](/reference/python-sdk/top-level-functions#fusedudf) & [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun):
```python showLineNumbers
...
This also works in combination with [`@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache):
...
This is because UDFs are [cached by default](#caching-a-udf) even without passing any [`cache_max_age`](#defining-your-cache-lifetime-cache_max_age) argument
...
{/* :::tip
    Read more about this in the [Python SDK page on `@fused.cache`](/reference/python-sdk/top-level-functions#fusedcache)
::: */}
...
- Using `@fused.cache` on regular Python functions when developing locally:
    - You don't need to even use [`fused.run()`](/reference/python-sdk/top-level-functions#fusedrun), you could use `@fused.cache` without ever using any other Fused functionality
