...
{/* TODO: Add example / section in docs */}

| Method      | Processing Method | Usability | How To      |
|---------------------- |-------------------|----------------------|-------------------------|
| Single File    | Area of Interest in 1 UDF     | Recompute required for every new area / resolution  | See Example UDF: [DEM_Tile_Hexify](https://www.fused.io/workbench/udf/catalog/DEM_Tile_Hexify-c9aae65c-0047-42ba-8b3c-4326a5a2e47b) (Set to [Single UDF Mode](/tutorials/Geospatial%20with%20Fused/filetile/#single-udf))  |
| Dynamic Tile   | Area of Interest in parallel UDFs  | Recompute required for every new area / resolution | [See H3 Tiling Docs Section](/tutorials/Geospatial%20with%20Fused/h3-tiling/dynamic-tile-to-h3#raster-to-hex) |
...
[Fused UDFs](/core-concepts/write/) are serverless Python functions, so we can also create visualizations with them. We've built a simple UDF library to render data on maps.

We can create a new UDF that loads the data and filters for a specific crop type (you can see the CDL Crop codes [here](https://developers.google.com/earth-engine/datasets/catalog/USDA_NASS_CDL#bands))
...
def udf(
    ...
    utils = fused.load("https://github.com/fusedio/udfs/tree/6dd2c4e/community/milind/map_utils")
...
<div style={{ position: 'relative', display: 'flex', justifyContent: 'center', marginBottom: '1em' }}>
  ...
  >
    <video
      ...
      title="Crop Canvas Exploration Video Walkthrough"
      muted
      playsInline
      loop
      autoPlay
...
<div style={{ position: 'relative', display: 'flex', justifyContent: 'center', marginBottom: '1em' }}>
  ...
  >
    <video
      ...
      style={{ maxWidth: '100%', borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', display: 'block' }}
      title="Elevation Canvas Video Walkthrough"
      muted
      playsInline
      loop
      autoPlay
...
- Hex partitioned dataset by Fused: `s3://fused-asset/data/era5/t2m_daily_mean_v2_1000/` (See on [File Explorer](https://www.fused.io/workbench/files?path=s3%3A%2F%2Ffused-asset%2Fdata%2Fera5%2Ft2m_daily_mean_v4_1000%2F))
...
<div style={{ position: 'relative', display: 'flex', justifyContent: 'center', marginBottom: '1em' }}>
  ...
  >
    <video
      ...
      style={{ maxWidth: '100%', borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', display: 'block' }}
      title="Temperature Canvas Video Walkthrough"
      muted
      playsInline
      loop
      autoPlay
...
- [Step 2: Join Datasets](/tutorials/Geospatial%20with%20Fused/use-cases/Crop%20Exploration%20with%20H3/joining_datasets)
