...
- [Installing `fused`](/reference/python-sdk/#python-install) with `[all]` dependencies (mainly to have `pandas` & `geopandas`):
...
Since our AIS dataset is not in a geo-partitioned, cloud native format, our first step is to [ingest](/guide/geospatial-ingestion/) it into a tiled format and put it on a cloud bucket. Thankfully we can [do all of this in Fused](/guide/geospatial-ingestion/ingest-your-data).
...
[`fused.file_path()`](/reference/python-sdk/top-level-functions/#fusedfile_path) returns the mount path of any file we'd like to create. When run locally, it returns the local `/tmp/fused/` directory on your machine.
If this function is called inside a UDF, and that UDF is run on Fused servers, the returned path will be of the shared mounted storage `/mount/`. Specifying [`engine='local'`](/reference/python-sdk/top-level-functions/#fusedrun) runs the UDF on your local machine.
...
The rest of the logic is [write a UDF](/guide/running-udfs/writing-udfs) to open each file, read it as a CSV and write it to parquet.
...
Since each UDF takes a few seconds to run per date, we're going to use [`fused.submit()`](/guide/running-udfs/parallel-processing) to call a large numbers of UDFs all at once that will each run over a single date.
...
[`fused.submit()`](/guide/running-udfs/parallel-processing) requires inputs as a list or `DataFrame`, in which case columns should have the argument names that our UDF expects, in this case `datestr`. We also recommend you always do a first run with `debug_mode=True` to [test your submit job](/guide/running-udfs/parallel-processing#debug-mode):
...
`debug_mode=True` runs the 1st value inside `date_ranges` with [`fused.run()`](/reference/python-sdk/top-level-functions/#fusedrun). This allows you to make sure your UDF + arg_list combo is working properly. 
...
:::tip
[`fused.submit()`](/guide/running-udfs/parallel-processing) has [more parameters](/reference/python-sdk/top-level-functions#fusedsubmit) you can control allowing you to change the number of `max_workers`, `engine` or the retry policies. Also take a look at the [technical docs](/reference/python-sdk/top-level-functions#fusedsubmit) for more details.
:::
...
These individual parquet files are now store on our mount disk. We could save them directly onto cloud storage but before that we can geo-partition them to make them even faster to read. This will make us reduce the time it takes to access our data from [minutes to seconds](/guide/geospatial-ingestion/why-ingestion/).
Fused provides a simple way to do this with the [ingestion process](/guide/geospatial-ingestion/).
...
We'll send this job to a [large instance using `job.run_batch()`](/guide/running-udfs/batch-jobs) as we latency doesn't matter much (we can wait a few extra seconds) and we'd rather have a larger machine & a larger storage:
...
This UDF simply returns our [Map viewport](/reference/udf-patterns/bounds-and-tiles#legacy-fusedtypesviewportgdf) as a `gpd.GeoDataFrame`, this is a good starting point for our UDF returning Sentinel 1 images
...
- Add a [cache decorator](/guide/scaling-up/cache) to code functions that retrieve data to speed up the UDF & reduce costs.
...
If we save and call this UDF with a [token](/guide/running-udfs/http-endpoints) we can call it from anywhere, from a Jupyter Notebook or from another UDF. Let's create a new UDF in Workbench:
...
We're going to leverage code from the community for this part, namely reading the AIS data from a geo-partitioned GeoParquet. Fused allows us to easily re-use any code we want and freeze it to a specific commit so it doesn't break our pipelines ([read more about this here](/guide/running-udfs/realtime#loading-udfs-from-github))
...
- Running [UDFs at scale with `run_batch()`](/guide/running-udfs/batch-jobs). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
- More about Fused core concepts like chosing between [running UDFs based on Tile or File](/reference/udf-patterns/bounds-and-tiles/)
