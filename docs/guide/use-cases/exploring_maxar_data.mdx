...
### Requirements
- [Access to Fused](/reference/python-sdk/authentication/)
...
- Show how to use [`fused.submit()`](/reference/python-sdk/top-level-functions#fusedsubmit) to fetch all the available STAC Catalogs all at once in parallel in just a few seconds
...
You can explore it yourself by directly [reading the code in Github](https://github.com/fusedio/udfs/blob/main/public/common). If you see any functions you'd like to use, we [strongly recommend](/guide/running-udfs/realtime#loading-udfs-from-github) you use `fused.load()` and pass the latest commit hash at the time you want to use it:
...
:::tip
You can easily rename your UDFs in Workbench. Rename this UDF to `Maxar_Open_Data_STAC_single_catalog` so we can call it later [directly by name](/guide/running-udfs/realtime#calling-udfs-by-name).
...
- Use [`fused.submit()`](/reference/python-sdk/top-level-functions#fusedsubmit) to fetch all the STAC Collections for each event name
...
- We're using [`@fused.cache`](/reference/python-sdk/top-level-functions/#fusedcache) to cache the Catalog request so our UDF doesn't need to do this request each time we execute it. It prevents being rate limited and doing the same request over and over against the same endpoint
- We're returning a list (`collections`) so if you run this in Workbench you'll notice nothing shows up on the map! That's also [why we print](/guide/running-udfs/best-practices/udf-best-practices#use-print) the first 5 rows. 

:::tip
Read through [the Best Practices](/guide/running-udfs/best-practices/udf-best-practices) for more handy tips on how to write efficient and easy to debug UDFs
:::
...
We're going to use [`fused.submit()`](/reference/python-sdk/top-level-functions#fusedsubmit) to run our first UDF in parallel. To do this we need a few things:
...
- We use `fused.submit(..., debug_mode = True)` to run only the 1st value from `collections_df`. This allows us to test that our `fused.submit()` job is [written correctly](/guide/running-udfs/best-practices/udf-best-practices#test-out-your-udfs-before-running-them-in-parallel). 

:::tip
`fused.submit()` allows you to run a single over a list / dataframe of inputs in parallel. Under the hood Fused spins up many realtime instances (see [technical docs for details](/reference/python-sdk/top-level-functions/#fusedsubmit)) that will each run the given UDF (in this case `"Maxar_Open_Data_STAC_single_catalog"`) all at the same time.

This is a powerful way to scale a process with just a single function call.

Read [the dedicated Docs section](/guide/running-udfs/parallel-processing) on `fused.submit()` for more
:::
...
Unpacking this UDF:
- This UDF takes :
    - a [`bounds`](/reference/udf-patterns/bounds-and-tiles#bounds) object. This allows us to pass the [current Workbench Map Viewport](/workbench/udf-builder/map/) to our UDF
...
:::note
These images can be loaded using `bounds` and [Tile mode](/reference/udf-patterns/bounds-and-tiles#tile-udf) because Maxar has provided these images as [Cloud Optimized Geotiffs](/guide/geospatial-ingestion/geospatial-file-formats#for-rasters-images-cloud-optimized-geotiff). This allows us to leverage their tiles & overviews and only load the data we need as we pan around the map
...
- 2. [Tile / File modes](/reference/udf-patterns/bounds-and-tiles/)
...
This change returns the image instead of the extent, but it returns the image all at once, because Workbench is set to ["Single" mode](/reference/udf-patterns/bounds-and-tiles#single-udf) by default
...
We can change this by setting Workbench to ["Tile" mode](/reference/udf-patterns/bounds-and-tiles#tile-udf):
...
- Use [`fused.submit()`](/reference/python-sdk/top-level-functions#fusedsubmit) to run a [UDF in parallel](/guide/running-udfs/parallel-processing)
- Use [`@fused.cache`](/reference/python-sdk/top-level-functions/#fusedcache) to cache requests to reduce costs and improve performance
- Use Workbench to [display images](/workbench/udf-builder/map/) in [different modes](/reference/udf-patterns/bounds-and-tiles/)

If you want to go a bit further you could:
- Explore the Best Practices to [make the most of UDFs](/guide/running-udfs/best-practices/udf-best-practices) or learn handy tips to use [Workbench](/guide/running-udfs/best-practices/workbench-best-practices) as its fullest
- Go more in depth with ["File" & "Tile" modes](/reference/udf-patterns/bounds-and-tiles/) in Workbench
- Dive into the different ways Fused allows you to use [caching](/guide/scaling-up/cache) to improve performance
