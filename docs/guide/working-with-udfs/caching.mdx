...
Look into [ingesting your data](/tutorials/Geospatial%20with%20Fused/geospatial-data-ingestion/) in partitioned [cloud native formats](/tutorials/Geospatial%20with%20Fused/geospatial-data-ingestion/geospatial-file-formats) if you're working with large datasets.

:::tip
The line between when to ingest your data or use `@fused.cache` to load data inside a UDF is a bit blurry. Check [this section](/tutorials/Geospatial%20with%20Fused/geospatial-data-ingestion/why-ingestion/#using-cache-as-a-single-use-ingester) for more
:::
...
Pass [`bounds`](/tutorials/Geospatial%20with%20Fused/filetile/#bounds) to make the output unique to each [Tile](/tutorials/Geospatial%20with%20Fused/filetile/#tile-udf).
