...
The goal of Fused is to make developing & running code faster for data scientists. This is done by using [efficient file formats](/guide/data-input-outputs/read-write/geospatial/ingestion) and making [UDFs simple to run](/guide/getting-started/udf-best-practices/realtime). On top of those, Fused relies heavily on caching to make recurring calls much faster.
...
Pass [`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds) to make the output unique to each [Tile](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf).
...
- Using `@fused.cache` on regular Python functions when developing locally:
    - You don't need to even use [`fused.run()`](/guide/getting-started/udf-best-practices/realtime#fusedrun), you could use `@fused.cache` without ever using any other Fused functionality
