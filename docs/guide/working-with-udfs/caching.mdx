...
The goal of Fused is to make developing & running code faster for data scientists. This is done by using [efficient file formats](/guide/data-input-outputs/read-write/geospatial/ingestion) and making [UDFs simple to run](/guide/working-with-udfs/execution/realtime). On top of those, Fused relies heavily on caching to make recurring calls much faster.
...
:::tip
Similar to how this works with [`fused.run()`](/python-sdk/top-level-functions/#fusedrun), you can overwrite [`cache_max_age`](/guide/working-with-udfs/caching/#defining-your-cache-lifetime-cache_max_age) when executing your function directly:
...
- Cached for [12h by default](/python-sdk/top-level-functions/#fusedcache) (can be changed with [`cache_max_age`](/guide/working-with-udfs/caching/#defining-your-cache-lifetime-cache_max_age))
...
:::tip
The line between when to ingest your data or use `@fused.cache` to load data inside a UDF is a bit blurry. Check [this section](/guide/data-input-outputs/read-write/geospatial/ingestion#using-cache-as-a-single-use-ingester) for more
:::
...
- Stored on S3
- You can overwrite the cache age by passing [`cache_max_age`](/guide/working-with-udfs/caching/#defining-your-cache-lifetime-cache_max_age) either when defining the UDF with [`@fused.udf(cache_max_age)`](/python-sdk/top-level-functions/#fusedudf) or when running the UDF with [`fused.run(udf, cache_max_age)`](/python-sdk/top-level-functions/#fusedrun)
...
### Caching & [`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds)

Pass [`bounds`](/guide/working-with-udfs/geospatial/single-vs-tile#bounds) to make the output unique to each [Tile](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf-udf).
...
This is because UDFs are [cached by default](/guide/working-with-udfs/caching/#caching-a-udf) even without passing any [`cache_max_age`](/guide/working-with-udfs/caching/#defining-your-cache-lifetime-cache_max_age) argument
...
- Using `@fused.cache` on regular Python functions when developing locally:
    - You don't need to even use [`fused.run()`](/guide/working-with-udfs/execution/realtime#fusedrun), you could use `@fused.cache` without ever using any other Fused functionality
