...
Use batch when your UDF exceeds [realtime limits](/guide/working-with-udfs/execution/realtime#realtime-limits):
...
|--------|----------|---------------|
| [`@fused.udf(instance_type=...)`](#in-workbench-fusedudfinstance_type) | Run current UDF as batch | Yes (Approval Required) |
| [`fused.run(..., instance_type=...)`](#calling-another-udf-fusedrun-instance_type) | Call another UDF as batch | **No**  |
| [`fused.submit(..., instance_type=...)`](#multiple-jobs-fusedsubmit-instance_type) | Multiple jobs on one machine | **No** |
...
Batch jobs should write results to [cloud storage](/guide/data-input-outputs/import-connection/cloud-storage) (S3, GCS, etc.) rather than returning large data:
