...
We'll use the same UDF as in the [running multiple UDFs section](/guide/working-with-udfs/execution/parallel):
...
As mentioned [above](/guide/working-with-udfs/execution/batch-jobs#running-jobrun_batch-from-notebook) to pass UDF arguments to a remote job, use `arg_list` to specify a `list` of inputs to run your job over:
...
- `instance_type`: Decide which type of machine to run your job on (see [below](#currently-supported-instance_type) for which ones we support)
...
To run a job from workbench, you can simply add an [`instance_type`](#currently-supported-instance_type) parameter and run the UDF as usual from the udf builder page:
...
To get data back from your `run_batch()` jobs is a bit more complicated than for ["regular UDF runs"](/guide/working-with-udfs/execution/parallel#real-time-logs). Our recommendation is to have your UDF write data directly to shared storage `/mount/` or cloud storage and access it after
