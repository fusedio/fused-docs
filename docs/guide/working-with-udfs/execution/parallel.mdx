...
Each input spawns a separate [realtime UDF](/guide/working-with-udfs/execution/realtime):
...
**Characteristics:**
- Each job has [realtime limits](/guide/working-with-udfs/execution/realtime#realtime-limits) (120s, ~4GB RAM)
...
- Resources depend on `instance_type` chosen; see [Batch Jobs](/guide/working-with-udfs/execution/batch-jobs#instance-types) for options
...
Use [non-blocking execution](/guide/working-with-udfs/execution/parallel#non-blocking-execution) with `collect=False` and gather file paths after:
...
| `engine` | `local` or `remote` (see [Execution engines](/guide/working-with-udfs/execution/realtime#execution-engines)) |
