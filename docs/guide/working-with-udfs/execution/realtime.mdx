...
| Inside a batch job | Runs in the batch job's compute (shares instance resources) |
| On your laptop | Runs on your local machine (requires `fused` package installed) |
...
This lets you leverage the [batch job's](/guide/working-with-udfs/execution/batch-jobs) resources (more RAM, no time limit) for the inner UDF call.
