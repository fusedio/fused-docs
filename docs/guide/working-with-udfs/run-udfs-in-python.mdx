...
Use [`fused.load()`](/python-sdk/api-reference/load) to get a UDF object, then call it directly:
...
| [Token](/guide/data-input-outputs/export-api/tokens-endpoints#shared-token) | `fused.load("fsh_***")` | Legacy per-UDF token; still supported. Share UDF without exposing code |
| Git commit | `fused.load("github.com/.../tree/{hash}/")` | Production stability |

Legacy `fsh_` tokens remain supported in the Python SDK; for new sharing, use [canvas tokens](/guide/data-input-outputs/export-api/tokens-endpoints) (`fc_<token>/<udf_name>`).
...
## [[Legacy](/python-sdk/api-reference/run)]: `fused.run()`
