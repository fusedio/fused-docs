...
**Group agent-facing UDFs on a dedicated canvas.** Keep data query and unit processing UDFs together on one canvas that you're comfortable sharing. Move large batch jobs, internal utilities, or anything you don't want agents to trigger to a separate canvas.
...
**Expose your canvas as an MCP endpoint** by clicking **Share → Open API** in Workbench. This generates an MCP-compatible endpoint that AI tools can connect to directly.

**Hide UDFs you don't want agents to see.** Only UDFs that are not hidden in Workbench are included in the MCP endpoint. If a UDF is a helper, an internal utility, or something you don't want an agent to call directly, hide it — it will still run when called by other UDFs, but won't appear as a tool.
