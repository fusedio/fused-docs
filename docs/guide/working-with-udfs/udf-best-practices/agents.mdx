...
UDFs work naturally as tools for LLM agents — they're HTTP endpoints with typed parameters, structured outputs, and no deployment step. An agent calls a UDF the same way any HTTP client does:
...
But agents have specific constraints: limited context windows, a [120s execution limit](/guide/working-with-udfs/udf-best-practices/realtime#limits), and no tolerance for ambiguous interfaces. This guide covers how to design UDFs that agents can discover, call, and chain reliably.
