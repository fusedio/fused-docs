...
Batch jobs should **write results to S3 or `/mnt/cache`**, not return them. Data returned from batch jobs can be lost if the connection times out.
...
See [Run UDFs in python](/guide/working-with-udfs/run-udfs-in-python#instance_type) for the full list of AWS/GCP instance types.
...
- [Run UDFs in python](/guide/working-with-udfs/run-udfs-in-python#instance_type) — `instance_type` reference
- [Run UDFs in parallel](/guide/working-with-udfs/run-udfs-in-parallel) — `fused.submit()` with batch instances
