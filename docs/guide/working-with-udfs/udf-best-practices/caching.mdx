...
|------|-----------------|---------|-------------|-------|
| [UDF cache](#udf-cache) | UDF call results | [S3](/guide/data-input-outputs/import-connection/cloud-storage) | 14 days | Good |
| [`@fused.cache`](#fusedcache) | Functions inside UDFs | [mount](/guide/data-input-outputs/import-connection/cloud-storage#mntcache-disk) | 48 hours | Fast |
...
| [HTTPS endpoint](/guide/working-with-udfs/run-udfs-as-api) | `udf.fused.io/token.json?cache_max_age=0` |
...
### Workbench: Widget-driven cache invalidation

In Workbench, **changing a default parameter in an upstream UDF does not auto-invalidate the downstream cache**. Default param changes are static code edits — the downstream UDF doesn't "see" them unless it re-runs with new inputs.

```python
# Upstream UDF — changing "Name_25" to something else won't invalidate downstream
@fused.udf
def udf(name: str = "Name_25"):
    import pandas as pd
    return pd.DataFrame({"greeting": [f"Hello, {name}!"]})

# Downstream UDF — its cached result stays stale even after upstream default changes
@fused.udf
def udf(name: str = "Name_20"):
    parent_udf = fused.load("upstream_udf")
    return parent_udf(name=name)
```

**Adding a Widget changes this.** When a [widget](/workbench/widget-builder) (e.g. a dropdown) is bound to a param, it broadcasts the selected value at runtime via canvas message. This re-executes both the upstream and any downstream UDFs that share that param — effectively auto-invalidating their caches.

```python
# Widget (name_picker dropdown) broadcasts selected_name →
# upstream re-executes → downstream re-executes automatically
@fused.udf
def udf(selected_name: str = "Name_25"):
    import pandas as pd
    return pd.DataFrame({"greeting": [f"Hello, {selected_name}!"]})
```

**To manually flush cache in Workbench:** right-click a UDF node and select **Clear Cache**, or pass `cache_max_age=0` at call time.

[Try this example on Workbench →](https://unstable.fused.io/canvas/fc_5M6Ag2R9xr3fvghVgurFmr?bounds=-625%2C-101%2C2149%2C2171)
...
|---|---|
| Run in Workbench | Cache A |
| HTTP request (shared link, API, curl) | Cache B |
| `.json` response format | Cache C |

This means cache invalidation triggered inside Workbench won't propagate to your live HTTP endpoints—and vice versa. If you've updated a UDF and need fresh results everywhere, you'll need to bust the cache in each context (e.g. by passing `cache_max_age=0` in the request).
...
See how much caching is saving you in the **[Profile](/workbench/profile)** page in Workbench. The usage dashboard shows cache hits vs actual compute across different time ranges.
