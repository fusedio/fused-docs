...
![Efficient Caching diagram](/img/caching-diagram.png)
...
If you want to invalidate the parent UDF's cache directly, call
`invalidate_cache()` on the loaded `Udf` object before invoking it:

```python
@fused.udf
def child_udf():
    parent = fused.load("parent_udf")
    parent.invalidate_cache()  # Invalidates cached results for parent_udf
    data = parent()
    return data
```
