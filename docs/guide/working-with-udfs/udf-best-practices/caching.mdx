...
### Widgets trigger cache invalidation

A [widget node](/guide/data-input-outputs/import-connection/widgets) bound to a param broadcasts its selected value at runtime when the user changes it. This causes the receiving UDF — and any downstream UDFs sharing that param — to re-execute, auto-invalidating their caches.

[Try this example on Workbench →](https://unstable.fused.io/canvas/fc_5M6Ag2R9xr3fvghVgurFmr?bounds=-625%2C-101%2C2149%2C2171)

Widget node (dropdown):
...
```json
{
  ...
  "props": {
    ...
    "param": "selected_name",
    "options": [
      { "value": "Name_10", "label": "Name 10" },
      { "value": "Name_25", "label": "Name 25" },
      { "value": "Name_50", "label": "Name 50" },
      { "value": "Name_99", "label": "Name 99" }
    ]
  }
}
```

Upstream UDF receiving the widget param:
...
Downstream UDF — also re-executes when the widget changes:

```python
@fused.udf
def udf(selected_name: str = "Name_25"):
    parent_udf = fused.load("upstream_udf")  # re-runs with the value broadcast by the widget
    return parent_udf(selected_name=selected_name)
```

Output when `Name_50` is selected:
...
You can also manually flush cache at any time in Workbench — either from the [Runtime panel](/workbench/udf-builder/runtime/) or by clicking the **⋮** menu on a UDF node and selecting **Invalidate cache**.
