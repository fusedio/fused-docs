...
The examples below use `flaky_task` — a UDF that sleeps 2s and raises a `ValueError` on ~50% of runs:
...
#  timedelta(seconds=9, microseconds=824293)]  # job 5 took 9s — an outlier
```
