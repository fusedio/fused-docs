...
:::note
A cache hit on the second run makes those lines appear near-instant, so profiler output can differ between runs. See [Efficient caching](/guide/working-with-udfs/udf-best-practices/caching) for details.
:::
