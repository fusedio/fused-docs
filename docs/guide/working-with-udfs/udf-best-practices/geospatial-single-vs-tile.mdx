---
id: geospatial-single-vs-tile
title: Geospatial processing
sidebar_label: Geospatial processing
sidebar_position: 5
---
...
A Single UDF is called like any other UDF via a [HTTPS request](/guide/data-input-outputs/export-api/tokens-endpoints):

```bash
https://udf.ai/fc_<CANVAS_TOKEN>/my_udf.csv
```
Replace `fc_<CANVAS_TOKEN>` and `my_udf` with your [canvas token and UDF name](/guide/data-input-outputs/export-api/tokens-endpoints).

Or via the [Fused Python SDK](/guide/advanced-setup/local-installation):
...
Will prioritize the current MAP VIEWPORT bounds, regardless of the [`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) parameter.
...
Will use the [`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) parameter, regardless of the current MAP VIEWPORT bounds.
...
A Tile UDF is called by passing [Web mercator XYZ tiles](https://en.wikipedia.org/wiki/Tiled_web_map) through the [HTTPS request](/guide/data-input-outputs/export-api/tokens-endpoints)
...
```
Replace `fc_<CANVAS_TOKEN>` and `my_udf` with your [canvas token and UDF name](/guide/data-input-outputs/export-api/tokens-endpoints). Use this template in mapping libraries; `{z}/{x}/{y}` is replaced with tile coordinates as users pan and zoom.

Example use cases:
- [QGIS](/guide/data-input-outputs/export-api/geospatial-export#qgis)
- [Felt](/guide/data-input-outputs/export-api/geospatial-export#felt)
- [DeckGL](/guide/data-input-outputs/export-api/geospatial-export#deckgl)
- [Mapbox](/guide/data-input-outputs/export-api/geospatial-export#mapbox)

**2. Specific tile call**
```bash
# Calls a single tile - San Francisco center at zoom 13
https://udf.ai/fc_<CANVAS_TOKEN>/my_udf/run/tiles/13/1316/3169?=png
```
...
- [Overture Maps Example](https://www.fused.io/workbench/udf/catalog/Overture_Maps_Example-64071fb8-2c96-4015-adb9-596c3bac6787)
    - Manipulate _all_ the buildings from the Overture Maps dataset by leveraging [Fused ingestion](/guide/data-input-outputs/read-write/geospatial/ingestion)
...
| Parameters | [`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) (Optional) | [`bounds`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#bounds) or `x`, `y`, `z` |
| Map rendering | Static | Dynamic |
| HTTP Call | Single Call | Tile Server calls or Single Call (specific tile) |
| [Runtime Tab](/workbench/udf-builder/runtime/) display | Unique output of the UDF called | Output of the _latest_ tile called |
...
This behaves the same as [`fused.types.Tile`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#legacy-fusedtypestile).
