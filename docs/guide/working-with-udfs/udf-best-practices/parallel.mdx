...
Each input spawns a separate [realtime UDF](/guide/working-with-udfs/udf-best-practices/realtime):
...
**Characteristics:**
- Each job has [realtime limits](/guide/working-with-udfs/udf-best-practices/realtime#realtime-limits) (120s, ~4GB RAM)
...
- Resources depend on `instance_type` chosen; see [Batch Jobs](/guide/working-with-udfs/udf-best-practices/batch-jobs#instance-types) for options
...
Use [non-blocking execution](/guide/working-with-udfs/udf-best-practices/parallel#non-blocking-execution) with `collect=False` and gather file paths after:
...
| `engine` | `local` or `remote` (see [Execution engines](/guide/working-with-udfs/udf-best-practices/realtime#execution-engines)) |
...
- 📈 [Making a Climate Dashboard](/examples/climate-dashboard) — Processing 20TB of data in minutes
- ⛴️ [Dark Vessel Detection](/examples/dark-vessel-detection#33---run-this-udf-over-a-month-of-ais-data) — Retrieving 30 days of AIS data
- 🛰️ [Satellite Imagery](/examples/satellite-imagery#preparing-fusedsubmit-to-run-in-parallel) — Processing Maxar's Open Data STAC Catalogs
