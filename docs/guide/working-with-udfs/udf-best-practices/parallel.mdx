...
This guide covers best practices for scaling parallel workloads. For the full `fused.submit()` reference, see [Run UDFs in parallel](/guide/working-with-udfs/fused-submit).
...
- Use `instance_type` for larger dedicated machines (see [How to run a batch job](/guide/working-with-udfs/udf-best-practices/batch-jobs))
...
See [How to run a batch job](/guide/working-with-udfs/udf-best-practices/batch-jobs) for details on batch instances.
...
- [Run UDFs in parallel](/guide/working-with-udfs/fused-submit) — full `fused.submit()` reference
