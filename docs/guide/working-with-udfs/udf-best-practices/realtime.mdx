---
id: realtime
title: How to run a Realtime job
sidebar_label: How to run a Realtime job
sidebar_position: 1
---

Realtime is the default way to run UDFs—no configuration needed. Every UDF runs in realtime mode unless you explicitly request a batch instance.

This guide covers best practices for calling UDFs from other UDFs and building pipelines. For the full `fused.run()` reference, see [Run UDFs in Python](/guide/working-with-udfs/run-udfs-in-python).

## Limits

| Resource | Limit |
|----------|-------|
| Execution time | 120s |
| RAM | ~4GB |

No startup time, instant execution. Need more? See [Batch Jobs](/guide/working-with-udfs/udf-best-practices/batch-jobs).

## Calling another UDF

Use `fused.run()` to call one UDF from another:

```python
@fused.udf
def parent_udf():
    # Call child UDF
    result = fused.run("child_udf", name="hello")
    return result
```

![Calling another UDF in workbench](/img/core-concepts/execution/fused_run_simple_demo.png)

In Workbench, `fused.run("udf_name")` creates a visual link between parent and child UDFs, making pipelines easy to follow.

## Best practices

### Keep child UDFs fresh

By default, child UDF results are cached. To always get fresh data from a child UDF:

```python
# In parent UDF
result = fused.run("child_udf", cache_max_age="0s")
```

:::warning
Setting `cache_max_age="0s"` means the child UDF runs from scratch every time—no caching. Use this when the child's output depends on frequently changing data.
:::

### Pin to commit hash for production

When calling UDFs from GitHub, pin to a specific commit:

```python
commit_hash = "bdfb4d0"
udf = fused.load(f"https://github.com/fusedio/udfs/tree/{commit_hash}/public/My_UDF/")
fused.run(udf)
```

Avoid pointing to `main` branch—your UDF will break when others push changes.

## When to scale up

| Need | Solution |
|------|----------|
| Run same UDF over multiple inputs | [How to run in parallel](/guide/working-with-udfs/udf-best-practices/parallel) |
| More than 120s or ~4GB | [Batch Jobs](/guide/working-with-udfs/udf-best-practices/batch-jobs) |

## See also

- [Run UDFs in Python](/guide/working-with-udfs/run-udfs-in-python) — full `fused.run()` reference
- [Tokens & Endpoints](/guide/data-input-outputs/export-api/tokens-endpoints) — call UDFs via HTTP
