...
Never put passwords or API keys in UDF source. Add them in [secrets management](/guide/advanced-setup/secrets-management), then read them at runtime (for Snowflake, set names like `SNOWFLAKE_USER` and `SNOWFLAKE_PASSWORD` there—see the [Snowflake integration guide](/workbench/integrations/snowflake)).
