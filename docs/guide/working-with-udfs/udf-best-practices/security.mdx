...
## Canvas access and shared tokens {#scope-access-with-canvas}

How **public** and **team-only** canvas sharing work, when to split UDFs across canvases, and how **session tokens** (Workbench or service account) apply to UDF URLs are documented in [Securing shared tokens](/guide/data-input-outputs/export-api/securing-shared-tokens).
...
Canvas passcode is only offered when the canvas is set to **public** sharing, not **team-only** (see [Securing shared tokens](/guide/data-input-outputs/export-api/securing-shared-tokens)).
...
- [Securing shared tokens](/guide/data-input-outputs/export-api/securing-shared-tokens) — public vs team-only, session tokens, service accounts
...
- [Encrypted S3 Bucket](/guide/advanced-setup/encrypted-s3-bucket) — connect SSE-KMS encrypted buckets
