...
**Do: load secrets from Secret Management**
...
1. **Values (IDs, filters, file paths, …)** — Don’t drop them into the SQL with an f-string or `+`. Write `?` where each value goes, then pass the real values in a list: `con.execute("... WHERE x = ?", [value])`. DuckDB treats those as **plain data**, not as extra SQL.
