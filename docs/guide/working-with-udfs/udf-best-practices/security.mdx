...
To grant temporary access to all UDFs on a canvas without sharing a permanent link, use [`fused.api.session_token()`](/python-sdk/api-reference/api#session_token) from within the Fused workbench. It returns a short-lived token (default `"1h"`) that authorizes calls to any UDF on that canvas.

Pass the token as a `fused_session_token` query parameter on any UDF HTTPS call from that canvas:
