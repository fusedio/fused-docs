...
3. Generate a **shared team link** — a permanent canvas URL scoped to your org, accessible by anyone on the team
...
## Step 3: Generate a shared team link

By default a canvas belongs to the user who created it. Generating a share token moves ownership to the org — the canvas URL is then tied to your team, not an individual.

Go to the **Team** section on the Fused home page, hover over the canvas, and click **Generate share token** (step 1 below). Fused opens a PR on your linked repo that adds a `shared_token.json` file. Once a maintainer merges it, hover over the canvas again and click **Open share link** (step 2 below).

<ClickZoomImage src="/img/working-as-a-team/generate_share_token_steps.png" alt="Generate share token" />
