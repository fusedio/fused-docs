...
UDFs can import functions from other UDFs with `fused.load` in the [UDFs GitHub repo](https://github.com/fusedio/udfs/tree/main) or [private GitHub repos](/guide/advanced-setup/git-integration/). Here the commit SHA `05ba2ab` pins the UDF to specific commit for version control (see [ways to reference a UDF](/guide/working-with-udfs/execution/realtime#ways-to-reference-a-udf)).
