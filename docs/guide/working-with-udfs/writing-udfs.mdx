...
The UDF Builder runs the UDF as a [Map Tile](/guide/working-with-udfs/geospatial/single-vs-tile) if the [first parameter](/guide/working-with-udfs/geospatial/single-vs-tile#the-bounds-object) is typed as `fused.types.Bounds`.
