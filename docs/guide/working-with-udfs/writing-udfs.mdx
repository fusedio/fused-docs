...
Use the [@fused.cache](/guide/working-with-udfs/caching/) decorator to persist a function's output across runs so UDFs run faster.
...
This example shows the [`bounds` parameter](/guide/working-with-udfs/geospatial/single-vs-tile#bounds) typed as `fused.types.Bounds`
...
To write UDFs that run successfully as both [`File` and `Tile`](/guide/working-with-udfs/geospatial/single-vs-tile), set `bounds` as the first parameter, with `None` as its default value. This enables the UDF to be invoked successfully both as `File` (when `bounds` isn't passed) and as `Tile`. For example:
...
| `fused.types.TileXYZ` ([Legacy](/guide/working-with-udfs/geospatial/single-vs-tile/#legacy-types)) | Mercantile tile coordinates | Bbox array or GeoJSON |
| `fused.types.TileGDF` ([Legacy](/guide/working-with-udfs/geospatial/single-vs-tile/#legacy-types)) | GeoDataFrame with x/y/z tile columns | Bbox array or GeoJSON |
| `fused.types.ViewportGDF` ([Legacy](/guide/working-with-udfs/geospatial/single-vs-tile/#legacy-types)) | GeoDataFrame for viewport (no x/y/z) | Bbox array or GeoJSON |

The UDF Builder runs the UDF as a [Map Tile](/guide/working-with-udfs/geospatial/single-vs-tile) if the [first parameter](/guide/working-with-udfs/geospatial/single-vs-tile#bounds) is typed as `fused.types.Bounds`.
...
When running a UDF with `fused.run`, it's possible to specify the [map tile](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf) Fused will use to structure the `bounds` object by using the following reserved parameters.
...
UDFs can import functions from other UDFs with `fused.load` in the [UDFs GitHub repo](https://github.com/fusedio/udfs/tree/main) or [private GitHub repos](/guide/advanced-setup/git-integration/). Here the commit SHA `05ba2ab` pins the UDF to specific commit [for version control](/guide/working-with-udfs/execution/realtime#git-commit-hash-recommended-for-most-stable-use-cases).
...
UDFs can return the following types of objects. Fused will try to convert the returned object to the [requested file format]([Dynamic File API](/guide/data-input-outputs/export-api/dynamic-file-api#output-formats)).
...
- `numpy.ndarray`, `xarray.Dataset`, `xarray.DataArray`
  - Arrays must be 2D or higher, 1D arrays are not supported.
  - Fused Workbench only supports rendering of `uint8` arrays
  - Rasters without spatial metadata should indicate their [tile bounds](/guide/working-with-udfs/geospatial/single-vs-tile#tile-udf)
...
This will allow you to access your UDF using a [token](/guide/working-with-udfs/execution/realtime#token), from a [Github commit](/guide/working-with-udfs/execution/realtime#git-commit-hash-recommended-for-most-stable-use-cases) or directly importing it in [Workbench from the Github URL](/workbench/udf-explorer/#add-from-github-url)
