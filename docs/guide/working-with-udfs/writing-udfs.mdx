...
```
:::info
The Map Builder in Workbench imports the `fused` module automatically. To write UDFs outside of Workbench, install the [Fused Python SDK](/guide/advanced-setup/local-installation) with `pip install fused` and import it with `import fused`.
:::
...
UDFs exported from the [Map Builder](/workbench/udf-builder/) or saved locally are formatted as a `.zip` file containing associated files with the UDFs code, `utils` Module, metadata, and `README.md`.
...
A common approach to debug UDFs is to show intermediate results in the [Map Builder](/workbench/udf-builder) [runtime panel](/workbench/udf-builder/runtime/) with `print` statements.
