...
The core principle behind Fused are Python functions that can be run via HTTP requests, called **User Defined Functions ([UDFs](/core-concepts/why/))**.
...
_We're going to create a simple User Defined Function and return it's results in a Google Sheets. Feel free to follow along by [first making sure you can login](/user-guide/login)._

**User Defined Functions** are Python functions that run on the Fused backend with a HTTP endpoint. 

Here's an example, we're going to load New York City metro station, taken from a GeoJSON directly hosted on Github [from the `folium-example-data` public repo](https://github.com/python-visualization/folium-example-data):
...
All you need to make a Python function a Fused User Defined Function is to add the `@fused.udf()` decorator to it.
...
Fused Workbench runs your UDF in real-time. You don't need to execute your code, it's always running, you can see it in the screen shot above:
1. Our UDF
...
Here's how to share our `GeoDataFrame` from Workbench into Google Sheets:
...
- Our [quick start example that walks you through a full end-to-end data science example](/user-guide/quickstart)
- Get to know what [UDF, User Defined Functions, are in more depth](/core-concepts/why)
- Become more familiar with Workbench, our browser based, no install required, IDE.
