...
api = FusedAPI()
token = api.create_udf_access_token('coffee_narwhal') # What is `coffee_narwhal`? Where does this come from?
url = token.get_tile_url() # I need to test this out to see if this actually works. Need to opena  new VS code project I think
print(url)
