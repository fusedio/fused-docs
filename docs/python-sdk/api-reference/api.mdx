...
- **path** (`str`) – Directory or file to delete, like `s3://fused-user/fused/aman/my-old-table/`
...
```python
fused.api.delete("s3://fused-user/fused/aman/deprecated_table/")  # change to your own path
```
...
- **path** (`str`) – URL to a file, like `s3://fused-user/fused/aman/file.parquet`
...
```python
fused.api.get("s3://fused-user/fused/aman/file.parquet")  # change to your own path
```
...
- **path** (`str`) – Parent directory URL, like `s3://fused-user/fused/aman/`
...
```python
fused.api.list("s3://fused-user/fused/aman/")  # change to your own path
```
...
Resolve a path in your home directory to the full S3 URI.

**Parameters:**

- **path** (`str`) – The path to resolve within your home directory (for example, `my-data/output.parquet`).
...
```python
s3_path = fused.api.resolve("my-data/output.parquet")
# Returns: "s3://fused-user/fused/aman/my-data/output.parquet"
```
...
```python
fused.api.sign_url("s3://fused-user/fused/aman/table_directory/file.parquet")  # change to your own path
```
...
- **path** (`str`) – URL to a prefix, like `s3://fused-user/fused/aman/some_directory/`
...
```python
fused.api.sign_url_prefix("s3://fused-user/fused/aman/table_directory/")  # change to your own path
```
...
- **remote_path** (`str`) – URL to upload to, like `s3://fused-user/fused/aman/new-file.txt`
...
```py
fused.api.upload("my_file.json", "s3://fused-user/fused/aman/my_bucket/my_file.json")  # change to your own path
```
