...
Optimized path: assumes the server provides full metadata (start_offset, end_offset, metadata, row_group_bytes) for all row groups and uses prefetched metadata from the `/datasets/items-with-metadata` endpoint, so no metadata API calls are made. If any row group is missing required metadata, raises an error indicating the dataset must be re-indexed.
...
- **max_concurrent_downloads** (`int | None`) – Max simultaneous downloads. If None, uses a default based on number of files.

**Returns:**

- `pa.Table` – PyArrow Table (or (table, timing_info) if `return_timing_info`).

**Raises:** `ValueError` if any row group is missing required metadata (dataset needs re-indexing).
...
Read data from an H3-indexed dataset by querying hex ranges (slower path with metadata API calls).

Queries the dataset index for matching row groups, then fetches metadata and downloads data. Adjacent row groups from the same file are combined into single downloads for better S3 performance. Batch size for combining row groups is controlled by `fused.options.row_group_batch_size` (default 32KB).
...
- **verbose** (`bool`) – If True, print progress. Default False.
- **return_timing_info** (`bool`) – If True, return (table, timing_info) instead of just the table.
- **metadata_batch_size** (`int`) – Max row group metadata requests to batch per API call. Default 50.
