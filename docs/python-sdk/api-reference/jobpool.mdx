...
Use `n_processes_per_worker` to batch multiple items per lambda invocation, reducing total invocations for large job counts:

```python
# Process 4 items in parallel within each lambda instance
