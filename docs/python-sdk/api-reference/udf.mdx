...
Calls the UDF, executing it with the given arguments and returning the result.

See [instance types](/guide/working-with-udfs/fused-run#instance_type) and [caching](/guide/working-with-udfs/udf-best-practices/caching).
...
@fused.udf
def main():
    assert my_udf.map([1, 2, 3, 4]).collect()["result"].tolist() == [1, 4, 9, 16]
```
...
Async version of `map()`. Batch instance types are not supported. See [caching](/guide/working-with-udfs/udf-best-practices/caching).
