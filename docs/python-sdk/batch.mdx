...
## 2. Run UDF [on an offline instance](/guide/getting-started/udf-best-practices/batch-jobs)

To go beyond the 120s limit of the default [`fused.run(udf)`](/guide/getting-started/udf-best-practices/realtime#fusedrun) call we'll define a job and use [`job.run_batch()`](/guide/getting-started/udf-best-practices/batch-jobs#advanced-run_batch-with-arg_list) to kick off a call on a large, offline instance.
...
`job_id` has a [number of methods to monitor the job](/guide/getting-started/udf-best-practices/batch-jobs#monitoring-jobs). For example `job_tail_logs` streams logs as the job runs.
