...
To go beyond the 120s limit of the default [`fused.run(udf)`](/guide/working-with-udfs/execution/realtime#fusedrun) call we'll define a job and use [`job.run_batch()`](/guide/working-with-udfs/execution/batch-jobs#running-a-large-job-jobrun_batch) to make kick off a call on a large, offline instance.
