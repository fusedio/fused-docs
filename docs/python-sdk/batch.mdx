...
To go beyond the 120s limit of the default [`fused.run(udf)`](/core-concepts/run-udfs/run-small-udfs/#fusedrun) call we'll define a job and use [`job.run_batch()`](/core-concepts/run-udfs/run_large/#running-a-large-job-jobrun_remote) to make kick off a call on a large, offline instance.
