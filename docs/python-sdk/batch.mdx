...
To go beyond the 120s limit of the default [`fused.run(udf)`](/guide/working-with-udfs/udf-best-practices/realtime#fusedrun) call we'll define a job and use [`job.run_batch()`](/guide/working-with-udfs/udf-best-practices/scaling-out#starting-a-batch-job) to kick off a call on a large, offline instance.
