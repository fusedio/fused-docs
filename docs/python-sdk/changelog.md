...
Breaking changes:
- Large (batch) jobs have been updated to pass parameters into the UDF the same way as other UDF runs. For compatabilty, if the parameters passed into the job do not correspond with the parameters, a dictionary parameter is passed into the UDF instead. This will be deprecated and removed in a future release.
...
- UDFs now support parameters annotated as `shapely.Geomtry` or `shapely.Polygon`.
...
- Many fixes to ensure compatabilty between the `fused` module available on PyPI and the `fused` module within the Fused backend.
