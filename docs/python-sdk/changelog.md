...
- `fused.submit` API has been improved.
- It is now possible set the cache storage location in `@fused.cache`.
- `@fused.cache` can now exclude arguments from the cache key.
- `@fused.cache` uses Pandas' own way of hashing DataFrames.
- Added storage argument to `fused.file_path`.
...
- `fused.load` can load by ID again.
...
- UDFs can be run by username and UDF name in addition to email and UDF name.
- Fixed a bug that could occur where Fused would try to duplicate index column names.
...
- The share page has been improved.
- Collections catalog page has been improved.
- When adding a duplicate UDF to Workbench, you will be prompted to duplicate or replace it.
- Memory usage for UDFs can be found in the results panel.
