...
**Improvements**
- UDFs won't execute when opening a Canvas in Workbench until the Canvas is saved
- Slow save: Workbench will show a warning instead of blocking modal when Canvas is saving
- Canvas: don’t auto-run on open (respects auto-run toggle)
- If Canvas is shared as team, UDF will show a warning that that Share Link won't open in a new tab
- Canvas renaming/saving improvements; optimistic name on conflict during upload
...
**Bug fixes**
- Canvas save state and save button (including zip upload) state corrected
- Opening Version page no longer triggers UDF reruns
- Overlay no longer hides on filter change
- Checkpoint diff ignores README-only changes
- Node toolbar (ghost bug) and capture preview for canvas-rendered graphs fixed

**Upgrades**
- DuckDB reverted to v1.4.3 due to no CRS geometry issue in v1.5.0
