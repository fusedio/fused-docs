...
**New Canvas wide shared**

UDFs now don't have individual access tokens, instead they are shared at the canvas level:

```python showLineNumbers
https://udf.ai/fc_MY_CANVAS_TOKEN/my_udf_name.parquet
```

Previous `fsh_` tokens are still supported, but are now deprecated & backwards compatible

**Finer AI control in Canvas**

- AI chat: stop button and running indicator
- Blocking modal when canvas is unsaved for a while
- Configure autorun per canvas
...
- [Experimental] Improved PR creation from checkpoint 
- [Experimental] Widgets: enhanced logging; text editing in studio nodes

**Bug fixes**
- UDFs will not auto-run on canvas reload/refresh
- UDF execution modal only when manually triggered
- Team-scoped canvas tokens will run as expected
- Hidden UDFs will not be executed when the canvas is loaded or edited
- A blocking modal will now show when the canvas is unsaved for a while prompting to download or checkpoint until save works
