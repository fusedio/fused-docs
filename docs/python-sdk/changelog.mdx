...
**Embedded Workbench for fused-render**

Canvases opened from fused-render now embed a fully authenticated Workbench editor directly in the app instead of a separate browser tab — with autosave, sharing, checkpoints, and the file explorer all working inside the embedded view. A read-only lock keeps the editor from clobbering changes while fused-render's sync is writing to the canvas.

**Improvements**

- Embed mode: AI buttons, side panels, and extra toolbar controls are hidden for a focused, minimal view

**Bug fixes**

- Fixed: `fused canvas pull` no longer treats the seeded `CLAUDE.md` and `.fusedignore` files as deletions or diffs
- Fixed: a crash on the UDF logs page
...
[Widgets](/guide/data-input-outputs/import-connection/widgets) gain a transformer component for custom JavaScript logic, per-widget background colors, an AI chat preset (beta), and schema linting with autocompletion in the JSON UI editor.

<img
  src="https://fused-magic.s3.us-west-2.amazonaws.com/changelog_assets/v2_7_1/2_7_1_widget.gif"
  alt="New widget components and capabilities in Fused"
  style={{ maxWidth: "800px", borderRadius: "12px", margin: "24px 0" }}
/>
...
- [Canvas](/workbench/udf-builder/canvas): AI suggestions are now scoped to the current canvas for more relevant results
- [Canvas](/workbench/udf-builder/canvas): canvas files can be saved and loaded as TOML for easier version control and sharing
- [Canvas](/workbench/udf-builder/canvas): node descriptions now appear as overlays directly on canvas nodes
- [Canvas](/workbench/udf-builder/canvas): stale canvas detection shows a diff before changes are overwritten
- [Canvas](/workbench/udf-builder/canvas): back and forward navigation buttons added to canvas history
- [Canvas](/workbench/udf-builder/canvas): parameter names now autocomplete in the JSON editor
- [Canvas](/workbench/udf-builder/canvas): AI chat conversations can be exported
- [Canvas](/workbench/udf-builder/canvas): create a pull request directly after saving a checkpoint
- [Widgets](/guide/data-input-outputs/import-connection/widgets): new transformer component runs custom JavaScript logic inside a widget
- [Widgets](/guide/data-input-outputs/import-connection/widgets): background color can be set per widget
- [Widgets](/guide/data-input-outputs/import-connection/widgets): JSON UI editor now validates against the schema with linting and autocompletion
- [Widgets](/guide/data-input-outputs/import-connection/widgets): new AI chat preset component (beta)
...
The [Canvas](/workbench/udf-builder/canvas) has been redesigned — new controls layout, collapsible code editor, node description overlays, a revamped edge toolbar, and refined typography throughout. The file explorer moves to the bottom bar and a new **Quick action** menu (Cmd+K) lets you search canvases, UDFs, and actions from anywhere.

<img
  src="https://fused-magic.s3.us-west-2.amazonaws.com/changelog_assets/v2.7.0/v_2_7_0.png"
  alt="Canvas redesign showing collapsible code editor, bottom file manager bar, and AI chat"
  style={{ maxWidth: "800px", borderRadius: "12px", margin: "24px 0" }}
/>

**Auto canvas checkpoints**

The [Canvas](/workbench/udf-builder/canvas) now automatically saves checkpoints as you work so you can restore any previous state without manually saving.
...
- [Canvas](/workbench/udf-builder/canvas): AI can auto-rename canvases based on their content
- [Canvas](/workbench/udf-builder/canvas): team-scoped shared canvases no longer require a passcode
- [Widgets](/guide/data-input-outputs/import-connection/widgets): widget builder now has separate editor and preview tabs
- [Widgets](/guide/data-input-outputs/import-connection/widgets): widget configs now support JSON5 format
- [Widgets](/guide/data-input-outputs/import-connection/widgets): config previews stream live while editing
- Logs can now be opened in a new browser tab for easier debugging
- `fused.cache` results now expose a `cached` attribute so you can tell whether a result came from cache
- UDFs can now return PIL Image objects for raster tile rendering
...
An AI assistant built into the [Widgets](/guide/data-input-outputs/import-connection/widgets) builder that lets you iterate on widget layout and logic through natural language.

<img
  src="https://fused-magic.s3.us-west-2.amazonaws.com/changelog_assets/v2.6.1/v2_6_1_widget_ai.gif"
  alt="Widget AI Chat demo"
  style={{ maxWidth: "600px", borderRadius: "12px", margin: "24px 0" }}
/>
...
- [Canvas](/workbench/udf-builder/canvas): HTML template nodes for embedding custom layouts
- Workbench: Batch job execution flow simplified
- Allow passing `None` values to UDFs regardless of type annotation

**Bug fixes**

- Fixed: non-numeric arrays incorrectly serialized as raster tiles
- Fixed: `to_fused` silently falling back to the default collection when none matched
- Fixed: Shift+Enter not triggering run for UDFs with manual run parameters
...
See the [Snowflake integration docs](/workbench/integrations/snowflake) to get started.
...
See the [Airtable integration docs](/workbench/integrations/airtable) to get started.

![Integrations panel on the Fused home page](/img/changelog/v2_6_0_integrations.png)

**Improvements**

- [Widgets](/guide/data-input-outputs/import-connection/widgets): now generally available — no longer behind a feature flag
- [Widgets](/guide/data-input-outputs/import-connection/widgets): AI auto-fix from the error log; copy/share button in the code editor
- [Canvas](/workbench/udf-builder/canvas): "Folders" renamed to "Sections"; stale indicator added; edge improvements
- [Canvas](/workbench/udf-builder/canvas): "Generate MCP" button added; AI suggestions now use canvas context
- Clickable UDF names in the code editor — click to jump to the definition
- Home page UI polish
- Public [Canvas](/workbench/udf-builder/canvas) now support preview images
- [`fused.submit`](/python-sdk/api-reference/fused-submit) shows the first error before retrying; `delete_cache` renamed to `invalidate_cache`
...
See the [Slack integration docs](/workbench/integrations/slack) to get started.
...
<img
  src="/img/changelog/v2_4_2_home_page.jpeg"
  alt="New Fused home page showing recents, resources, and integrations"
  style={{ maxWidth: "600px", borderRadius: "12px", margin: "24px 0" }}
/>

**Collection shared tokens**

[Collections](/guide/data-input-outputs/export-api/tokens-endpoints) now support shared tokens, making it easier to share data access without exposing credentials.

**Improvements**

- [Canvas](/workbench/udf-builder/canvas): sharing is now available for paid plans only; canvases can now be deleted directly from GitHub
...
- Fixed: [Canvas](/workbench/udf-builder/canvas): runtime loading bug on canvas open
...
- [Experimental] [Widgets](/guide/data-input-outputs/import-connection/widgets): URL query params now sync with param broadcasts in shared Widget mode
- [Experimental] [Widgets](/guide/data-input-outputs/import-connection/widgets): improved loading style when a UDF is running inside a Widget
...
- Fixed: [Experimental] [Widgets](/guide/data-input-outputs/import-connection/widgets): dropdown not rendering in fullscreen mode
...
<div style={{textAlign: "center", marginBottom: "20px"}}>
  <LazyReactPlayer
    ...
    url="https://fused-magic.s3.us-west-2.amazonaws.com/changelog_assets/v2_4_0/CleanShot%202026-04-09%20at%2010.48.34.mp4"
  />
</div>
...
- Fixed: [widgets](/guide/data-input-outputs/import-connection/widgets) on shared canvases get correct catalog/collection context
...
<img
  src="/img/changelog/v2_3_0_home_page.png"
  alt="Canvases home page with search, sort, and canvas cards"
  style={{ maxWidth: "600px", borderRadius: "12px", margin: "24px 0" }}
/>

**Improvements**

- [Experimental] Session tokens: control access to shared UDFs and canvases similarly to a presigned S3 URL; `fused-py` supports the same flow
- [Experimental] Widgets: H3 hex picker preset. Improved defaults & SQL editor
...
- Shared canvases run UDFs reliably again; `fused-py` shared-canvas tokens behave correctly
- Shared canvas works in Safari
- Basic tier: Improved realtime caching & secrets UI
- `fused.ingest()` accepts point layers with float32 columns
- Collection name is not saved into metadata by default unless needed
...
- `fc_****` prefix for canvas tokens
- `my_udf_name` for any UDFs in the canvas
- `.parquet` extension for Parquet files
...
This will require you to share your canvas first. Read more about the new canvas tokens in the updated [UDF as API](/guide/working-with-udfs/run-udfs-as-api) guide.
...
- You can now duplicate UDFs & Sticky notes with `Cmd + D` or `Ctrl + D`
- Checkpoint modal: better folder name validation
...
**🗒️ Sticky notes now support markdown!**

Bullet points, italic, links, you can now do all of that in sticky notes!

**Breaking Changes**
- Removed "App Builder" from deprecated features. Fused Apps can still be toggled on under the "Experimental" tab in Preferences. This allows you to still build Fused Apps directly in canvas.

**New Features**
- Paste images directly in Canvas! 
- AI can now create Canvas checkpoints.
- Download zip button for shared apps.
- Canvas preview images.
- Sticky notes: color picker.
- GPT 5.3 Codex model available in AI chat.
- Changelog modal now shown in Workbench on new deployments.

**Improvements**
- Experimental: "JSON UI" renamed to "Widget" across the product.
- Smarter detection of batch jobs & parallel runs in Workbench.
- Batch & Parallel runs will not show a modal anymore when first opening a Canvas, instead simply be paused.
- Canvas: improved helper lines with equal spacing & multi-selection support, outline node capture.
- AI: auto compaction, improved error handling, streaming switched from WebSocket to SSE.
- Default UDF runs are now `verbose=False`.
- Fused tokens highlighted as primary in editor.

**Bug Fixes**
- App sharing, sidebar filtering, and UDF copying from shared canvases.
- Python SDK: warning when ignoring `n_processes_per_worker` for local engine.
- Various AI, Canvas & UDF bug fixes.


## v2.0.4 (2026-02-18)

**New Features**
- [Checkpoints](/workbench/canvas-checkpoints) can now be named! 

**Improvements**
- Canvas catalog now loads faster.
- Canvas embed mode: JSON UI dropdowns fixed.
- UDF argument type can be specified as strings (e.g. `"fused.types.Bounds"`).
- Canvas general improvements.
- Versions page cleanup.

**Bug Fixes**
- [Experimental] Prevent dropdown reset in JSON UI in share mode.
- Map no longer gets blocked in certain cases.
- Memory usage now shown properly in logs.
- Copy UDF when adding from Command Palette.
- Modal deduplication fixes.
- Prevent "not logged in" flash while fetching access tokens.
- Geometry now renders correctly in tables.
- Catalog no longer appears on share page.
- Canvas download without state fixed.
- Batch job instance type now passed correctly from client.

## v2.0.3 (2026-02-11)

**New Features**

- Directly ask the AI to search for any code across all your UDFs! (RAG over user UDFs and UDF indexer).
- Support for Opus 4.6 in AI chat.
- Canvas history for UDF renaming with undo/redo support.
- Confirmation before deleting UDFs that have batch jobs or a map.

**Improvements**
- Better canvas auto-saving.
- Preferences modal, canvas centering (like Fused Maps), and catalog background behavior.
- Canvas memory optimization and modal display fixes.
- AI: MCP adjustments, and RAG/chat touchups.

**Bug Fixes**
- Delete UDFs via autosave instead of individual deletes.
- Re-opening canvas no longer makes team UDFs read-only.
- H3 ingest: fallback logic to large instance.
- Prevent infinite render loop in AI chat; fix chat history bug.
- File explorer: tooltips display correctly.
- Default collection can be created during partial sync.

## v2.0.2 (2026-02-05)

**New Features**
- AI: Chat history button to jump to previous chats.

**Improvements**
- Checkpoint actions disabled while auto-saving.
- Fewer unnecessary network requests.

**Changes**
- UDF Explorer and File Explorer removed from Canvas center sticky bar (still accessible in top left above UDF list).

**Bug Fixes**
- Canvas: UDF list scroll, app catalog tab switching, and general improvements.
- Table widget: copying selected text no longer copies the whole UDF.
- AI: Profile modal and chat text selection.
- Various AI and Canvas bug fixes.

## v2.0.1 (2026-02-03)

Fast follow release to fix some issues with the new Canvas-first architecture.

**Changes**
- `@fused.udf(instance_type=...)` deprecated in favor of `@fused.udf(engine=...)`. The `engine` parameter accepts `'local'`, `'remote'`, `'realtime'`, or batch instance types (`'small'`, `'medium'`, `'large'`, `'m5.large'`, etc.).
- Removed File Explorer and UDF Explorer from the Canvas lower sticky bar.
- `JobPool.df()` renamed to `JobPool.logs_df()`. New `df()` is an alias for `collect()`.

**Bug Fixes**
- Fixed checkpoint restore layout and modal UI.
- Fixed Canvas flickering and viewport change issues.
- Fixed multi-select rerun behavior.
- History size now configurable in Preferences.
- Various AI and Canvas bug fixes.

## v2.0.0 (2026-02-02)

This is a major release introducing a new Canvas-first architecture. UDFs now live inside Canvases, with a simplified Workbench layout and updated `fused-py` API.

**Breaking Changes**
- UDFs now belong to a Canvas. UDFs calling `fused.load()` or `fused.run()` to UDFs outside the current Canvas will fail. See [Migration](#migration) below.
- `fused.run(udf, var=1)` is now `my_udf(var=1)`.
- `fused.submit(my_udf, arg_list)` is now `my_udf.map(arg_list)`.

**New Features**
- **Canvas-first architecture:** Every UDF now lives inside a Canvas. Think of Canvases as folders and UDFs as files.
- **Auto-save:** UDFs save automatically as you edit. No more manual saves.
- **Checkpoints:** Save snapshots of your Canvas state and restore them anytime.
- **Undo support:** Accidentally delete a UDF? Press `Cmd+Z` (`Ctrl+Z` on Windows/Linux) to bring it back.
- **Copy/Paste UDFs:** `Cmd+C` any UDF and `Cmd+V` to paste it into any Canvas.
- **Multi-tab Workbench:** Open multiple Canvases in separate browser tabs.

**Improvements**
- Unified Workbench layout: Canvas, Map, and Code Editor now share a single view.
- Simplified navigation: Jobs, Tokens, and Preferences moved under Account (top right).
- Canvas catalog now accessible from the Fused logo (top left).
- Sharing a Canvas automatically embeds your current view.
- AI now thinks before implementing inline changes.

**Deprecations**
- App Builder is deprecated. Still accessible under Account → Preferences → Deprecated.

### Migration

**What happens to my existing UDFs?**
- UDFs already in a Canvas → No change.
- Standalone UDFs → Moved to a `default` Canvas.
- UDFs in multiple Canvases → Each Canvas now has its own independent copy.

**Fixing cross-Canvas UDF calls:**
...
- Added [`fused.h3.run_ingest_raster_to_h3`](/python-sdk/api-reference/h3#run_ingest_raster_to_h3) allowing for raster ingestion into H3 cells.
...
[Batch jobs](/guide/working-with-udfs/udf-best-practices/scaling-out#dedicated-instances):
...
- Added a dedicated docs page with [examples of Canvas](/workbench/udf-explorer)
...
**Improvements:**
- [Shared tokens](/guide/data-input-outputs/export-api/tokens-endpoints) will now use `udf.ai`.
- [Shared token](/guide/data-input-outputs/export-api/tokens-endpoints) URLs now default to HTML when no format is specified.
- UDF list in canvas shows the UDF status.
- Results panel will no longer pop-up in canvas.
- Workbench's [UDF Explorer](https://www.fused.io/workbench/catalog) will now open quicker with cached UDFs.
...
Learn how to connect your own data or analysis [here](/guide/data-input-outputs/import-connection/ai-data-connection)

- Experimental: "MCP Creator" AI profile to simplify creation of MCP servers from UDFs. [Read more](/guide/data-input-outputs/import-connection/ai-data-connection).
...
**[`fused-py`](/guide/advanced-setup/local-installation)**
...
Breaking changes:
- [Large (batch) jobs](/guide/working-with-udfs/udf-best-practices/scaling-out#dedicated-instances) have been updated to pass parameters into the UDF the same way as other UDF runs. For compatibility, if the parameters passed into the job do not correspond with the parameters, a dictionary parameter is passed into the UDF instead. This will be deprecated and removed in a future release.
- The `context` and [`bbox`](/guide/working-with-udfs/udf-best-practices/geospatial-single-vs-tile#legacy-bbox-object) parameters to a UDF are no longer treated as special.
...
- [`fused.load`](/python-sdk/api-reference/fused-load) now support loading UDFs from Github Pull Request URLs.
...
- Fixed [`@fused.cache`](/guide/working-with-udfs/udf-best-practices/caching#fusedcache) in [Fused Apps](/workbench/app-builder/).
...
- Fixed calling `fused.run` in [large jobs](/guide/working-with-udfs/udf-best-practices/scaling-out#dedicated-instances).
...
- Introducing [`fused.submit()`](/python-sdk/api-reference/fused-submit) method for multiple job run
- Improvement to [UDF caching](/guide/working-with-udfs/udf-best-practices/caching#udf-cache)
    - All UDFs are now cached for 90 days by default
    - Ability to customize the age of cached data & UDFs with the new [`cache_max_age`](/guide/working-with-udfs/udf-best-practices/caching#cache_max_age-reference) argument when defining UDFs, running UDFs or when caching regular Python functions
...
- Added a Delete Button in the Shared Tokens page (under [Profile page](/workbench/profile))
