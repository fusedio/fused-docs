...
[Batch jobs](/guide/working-with-udfs/udf-best-practices/batch-jobs):
...
Breaking changes:
- [Large (batch) jobs](/guide/working-with-udfs/udf-best-practices/batch-jobs) have been updated to pass parameters into the UDF the same way as other UDF runs. For compatibility, if the parameters passed into the job do not correspond with the parameters, a dictionary parameter is passed into the UDF instead. This will be deprecated and removed in a future release.
...
- Fixed [`@fused.cache`](/guide/working-with-udfs/udf-best-practices/caching/#caching-any-python-function-fusedcache) in [Fused Apps](/workbench/app-builder/).
...
- Fixed calling `fused.run` in [large jobs](/guide/working-with-udfs/udf-best-practices/batch-jobs).
...
- Improvement to [UDF caching](/guide/working-with-udfs/udf-best-practices/caching/#caching-a-udf)
    - All UDFs are now cached for 90 days by default
    - Ability to customize the age of cached data & UDFs with the new [`cache_max_age`](/guide/working-with-udfs/udf-best-practices/caching/#defining-your-cache-lifetime-cache_max_age) argument when defining UDFs, running UDFs or when caching regular Python functions
