...
- [Experimental] Session tokens: control access to shared UDFs and canvases similarly to a presigned S3 URL; supported in Fused-py as well
- [Experimental] Widgets: H3 hex picker preset, defaults, and SQL editor improvements
- Clone a canvas from the UI
...
- Sharing is smoother: better share modal, multi-select when sharing, and shared links open zoomed to the bounds you had selected when clicking "Share"
- Overall speed improvements across canvas, catalog, DataTable, and UDF explorer
- Basic tier: external monitoring aligned with usage

**Bug fixes**

- Shared canvases run UDFs reliably again; fused-py shared-canvas tokens behave correctly
- Shared canvas fixed in Safari
- Basic tier: realtime cache wiring, secrets UI for basic users, Workbench loads if checkout is canceled mid-flow
- `fused.ingest()` accepts point layers with float32 columns
- Lower memory use in the Workbench client
- Collection name is not saved into metadata by default unless needed
- UDFs no longer need an extra click to run in some cases; fewer stray reruns from log-only updates
