...
- [Canvas](/workbench/udf-builder/canvas): sharing options now adapt based on execution environment tier; default canvases no longer generate a share token automatically; canvases can now be deleted directly from GitHub
- [H3 analytics](/guide/h3-analytics/overview): `run_partition` now supports non-count metrics
...
- Fixed: [Experimental] [Widgets](/guide/data-input-outputs/import-connection/widgets): `iframe` loading fallback not triggering correctly
