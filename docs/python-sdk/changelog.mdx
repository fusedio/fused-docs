...
- Improved clone canvas flow; resize several nodes at once; parameters track origin and conflicting updates; undo/redo restores node data; clearer save state (including accidental close when out of sync); edges wait until parents finish saving
- Share URL opens zoomed to the shared viewport; share modal and multi-select sharing; checkpoint modal UX; execution timer improvements; canvas, catalog, and DataTable performance; canvas name in the window title; logo in share view goes home; fullscreen and share-view fixes; outline/sticky-note and duplicate-name behavior; edge handle and pellet tweaks; UDF count filter on catalog and home; selection toolbar alignment and layer order; open a collection in a new tab; edges in share view without delete controls; sticky notes read-only in share view; fewer stray runs when only logs update
- UDF explorer opens faster
- Basic tier: external monitoring for usage signals
- `geoai-py` added to the environment
- [Experimental] Widgets: H3 hex picker preset (hex icon), H3 defaults, SQL editor resolve, edge animation and lock-mode selection fixes

**Bug fixes**

- Shared canvases: UDFs auto-run as expected; database `collection_name` respected for shared-token requests; fused-py shared-canvas token handling corrected
- Shared Canvas works in Safari
- Basic tier: cache scheme passed correctly to realtime; secrets table error for basic users; Workbench loads if checkout is canceled
- Ingest: point datasets with float32 columns ingest correctly
- Session and public tokens: clearer error on invalid session token; fewer public UDF token clashes
- Metadata: collection name is not saved by default unless needed
- UDFs that needed an extra click to run; child UDFs not fired only from polling log updates; saving and catalog edge cases

**Upgrades**

- stlite v1.5.0
- Lower memory pressure in the Workbench client
