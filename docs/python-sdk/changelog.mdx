...
**Canvas sharing enhancements**

Canvas sharing options now adapt based on the execution environment tier. Default canvases no longer generate a share token automatically. You can also delete an existing canvas directly from GitHub.

**iFrame component in JSON-UI**

A new `iframe` component lets you embed any URL directly inside a JSON-UI render widget.

**Export chat history**

Chat conversations can now be exported from the chat history sidebar.

**New home page**

A redesigned home page with a landing experience, quick-access menu, recents section, and live integration status.

**H3 analytics: non-count metrics in `run_partition`**

`run_partition` now supports metrics beyond simple counts when using the H3 analytics ingest workflow.
...
- Batch jobs upgraded to Amazon Linux 2023
- Blocking confirmation modals replaced with inline gate UI
- UDF result state is now managed with per-UDF version atoms for better isolation
- SQL query handling in DuckDB queries has been refactored for reliability
- NumPy args and DataFrame keys are now cached to reduce repeated computation
...
- UDF catalog detail management and file explorer routing improved
- Google Drive secrets and policies support added

**Bug fixes**

- Fixed: bar chart no longer re-animates on unrelated state changes
- Fixed: CodeMirror text selection inside JSON-UI render widgets
- Fixed: AI running badge overlapping the top bar
- Fixed: serialization error when DataFrame index and column names conflict
- Fixed: AI edit confirmation flow edge cases
- Fixed: recents card count on home page not updating after reload
- Fixed: integration statuses on home page now sync correctly
- Fixed: Firefox doc popup layering in the code editor
- Fixed: shared AI widgets now show a login CTA for unauthenticated users
- Fixed: iFrame loading fallback not triggering correctly
- Fixed: runtime loading bug on canvas open
- Fixed: two Kubernetes agents with the same ID conflicting
