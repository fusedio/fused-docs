...
- [`fused.submit`](/python-sdk/api-reference/submit) shows the first error before retrying; `delete_cache` renamed to `invalidate_cache`
...
- [`fused.load`](/python-sdk/api-reference/load) now support loading UDFs from Github Pull Request URLs.
...
- Introducing [`fused.submit()`](/python-sdk/api-reference/submit) method for multiple job run
