...
Breaking changes:
- [Large (batch) jobs](/guide/working-with-udfs/execution/batch-jobs) have been updated to pass parameters into the UDF the same way as other UDF runs. For compatibility, if the parameters passed into the job do not correspond with the parameters, a dictionary parameter is passed into the UDF instead. This will be deprecated and removed in a future release.
...
- Fixed calling `fused.run` in [large jobs](/guide/working-with-udfs/execution/batch-jobs).
