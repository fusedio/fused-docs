...
This function returns the zoom level at which a tile exists that, potentially shifted slightly, fully covers the bounding box. It supports multiple input types and handles various geospatial data formats.


**Arguments**:

- `bounds` _fused.types.Bounds_ - This is a list of 4 points representing the bounds (extent) of a geometry. The 4 points represent [xmin, ymin, xmax, ymax] of the bounds.


**Returns**:

- `int` - The estimated zoom level(0-20).
...
def udf(bounds: fused.types.Bounds):
		...
		z = fused.estimate_zoom(bounds)
		return z

z = fused.estimate_zoom(bounds)

fused.run(udf, bounds)
```
