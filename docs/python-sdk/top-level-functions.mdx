...
Returns: `The located file path.`

**Examples:**

When run locally, `file_path` returns the local `/tmp/fused/` directory on your machine. If called in a UDF is run on Fused servers, the returned path will be of the 
shared mounted storage [`/mount`](/core-concepts/content-management/file-system/#mntcache-disk). 
Because `file_path` on a batch job using [`job.run_remote()`](/core-concepts/run-udfs/run_large/#getting-results) returns the directory of the remote instance, it rec
ommended to write data directly to `/mount`.
