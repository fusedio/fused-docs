...
- **cache_reset** (<code>[bool](#bool) | None</code>) – Ignore `cache_max_age` and overwrite cached result.
...
- **cache_verbose** (<code>[bool](#bool) | None</code>) – Print a message when a cached result is returned
...
- **import_globals** (<code>[bool](#bool)</code>) – Expose the globals defined in the UDF's context as attributes on the UDF object (default True).
...
- **udf** (<code>([str](#str), [Udf](#fused.models.udf.Udf) or [UdfJobStepConfig](#fused.models.api.UdfJobStepConfig))</code>) – the UDF to execute.
...
- **file_path** (<code>[str](#str)</code>) – The relative file path to locate.
