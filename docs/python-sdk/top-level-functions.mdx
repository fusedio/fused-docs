...
submit(
    ...
    *,
    ...
    collect: bool = True,
    batch_type: BatchType = "thread_pool",
    cache_max_age: Optional[str] = None,
    cache: bool = True,
...
- **collect** (<code>bool</code>) – If True, waits for all jobs to complete and returns the collected DataFrame
  ...
  Default is True.
- **batch_type** (<code>BatchType</code>) – The type of batching to use. Either "thread_pool" (default) for
...
- <code>[Union](#typing.Union)\[[BaseJobPool](#fused._submit.BaseJobPool), [ResultType](#fused._run.ResultType), [DataFrame](#pandas.DataFrame)\]</code> – JobPool, AsyncJobPool, or DataFrame depending on batch_type and collect parameters
...
```py
df = fused.submit(udf, range(10), batch_type="async_loop")
```
