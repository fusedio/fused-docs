...
```python
monthly_mean = daily_mean_temp.groupby(pd.to_datetime(daily_mean_temp.index).strftime('%Y-%m'))['result'].mean()
monthly_mean_df = monthly_mean.reset_index()
monthly_mean_df.columns = ['month', 'mean_temp']
```

Saving to file:

{/* NOTE: Would need access to S3 to be able to do this. Do it in a UDF instead? */}

```python
monthly_mean_df.to_parquet("s3://fused-sample/demo_data/climate/monthly_mean_temp.parquet")
```

### Making your first chart

Fused allows allows you to make simple interactive charts & dashboard through a Canvas.

Let's load our data in a UDF first:

```python
@fused.udf
def udf(
    path: str = "s3://fused-sample/demo_data/climate/monthly_mean_temp.parquet"
):
    import pandas as pd
