...
Without having an environment you can still access [Workbench](/workbench/), [install](/python-sdk/#install) `fused` locally and [run Public UDFs](/core-concepts/run-udfs/run-small-udfs/#public-udf-name) (in Workbench & locally)
:::
...
<Tabs className="unique-tabs">
  ...
  <TabItem value="locally" label="Local">
    ...
    By default `fused.run()` run your UDFs allowing you do to write code locally and then immediately run it on our server with `fused.run()` without deploying anything!
...
We'll demo the rest of this section in Workbench but you can follow along in a notebook if you'd like to as well!

Workbench allows you to easily edit your code and it executes instantly without you having to hit "Run".
