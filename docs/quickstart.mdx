...
- [Create a Fused app](/quickstart/#create-an-app-beta)  
...
The UDF editor calls [`fused.run(udf)`](/python-sdk/top-level-functions/#run) upon updates to the code. 

  :::note

  A result of the code executing instantly is the function in the editor must be named `udf`.
...
- Take a deeper look at [Workbench](/workbench/overview/) so you can fully utilize the tools at your disposal
