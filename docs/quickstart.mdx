...
<Tabs className="unique-tabs">
  ...
  <TabItem value="local" label="Locally">
    ...
    By default [`fused.run()`](/python-sdk/top-level-functions/#run) run your UDFs allowing you do to write code locally and then immediately run it on our server with [`fused.run()`](/python-sdk/top-level-functions/#run) without deploying anything!
