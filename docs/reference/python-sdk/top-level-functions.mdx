...
- **instance_type** (<code>Optional[str]</code>) – The type of instance to use for remote execution ('realtime',
...
- <code>Callable[..., Udf]</code> – A callable that represents the transformed UDF.
...
- **func** (<code>Callable</code>) – The function to be decorated.
- **cache_max_age** (<code>str | int</code>) – Supported units: seconds (s), minutes (m), hours (h), days (d). E.g. "48h", "10s".
...
- **cache_storage** (<code>StorageStr | None</code>) – "auto", "mount" or "local".
- **cache_key_exclude** (<code>Iterable[str]</code>) – Parameter names to exclude from the cache key.
...
> 📖 **Guide:** See [Caching Guide](/core-concepts/cache/) for best practices.
...
- **url_or_udf** (<code>Union[str, Path]</code>) – GitHub URL, "email/udf_name", or local file path.
- **cache_key** (<code>Any</code>) – Optional caching key.
- **import_globals** (<code>bool</code>) – Expose globals as UDF attributes (default True).
...
- **udf** – UDF name, token, object, or config.
- **engine** – `"remote"` (default) or `"local"`.
- **instance_type** – `"realtime"`, `"small"`, `"medium"`, `"large"`.
- **cache_max_age** – Cache duration (e.g. "48h", "10s").
...
- **arg_list** – List of values or dicts for parametrization.
- **max_workers** – Maximum parallel workers (default 32).
- **debug_mode** – Run only first item for testing.
- **collect** – If True, wait and return DataFrame.
...
- **input** – GeoDataFrame or path(s) to files on S3.
- **output** – S3 location for output.
- **partitioning_method** – `"area"`, `"length"`, `"coords"`, or `"rows"`.
- **target_num_chunks** – Target number of output files.
...
> 📖 **Guide:** See [Data Ingestion](/tutorials/Geospatial%20with%20Fused/geospatial-data-ingestion/) for details.
