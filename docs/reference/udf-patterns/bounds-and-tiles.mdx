...
sidebar_position: 1
description: Understanding Single vs Tile UDF execution modes
---
...
A Single UDF is called like any other UDF via a [HTTPS request](/guide/running-udfs/http-endpoints):

```bash
https://www.fused.io/.../run/file?format=csv
```

Or via the [Fused Python SDK](/reference/python-sdk/):
...
Will prioritize the current MAP VIEWPORT bounds, regardless of the [`bounds`](#bounds) parameter.
...
Will use the [`bounds`](#bounds) parameter, regardless of the current MAP VIEWPORT bounds.
...
A Tile UDF is called by passing [Web mercator XYZ tiles](https://en.wikipedia.org/wiki/Tiled_web_map) through the [HTTPS request](/guide/running-udfs/http-endpoints)
...
Example use cases:
- [QGIS](/reference/udf-patterns/integrations#qgis)
- [Felt](/reference/udf-patterns/integrations#felt)
- [DeckGL](/reference/udf-patterns/integrations#deckgl)
...
- [Overture Maps Example](https://www.fused.io/workbench/udf/catalog/Overture_Maps_Example-64071fb8-2c96-4015-adb9-596c3bac6787)
    - Manipulate _all_ the buildings from the Overture Maps dataset by leveraging [Fused ingestion](/guide/geospatial-ingestion/)
...
| Parameters | [`bounds`](#bounds) (Optional) | [`bounds`](#bounds) or `x`, `y`, `z` |
| Map rendering | Static | Dynamic |
| HTTP Call | Single Call | Tile Server calls or Single Call (specific tile) |
| [Results Tab](/reference/workbench/udf-builder/results) display | Unique output of the UDF called | Output of the _latest_ tile called |
...
This behaves the same as [`fused.types.Tile`](#legacy-fusedtypestile).
