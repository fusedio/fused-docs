...
import SeeMoreImg from '/docs/tutorials/Data Science & AI/see_more.png';
...
df = fused.run(
    "https://unstable.fused.io/server/v1/realtime-shared/fsh_TlnNmpoPNYe0dDSx8LVnN/run/file?dtype_out_vector=csv",
...
This will internally create a common output schema for all the pages based on the query and then scrape the data from each page using a fused.submit call.
...
df = fused.run(
    ...
    query="Extract school names, ranks, and addresses for all NYC schools"
)
...
df = fused.run(
    ...
    extraction_prompt="Extract school names, ranks, and addresses"
)
