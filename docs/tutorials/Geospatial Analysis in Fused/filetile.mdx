...
<ReactPlayer playsinline={true} className="video__player" playing={false} muted={true} controls height="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/File_Tile_Viewport2.mp4" width="100%" />

We'll demonstrate the differences with a simple UDF that takes a [`bounds` object](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#the-bounds-object) as input and display how it behaves differently in each mode:
...
<ReactPlayer playsinline={true} className="video__player" playing={false} muted={true} controls height="100%" url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/Tile_bounds_udf.mp4" width="100%" />
...
This behaves the same as [`fused.types.Tile`](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#legacy-fusedtypestile).
...
This endpoint structure runs a UDF as a `Single`. See implementation examples with [Felt](/tutorials/others/out/felt/#vector) and [Google Sheets](/tutorials/others/out/googlesheets/#2-create-a-url-for-the-udf) for vector.
...
This endpoint structure runs a UDF as a `Tile`. The `{z}/{x}/{y}` templated path parameters correspond to the Tile's XYZ index, which Tiled web map clients dynamically populate. See implementation examples for Raster Tiles with [Felt](/tutorials/others/out/felt/#raster-tiles) and [DeckGL](/tutorials/others/out/deckgl/#raster-tile-layers), and for Vector Tiles with [DeckGL](/tutorials/others/out/deckgl/#vector-tile-layers) and [Mapbox](/tutorials/others/out/mapbox/#a-vector-tile-layers).
