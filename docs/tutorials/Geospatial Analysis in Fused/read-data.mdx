...
@fused.udf
def udf(
    bounds: fused.types.Bounds = [-122.5, 37.7, -122.3, 37.9],
    path: str = "s3://fused-sample/demo_data/satellite_cog.tif"
):
    import rioxarray as rxr
    
    # Read COG with spatial bounds
    raster = rxr.open_rasterio(path).rio.clip_box(*bounds)
    
    return raster.values, bounds
```
