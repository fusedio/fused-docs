...
ingesting a small DEM file for the New York City region, available at
`"s3://fused-asset/data/nyc_dem.tif"` (See in [File Explorer](https://www.fused.io/workbench/files?path=s3%3A%2F%2Ffused-asset%2Fdata%2Fnyc_dem.tif))
...
longer than the 2-min limit of realtime UDF runs.
...
def udf(
    ...
    # highlight-start
    utils = fused.load('Read_H3_dataset')
...
def udf(
    ...
    map_utils = fused.load("https://github.com/fusedio/udfs/tree/6dd2c4e/community/milind/map_utils")
