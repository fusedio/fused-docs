...
Default files (using res 7 - [example file](https://www.fused.io/workbench/files?path=s3%3A%2F%2Ffused-asset%2Fhex%2Fgeorgia_subset_4_counties_3dep_1m_res14_10_v3%2F604691848023244799.parquet)):
```
                   hex   data_avg                                         source_url  res
0  628243827645371391  71.423302  [s3://fused-asset/dem/3dep_tmn/georgia/USGS_1M...    7
1  628243827645387775  71.298765  [s3://fused-asset/dem/3dep_tmn/georgia/USGS_1M...    7
2  628243827645404159  71.156432  [s3://fused-asset/dem/3dep_tmn/georgia/USGS_1M...    7
3  628243827645420543  71.089234  [s3://fused-asset/dem/3dep_tmn/georgia/USGS_1M...    7
4  628243827645436927  71.193882  [s3://fused-asset/dem/3dep_tmn/georgia/USGS_1M...    7
```

[Overview](/tutorials/Geospatial%20with%20Fused/h3-tiling/ingesting-dataset-to-h3#overview-resolutions-overview_res) files (using res 9 - [example file](https://www.fused.io/workbench/files?path=s3%3A%2F%2Ffused-asset%2Fhex%2Fgeorgia_subset_4_counties_3dep_1m_res14_10_v3%2Foverview%2Fhex9.parquet)):
```
                   hex   data_avg        lat        lng
0  618202600000000000  71.259480  32.562230 -83.554810
1  618202600000000001  81.583760  32.550860 -83.554850
2  618202600000000002  75.432100  32.489230 -83.612340
3  618202600000000003  89.048190  31.786910 -84.338880
4  618202600000000004  89.011300  31.787690 -84.335540
```
...
we have `"data_avg"` (average height in the H3 cell), `"source_url"` (source raster files), and `"res"` (H3 resolution).
...
| 9                     |           105,333     |       201                   |       325                         |                                        |
| 10                    |            15,048     |        76                   |       123                         |                                        |
| 11                    |             2,150     |        29                   |        46                         | Landsat 8/9 (30m)                      |
| 12                    |             307.1     |      10.8                   |      17.5                         | Sentinel-2 (10m)                        |
| 13                    |              43.9     |       4.1                   |       6.6                         |                                        |
| 14                    |               6.3     |       1.5                   |       2.5                         | USGS 1m DEM                             |
| 15                    |               0.9     |       0.6                   |       0.9                         |                                        |
...
resolution of `overview_res - 5`, but this default can be overriden with the
`overview_chunk_res` keyword, or by specifying a `max_rows_per_chunk`.

### Ingesting multiple files

The [`run_ingest_raster_to_h3`](/python-sdk/api-reference/h3/#fusedh3run_ingest_raster_to_h3)
function takes a path to a single file, a path to a directory, or a list of file
paths as input (`src_path`). It can thus ingest a single raster file, or
multiple raster files at once.

When ingesting multiple files, each file is processed separately in the first
step of extracting the raster values and assigning to H3 cells. And then all
data is combined when calculating the metrics and repartitioning the data,
before writing to Parquet. Thus, this assumes that the raster files have no overlap.

For example, ingesting a subset of the Copernicus Digital Elevation Model (DEM) tiles:

```python showLineNumbers
@fused.udf
def udf():
    # ingesting Copernicus Digital Elevation Model (DEM) data
    # from https://copernicus-dem-90m.s3.amazonaws.com/readme.html
    # this dataset consists of 26k tiles, listing 4 here as example
    dem_listing = [
...
#### Running ingestion on GCS

Fused can run the ingestion fully on GCP [batch instances](/core-concepts/run-udfs/run_large), if they are available in your environment:
