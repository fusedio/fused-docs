...
Read more about [`fused.file_path()`](/python-sdk/top-level-functions/#fusedfile_path) and [mount](/core-concepts/content-management/file-system/#mntcache-disk)

Executing this from a notebook:
```python
# Running this in a notebook. No need for fused.run() in workbench
fused.run(udf)
```
