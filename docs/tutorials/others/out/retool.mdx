...
Follow this [guide](/tutorials/others/out/mapbox/) to create a mapbox `.html` map that renders vector tiles from a UDF.
...
This GeoJSON can be used in subsequent operations. For example, it could be passed as a parameter to downstream UDF calls as explained [here](/tutorials/others/out/http/#with-a-geojson).
