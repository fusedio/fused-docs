...
<Tabs className="unique-tabs">
  <TabItem value="udf-editor" label="UDF Editor" default>
    ...
    2. Edit the URL to return the data as a JSON by changing the `dtype_out_vector` from `html` to `json`
  ...
  <TabItem value="advanced-udf-editor" label="Advanced UDF Editor">
    ...
    3. Copy the HTTPS URL and change the `dtype_out_vector` from `html` to `json`
