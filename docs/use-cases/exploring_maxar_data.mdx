...
Unpacking this UDF:
- This UDF takes :
    - a [`bounds`](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#the-bounds-object) object. This allows us to pass the [current Workbench Map Viewport](/workbench/udf-builder/map/) to our UDF
...
- 2. [Tile / File modes](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/)
...
This change returns the image instead of the extent, but it returns the image all at once, because Workbench is set to ["File" mode](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#file-viewport) by default
...
We can change this by setting Workbench to ["Tile" mode](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#tile):
...
- Use [`fused.submit()`](/python-sdk/top-level-functions/#fusedsubmit) to run a [UDF in parallel](/core-concepts/run-udfs/run-small-udfs/#running-multiple-jobs-in-parallel)
- Use [`@fused.cache`](/python-sdk/top-level-functions/#fusedcache) to cache requests to reduce costs and improve performance
- Use Workbench to [display images](/workbench/udf-builder/map/) in [different modes](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/)
...
- Go more in depth with ["File" & "Tile" modes](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/) in Workbench
