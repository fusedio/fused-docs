...
You might receive a [timeout error](/core-concepts/run-udfs/run-small-udfs/#defining-small-job) in the `Results` tab. Try zooming into Utah on the map where the zonal areas are highlighted, to reduce the [size of the tile](/core-concepts/filetile/#tile) being passed into the UDF from the map.
