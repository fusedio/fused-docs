...
You can then run UDFs from _anywhere_ you have access to Python with [`fused.run(udf)`](/core-concepts/run-udfs/run-small-udfs/#fusedrun). These are still Python functions, giving you a lot of flexibility on what oyu can do, but we have some recommendations for keeping them fast & efficient.
...
⚡️ Aim for **UDFs that take 30s - 1min to run**
...
- Fused only supports a [few types](/core-concepts/write/#supported-types) at the moment. Any non-typed or unsupported types will be handled as `str`
