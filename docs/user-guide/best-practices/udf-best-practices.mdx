...
:::tip
Since [UDFs are cached by default](/core-concepts/cache/#caching-a-udf), using `fused.submit(udf, inputs, debug_mode=True)` means you won't have to run the 1st input again, as you'll have just run it! (unless `udf` is set with `cache_max_age=0`)
:::
