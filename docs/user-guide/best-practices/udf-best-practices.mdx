...
### Leverage [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#fusedsubmit): Run small tasks over large inputs

Sometimes you need to run a UDF that takes longer than 120s to run, either you need to load a large dataset or you need to run a more complex process.

In this case you can use [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#fusedsubmit) to run a small UDF in parallel. 

<details>
    <summary>Example: Running a large UDF with `fused.submit()`</summary>
    ...
    def fetch_data(inputs: list):
        ...
        for i in inputs:
            ...
            time.sleep(15)
            requested_data.append(f"processed_{str(i)}")

        return pd.DataFrame({"data": requested_data})
    ```
    However, running this UDF with `fused.run(fetch_data, input_data)` might timeout, i.e. go over the 120s limit. We still do want to fetch data across all our inputs which is where [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#fusedsubmit) comes in:
...
@fused.udf
def udf(
    bounds:fused.types.Tile=None, n:int=1
):
    ...
    return
```
