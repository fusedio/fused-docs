...
### Leverage [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#fusedsubmit) to run large UDFs
...
In this case you can use [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#fusedsubmit) to a small UDF in parallel. 

<details>
    <summary>Example: Running a large UDF with `fused.submit()`</summary>

    ```python showLineNumbers
    # Need to show example here of a large UDF
    ```
