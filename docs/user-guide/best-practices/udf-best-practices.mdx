...
<details>
    ...
    ```
    However, running this UDF with `fused.run(fetch_data, input_data)` might will take longer as we add inputs, we could even quickly go over [the 120s limit](/core-concepts/run-udfs/run-small-udfs/#defining-small-job). We still do want to fetch data across all our inputs which is where [`fused.submit()`](/core-concepts/run-udfs/run-small-udfs/#fusedsubmit) comes in:
