---
sidebar_label: "Querying Crops data with LLMs"
title: "Querying Crops data with LLMs"
tags: ['example', 'rag', 'embeddings', 'openai', 'vector-search']
sidebar_custom_props:
    name: "Querying Crops data with LLMs"
    image: 'https://fused-magic.s3.us-west-2.amazonaws.com/thumbnails/udfs-staging/Fused_Logo.png'
---

### Overview
This example demonstrates how to use Fused to create an end to end crop search system using Retrieval Augmented Generation (RAG) and the existing CDL udf. We'll leverage OpenAI's embedding model to generate embeddings for crop types and then use these embeddings to perform semantic searches.

### Requirements
- [Access to Fused](/user-guide/login/)
- OpenAI API key (stored in [Fused secrets](/workbench/preferences/#secrets-management))


In this example, we want to find out the most similar crops based on a natural language query. For instance, if you ask "show me all the red fruits", the system should return crops that are semantically similar to that description.
1. Get the crop data labels from USDA CDL
...
5. Filter the CDL data based on the query results

![Embedding Generation and Query Pipeline](/img/user-guide/examples/crop_search_rag/chart.png)
...
def udf():
    ...
    # Fetch and process crop data
    print("Fetching USDA crop data...")
    response = requests.get("https://storage.googleapis.com/earthengine-stac/catalog/USDA/USDA_NASS_CDL.json")
    crops = response.json()["summaries"]["eo:bands"][0]["gee:classes"]
    df = pd.DataFrame(crops)
    
    # Generate embeddings
    client = OpenAI(api_key=api_key)
    df = generate_crop_embeddings(client, df)
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                print(f"All attempts failed for text: {text[:50]}...")
                return [0.0] * 1536
...
def generate_crop_embeddings(client, df):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        df['embedding'] = list(executor.map(
            lambda row: get_embedding(client, row), 
            df['rich_description']
        ))
