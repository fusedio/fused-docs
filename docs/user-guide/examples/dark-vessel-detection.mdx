...
[`fused.file_path()`](/python-sdk/top-level-functions/#file_path) returns the mount path of any file we'd like to create. When run locally, it returns the local `/tmp/fused/` directory on your machine.
...
`debug_mode=True` runs the 1st value inside `date_ranges` with [`fused.run()`](/python-sdk/top-level-functions/#run). This allows you to make sure your UDF + arg_list combo is working properly. 
...
:::info
The code above demonstrates a basic User-Defined Function (UDF) that utilizes [Fused's utility functions](https://github.com/fusedio/udfs/blob/eeb22486da7b3a073371b278e3473c24438ec715/public/common/utils.py#L1891). These utilities provide a some general functions for geospatial operations and transformations. You can also define your own utils in the [utils](http://localhost:3000/workbench/udf-builder/code-editor/#utils-module) section of the workbench.
