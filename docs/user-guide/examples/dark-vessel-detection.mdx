...
We can then pass this directly to `fused.submit()`. Though we recommend you always do a first run with `debug_mode=True` to [test your submit job](/core-concepts/run-udfs/run-small-udfs/#debug-mode):
