...
<Tabs className="unique-tabs">
  ...
  <TabItem value="utils module" label="utils Module" default>
...
def udf(
 ...
 # Changed type check to hasattr
    print(f"Current zoom level: {bbox.z=}")
    ...
    if ais_df.shape[0] == 0:
        print("No AIS data within this bbox & timeframe. Change bounds or timeframe")
        return ais_df

    if hasattr(bbox, 'z'):  # Changed type check to hasattr
        if bbox.z > min_zoom_to_load_data:
            print(f"Zoom is {bbox.z=} | Only showing bounds")
            return ais_df
