...
:::note
    This tutorial assumes you have [access to Fused](/user-guide/login/), [installed the `fused` Python package](/python-sdk/#install) locally, and an S3 bucket connected to your Fused account
:::
...
- [Sentinel 1](https://sentinel.esa.int/web/sentinel/copernicus/sentinel-1) radar images from [Microsoft Planetary Computer Data Catalog](https://planetarycomputer.microsoft.com/dataset/sentinel-1-grd)
    <details>
    <summary>Details about the Sentinel data</summary>
...
Since our AIS dataset is not in a geo-partitioned, cloud native format, our first step is to [ingest](/core-concepts/data_ingestion/) it into a tiled format and put it on a cloud bucket. Thankfully we can [do all of this in Fused](/core-concepts/data_ingestion/ingestion-your-data/).
...
AIS datapoint from NOAA's platform are available per day but we'll aggregate them per month to make it simpler to manage. WE can run all of the code in this section locally, since we don't need to visualize any data.
...
```
/tmp/fused/AIS/2024_09/01.parquet
```
...
You can also [run multiple small UDFs](/core-concepts/run-udfs/run-small-udfs/#running-multiple-small-fusedrun-jobs-in-parallel) in parallel.
...
These individual parquet files are now store on our mount disk. We could save them directly onto cloud storage but before that we can geo-partition them to make them even faster to read. This will make our reduce the time it takes to access our data from [minutes to seconds](/core-concepts/data_ingestion/why-ingestion/).
Fused provides a simple way to do this with the [ingestion process](/core-concepts/data_ingestion/).
...
```python {5-8,12-16} showLineNumbers
@fused.udf
def s1_udf(
    bbox: fused.types.ViewportGDF,
):
...
```python {11,16,26-29,43-46} showLineNumbers
@fused.udf
def udf(
    bbox: fused.types.ViewportGDF,
    bands=["vv"],
...
@fused.cache
def get_data(bbox, time_of_interest, resolution, bands, data_name="sentinel-1-grd"):
    """
    ...
    if len(items) < 1:
        ...
        ds=odc_load(bbox,resolution, time_of_interest)
        da = ds[bands[0]].isel(time=0)
...
```python {17,21-41} showLineNumbers
# In the UDF editor
@fused.udf
def udf(
    bbox: fused.types.ViewportGDF,
...
```python {18-34,61-81} showLineNumbers
@fused.udf
def udf(
    bbox: fused.types.ViewportGDF,
    bands=["vv"],
...
@fused.cache
def vectorise_raster(raster, bbox, threshold: float = 0.5):
    ...
    transform = rasterio.transform.from_bounds(*bbox.total_bounds, raster.shape[1], raster.shape[0])
...
```python {30-31} showLineNumbers@fused.udf
def udf(
    bbox: fused.types.ViewportGDF,
    bands=["vv"],
    resolution=10,
...
If we call this UDF with a [token](/core-concepts/run-udfs/run-small-udfs/#token) we can call it from anywhere, from a jupyter notebook or from another UDF. Let's create a new UDF in Workbench:
...
We can use [this bit of code called `tile_to_table`](https://github.com/fusedio/udfs/blob/f8f0c0f5f5565c4f8be599c9fcb43368352f145f/public/common/utils.py#L163) which will either load the AIS data or the bounding box depending on our zoom level to keep our UDF fast & responsive. 
...
def udf(
    ...
    s1_acquisition_month_day_hour_min = '2024-09-04T00:19:09.175874',
    time_delta_in_hours: float = 0.1, # by default 6min (60min * 0.1)
    min_zoom_to_load_data = 14,
    ais_table_path = "s3://fused-users/fused/demo_user/AIS_2024_ingester/prod_2024_09", # This is the location where we had ingested our geo-partitioned AIS data
):
    """Reading AIS data from Fused partitioned AIS data from NOAA (data only available in US)"""
    import pandas as pd
    from datetime import datetime, timedelta
    local_utils = fused.load(
    ...
    # This is based on our own ingestion process, we partitioned the data per month
    # This could have been done any other way, so this reader does depend on how we created the ingestion in the first place

    month_date = sentinel1_time.strftime('%Y_%m')
    monthly_ais_table = f"{ais_table_path}prod_{month_date}/"
    print(f"{monthly_ais_table=}")
    if type(bbox) == fused.types.TileGDF:
        # This is only for workbench, doesn't work on fused.run() in notebook 
        print(f"{bbox.z=}")

    @fused.cache
    def getting_ais_from_s3(bbox, monthly_table):
        return local_utils.table_to_tile(
            bbox,
            table=monthly_ais_table, 
            use_columns=None, 
            min_zoom=min_zoom_to_load_data # Loading AIS takes a little while, so keeping it to only a very close range
        )

    ais_df = getting_ais_from_s3(bbox, monthly_ais_table)

    if ais_df.shape[0] == 0:
        print("No AIS data within this bbox & timeframe. Change bbox or timeframe")
        return ais_df
    
    if type(bbox) == fused.types.TileGDF:
        if bbox.z > min_zoom_to_load_data:
            print(f"Zoom is {bbox.z=} | Only showing bbox")
            return ais_df
...
:::tip
    You can change the styling of your layers in [the Visualise tab](/workbench/udf-builder/styling/) to make them look like the screenshot above
:::
...
Any point with the cloest AIS point > 100m from the Sentinel 1 boat will be considered a potentiel "dark vessel"
...
def udf(
    bbox: fused.types.ViewportGDF=None,
    time_of_interest="2024-09-03/2024-09-10",
    ais_search_distance_in_meters=10,
):
    import fused
    ...
    @fused.cache()
    def get_ais_from_s1_date(s1_acquisition_month_day_hour_min=s1_acquisition_month_day_hour_min, bbox=bbox):
        return fused.run("fsh_FI1FTq2CVK9sEiX0Uqakv", s1_acquisition_month_day_hour_min=s1_acquisition_month_day_hour_min, bbox=bbox)
...
(`bbox=[-71.08534386325134,41.06338103121641,-70.89011235861962,41.15718153299125]` & `s1_acquisition_month_day_hour_min = "2024-09-03T22:43:33"`)
