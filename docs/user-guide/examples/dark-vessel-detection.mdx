...
Since our AIS dataset is not in a geo-partitioned, cloud native format, our first step is to [ingest](/core-concepts/data%20ingestion/) it into a tiled format and put it on a cloud bucket. Thankfully we can [do all of this in Fused](/core-concepts/data%20ingestion/ingestion-your-data/).
...
:::tip
    You can read more on how to run multiple small UDFs in [the dedicated section](/core-concepts/run-udfs/run-small-udfs/#running-multiple-small-fusedrun-jobs-in-parallel)
:::
...
:::tip
    You can read more about why we need to ingest data [in the dedicated section](/core-concepts/data%20ingestion/why-ingestion/)
:::
...
<details>
  <summary>Expand to see code split between Code Editor & Utils Module</summary>
    ...
    In Utils Module (and renaming the "Module Name" to `local_utils`):
    ```python showLineNumbers
    @fused.cache
    def get_data(bbox, time_of_interest, resolution, bands, data_name="sentinel-1-grd"):
        """
...
- Running [UDFs at scale with `run_remote()`](/core-concepts/run-udfs/run_multiple/#using-offline-instances-run_remote). You could use this to run this pipeline over a larger area or over a much longer time series (or both!) to find out more potentiel dark vessels
