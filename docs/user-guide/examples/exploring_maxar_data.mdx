...
- Show how to use [`fused.submit()`](/python-sdk/top-level-functions/#submit) to fetch all the available STAC Catalogs all at once in parallel in just a few seconds
...
- 2. [Tile / File modes](/core-concepts/filetile/)
...
This change returns the image instead of the extent, but it returns the image all at once, because Workbench is set to ["File" mode](/core-concepts/filetile/#file-viewport) by default
...
We can change this by setting Workbench to ["Tile" mode](/core-concepts/filetile/#tile):
...
- Use [`fused.submit()`](/python-sdk/top-level-functions/#submit) to run a [UDF in parallel](/core-concepts/run-udfs/run-small-udfs/#running-multiple-jobs-in-parallel)
- Use [`@fused.cache`](/python-sdk/top-level-functions/#fusedcache) to cache requests to reduce costs and improve performance
- Use Workbench to [display images](/workbench/udf-builder/map/) in [different modes](/core-concepts/filetile/)

If you want to go a bit further you could:
- Explore the Best Practices to [make the most of UDFs](/user-guide/best-practices/udf-best-practices/) or learn handy tips to use [Workbench](/user-guide/best-practices/workbench-best-practices/) as its fullest
- Go more in depth with ["File" & "Tile" modes](/core-concepts/filetile/) in Workbench
