...
In this guide, we'll estimate how much alfalfa grows in areas defined by polygon geometries. You'll first upload your own vector table with `fused.ingest`. This spatially partitions the table and writes it in your account's S3 bucket as a [GeoParquet](https://geoparquet.org/). You'll then calculate zonal stats over a raster array of alfalfa crops in the [USDA Cropland Data Layer](https://catalog.data.gov/dataset/cropscape-cropland-data-layer) (CDL) dataset.
...
The `year` parameter is used to structure the path of the CDL dataset which the utility function `read_tiff` loads for the spatial bounds defined by `bbox`. The `crop_id` parameter 36 corresponds to alfalfa the CDL [colormap](https://storage.googleapis.com/earthengine-stac/catalog/USDA/USDA_NASS_CDL.json), which the UDF uses to mask the raster array.
