...
<!---
TODO: Add a simple figure illustrating zonal stats here
-->
...
:::warning[Note for Plinio]

The preceding paragraph may lead to confusion, because the next section does not use the workbench. This instruction should probably be deferred until the user needs to do it.
...
It is unclear where the user should perform the following steps. Do they need to setup a Python environment? If so, the order of content should probably be:

- how to setup the Python environment
- how to write and execute the Python function
- how to view the logs and results

:::

Pass the URL of the table to `fused.ingest`. When you kick off an ingest job with `run_remote`, Fused spins up a server to partition your table and writes the output to the path specified by the `output` parameter. In the codeblock below, `fd://tl_2023_49_bg/` is the base path to your account's S3 bucket.
...
Describe how to authenticate, or link to other docs.

:::

If you run this in a Python environment, make sure to install the latest Fused SDK with `pip install fused` and authenticate.  Once the job is complete, you can preview the output dataset in the [File Explorer](/workbench/file-explorer/).
