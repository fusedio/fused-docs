...
### a. HTTP endpoint
Save the UDF, then click the 'Share' button located below the UDF name on the left side. In the dialog, click 'Share' again to generate a [token](/core-concepts/run-udfs/run-small-udfs/#token). Once the token is created, you'll see an HTTP endpoint that you can use to call the UDF from anywhere. Try it out by changing the endpoint's query parameters — the output will be in GeoJSON format.
