...
- `read_tiff` loads an array of the CDL GeoTIFF for the specified `year` and the spatial extent of `bbox`
- `table_to_tile` loads a `GeoDataFrame` of the geo partitioned vector table for the spatial extent of `bbox`
...
Try running the UDF in the [UDF Builder](/workbench/udf-builder/) and visually inspect the output on the map. See what happens when you change `year`. Introduce print statements such as `print(arr)` and `print(gdf)` to show logs in the console.
...
Now that you have a UDF, let's see three ways you can [Run UDFs](/core-concepts/run/) remotely: via an HTTP endpoint, in a Python environment, and in parallel.

### a. HTTP endpoint

Save the UDF and create an HTTP endpoint for it by clicking "Share" in its Settings tab. You'll see snippets to call the UDF from anywhere using a [shared token](/core-concepts/run/#shared-token). Copy the HTTP endpoint and structure its query parameter as shown. You should see the output in GeoJSON format.
...
```python showLineNumber
gdf_final.groupby(['GEOID', 'year']).agg({'count':'sum', 'stats': 'mean'}).reset_index()
```


## 5. Create an app

Now that you've created a UDF and explored different ways to invoke it, you're can create a data app to share your results. We'll structure the HTTP endpoint you created above to act as a Tile server (with `/run/tiles/{z}/{x}/{y}`), allowing it to be called from a [pydeck](https://deckgl.readthedocs.io/en/latest/index.html) `TileLayer` within the Fused [App Builder](/workbench/app-builder/). We'll also create a [Streamlit dropdown](https://docs.streamlit.io/develop/api-reference/widgets/st.selectbox) for users to set the year parameter.
