...
def udf(
    ...
    gdf = fused.utils.common.table_to_tile(
      ...
      min_zoom=5,
      use_columns=['NAMELSAD', 'MTFCC', 'FUNCSTAT', 'geometry']
    )
    gdf.crs = 4326
...
You can also pass a `GeoDataFrame` to explicitly define a custom `bbox`.
