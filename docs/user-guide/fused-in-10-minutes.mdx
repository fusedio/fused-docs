---
sidebar_label: "🕒 Fused in 10 minutes"
...
First, set up a Python environment, install the latest Fused SDK with `pip install fused`, and [authenticate](/python-sdk/get-started/).
...
```mermaid
---
title: Overview of Zonal Stats UDF
---
graph LR
    A("Crop mask raster (arr)") --> E{Zonal Stats}
    B("Polygons table (gdf)") --> E
    E(Polygons table with aggregates)
```
</div>
