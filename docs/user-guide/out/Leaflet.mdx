...
Modify the HTTP endpoint to introduce templatized XYZ Tile parameters (`{{z}}/{{x}}/{{y}}`) as well query strings for other UDF-specific parameters (`?crop_type={crop_type}`), and pass it to the `TileLayer`.
...
Similarly, a Vector Tiles are rendered by passing the HTTP endpoint to a `VectorTileLayer`.
