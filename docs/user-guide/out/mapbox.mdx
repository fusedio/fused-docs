...
Create [mapbox](https://www.mapbox.com/) maps that load data from UDF [HTTP endpoints](/core-concepts/run/#http-requests).
...
First,, create a UDF and [generate nan HTTP endpoint](/core-concepts/run/#http-requests).

## 2. Create a Mapbox HTML map

Create an `.html` file following this template. This code creates a mapbox map, adds a sourceand , and then a layer that renders data from that source. Supported layer types are:
...
:::info
When rendering `mvt` data types, Mapbox may clip features that exceed the bounds of the tile. A workaround is for the UDF to [clip](https://geopandas.org/en/stable/docs/reference/api/geopandas.clip.html) the output at the `bbox` bounds or to only return features [within](https://geopandas.org/en/stable/docs/reference/api/geopandas.GeoSeries.within.html)` the` `bbox`. See [Discord thread](https://discord.com/channels/1199097729243152434/1261105758360305775/1261105758360305775) for more information.
:::
