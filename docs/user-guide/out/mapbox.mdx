...
Create [mapbox](https://www.mapbox.com/) maps that load data from UDF [HTTP endpoints](/core-concepts/run/#http-requests) endpoints.
...
Create an `.html` file following this template. This code creates a mapbox map, adds a source, then a layer that renders data from that source. Supported layer types are:
