...
Microsoft [Excel](https://www.microsoft.com/en-ca/microsoft-365/excel) can load data from a UDF's [HTTP Endpoint](/core-concepts/run-udfs/run-single-udfs/#http-requests) that returns a `.csv`.
...
Now, [create an HTTP endpoint](/core-concepts/run-udfs/run-single-udfs/#http-requests) for the UDF.
