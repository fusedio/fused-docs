...
[Streamlit](https://streamlit.io/) is an open source Python app builder to turn data scripts into shareable web apps. You can run Fused UDFs from Streamlit to create apps powered by UDFs. In fact, the Fused [App Builder](/app-builder/) runs a serverless version of Streamlit.
...
This script creates a minimalist Streamlit app that runs the UDF and then displays its output `DataFrame`.
