...
Take, for example, U.S. Census Block Groups in `s3://fused-asset/infra/census_bg_us/`. If you select the directory you'll notice that when the map is zoomed-out boxes showing up corresponding to the spatial extent of the geometries in the dataset. As you zoom in, the boxes will give way to the polygons of the Census Block Groups. Clicking on individual files will show the polygons they contain.
