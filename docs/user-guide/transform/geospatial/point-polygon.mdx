...
This walkthrough shows how to enrich Overture Maps building polygons with hazard metadata from the National Structure Inventory (NSI) using point in polygon analysis. The Overture Maps buildings footprints is a `Polygon` dataset while NSI is a `Point` dataset with metadata related with natural and man-made hazards. The two can be joined to visualize buildings at risk from hazards.
...
To scale the analysis across broader regions and larger datasets, you can run the UDF in a way that first spatially filters each table before performing the point-in-polygon calculation. This approach enhances computational efficiency and allows the analysis to run in parallel, making it suitable for large-scale geospatial assessments.

### Conclusion

Point-in-polygon is a powerful technique to enrich datasets. By combining the detailed building footprints from Overture Maps with the comprehensive hazard and value information from the National Structure Inventory, we can generated rich, informative risk maps and indices.
