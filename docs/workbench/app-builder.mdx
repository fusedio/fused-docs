...
Furthermore, it can be useful to build apps without pre-installed dependencies. Use [micropip](https://pypi.org/project/micropip/) to install packages at runtime. When using this approach, `requests` should be installed before `fused_app` is imported. This is also useful for [GitHub Gists](/workbench/app-builder/#github-gists), which cannot install dependencies ahead of execution.
