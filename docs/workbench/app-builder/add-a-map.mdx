---
id: app-map
title: Adding a Map to your App (Legacy)
tags: [app]
sidebar_position: 2
---
...
You can find this app [right here](https://www.fused.io/app/fsh_7dELPiyQTUMkfA4kI4ZIbw) and test it for yourself!
...
:::note
    ...
    Read more about `fused.run` in the [`fused.run` reference](/python-sdk/api-reference/fused-run)
:::

## Folium

Create a [streamlit-folium](https://folium.streamlit.app/) `TileLayer` that calls a UDF HTTPS endpoint. Replace `fc_<canvas_token>` and `my_udf` with your [canvas token and UDF name](/guide/data-input-outputs/export-api/tokens-endpoints).
...
url_raster = 'https://udf.ai/fc_<canvas_token>/my_udf/run/tiles/{z}/{x}/{y}?=png'
