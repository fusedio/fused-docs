...
Call a UDF by name (or legacy token) with `fused.run` and pass parameters from a [slider](https://docs.streamlit.io/develop/api-reference/widgets/st.slider).
...
df = fused.run('my_udf', count=threshold)  # use your UDF name
```
...
@st.cache_data
def cached_output():
    return fused.run('my_udf')  # use your UDF name

df = cached_output()

st.write(fused.run('my_udf'))
```
