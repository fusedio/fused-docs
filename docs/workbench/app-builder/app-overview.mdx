...
Call UDF [HTTP endpoints](/core-concepts/run-udfs/run-single-udfs/#http-requests) with the [requests library](https://pypi.org/project/requests/) and pass parameters from a [dropdown selectbox](https://docs.streamlit.io/develop/api-reference/widgets/st.selectbox).
