...
Download the JSON Key file associated with the Service Account. This file contains credentials that Fused will use to access the GCS bucket. It should look like this (replace placeholders with your values):
...
You can check your connection is working by listing some files:
...
def udf():
    ...
    gcs_blob_prefix= "path/to/your/data"
    
    # connect to GCP
    bucket = client.bucket(bucket_name)

    # list existing blobs in GCS under the prefix
    gcs_existing = set(
        blob.name.replace(gcs_blob_prefix, "")
        for blob in bucket.list_blobs(prefix=gcs_blob_prefix)
    )
    print(gcs_existing)
```
