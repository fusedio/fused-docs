...
Contact Fused to set your S3 or GCS buckets on the File Explorer for all users in your organization. Once connected, users can browse, preview, and create UDFs from the bucket's files. Alternatively, set a bucket as a "favorite" so it appears in the File Explorer for your account only.

For buckets that are not publicly accessible, follow the steps below to set up the necessary permissions.
...
Set up a Google Cloud service account with permissions to access the GCS bucket. See the Google Cloud documentation for service accounts:
