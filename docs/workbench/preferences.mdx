...
- **Environment**: Setting up [kernels](#kernel), [Secret management](#secrets-management) and [environment variables](#environment-variables). See below for details.
...
:::info Canvas and legacy UDF tokens are tied to the kernel at creation

When you share a canvas or create a legacy per-UDF token, the token is tied to the kernel at the time of creation. Even if you change the kernel later in Workbench, tokens created on a previous kernel will still use the previous kernel.

Reach out to Fused support at `info@fused.io` to migrate your tokens to the new kernel.
