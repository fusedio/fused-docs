...
:::info UDF shared tokens are tied to the kernel at the time of creation

When creating a shared token for a UDF, the token is tied to the kernel at the time of creation. Even if you change the kernel later in Workbench, tokens created on a previous kernel will still use the previous kernel.
