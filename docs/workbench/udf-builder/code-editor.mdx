...
UDFs saved in the UDF Builder can be called with [HTTP endpoints](/core-concepts/run-udfs/run-single-udfs/#http-requests) using [the public UDF name](/core-concepts/run-udfs/run-single-udfs/#public-udf-name) and/or [tokens](/core-concepts/run-udfs/run-single-udfs/#token).
