...
The "Share" section shows snippets to run the UDF using a public token. This allows any application to invoke the UDF without authentication - including `cURL` calls, [Lonboard](/user-guide/out/lonboard/), [Leaflet](/user-guide/out/leaflet/), [Mapbox](/user-guide/out/mapbox/), [Google Sheets](/user-guide/out/googlesheets/), [DuckDB](/user-guide/out/duckdb/), the Fused [App Builder](/app-builder/), and Python applications with [`fused.run`](/core-concepts/run/#fusedrun).
