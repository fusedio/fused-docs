...
2. [HTTPS Request](/guide/working-with-udfs/execution/realtime) / Calling UDF with [shared token](/guide/working-with-udfs/execution/realtime#token)
...
- Running the same UDF twice might lead to different runtimes especially if you call [cached functions](/guide/working-with-udfs/caching/#caching-any-python-function-fusedcache) or [cached UDFs](/guide/working-with-udfs/caching/#caching-a-udf).
...
On UDF Builder, UDFs can explicitly be set to run as [Tile or File](/guide/working-with-udfs/geospatial/single-vs-tile) - or autoselect between the two if the `bounds` object is [typed](/guide/working-with-udfs/geospatial/single-vs-tile#bounds).
