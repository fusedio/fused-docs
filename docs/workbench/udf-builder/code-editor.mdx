...
3. [HTTP Request](/tutorials/others/out/http/) / Calling UDF with [shared token](/core-concepts/run-udfs/run-small-udfs/#token)
...
On UDF Builder, UDFs can explicitly be set to run as [Tile or File](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/) - or autoselect between the two if the `bounds` object is [typed](/tutorials/Geospatial%20Analysis%20in%20Fused/filetile/#bounds-object-types).
