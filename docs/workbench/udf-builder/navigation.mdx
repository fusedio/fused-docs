...
UDFs saved in the UDF Builder can be called with [HTTPS endpoints](/guide/data-input-outputs/export-api/tokens-endpoints) using [the public UDF name](/guide/working-with-udfs/execution/realtime#public-udf-name) and/or [tokens](/guide/working-with-udfs/execution/realtime#token).

The "Shared token" section shows snippets to run the UDF using a public token. This allows any application to invoke the UDF without authentication - including `cURL` calls, Lonboard, Leaflet, [Mapbox](/guide/data-input-outputs/export-api/geospatial#mapbox), Google Sheets, DuckDB, the Fused [App Builder](/workbench/app-builder/), and Python applications with [`fused.run`](/guide/working-with-udfs/execution/realtime).
