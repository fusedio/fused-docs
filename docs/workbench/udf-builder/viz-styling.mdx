...
The UDF builder displays data from the UDF into the map view. You can change the visual representation of a UDF's output using the visualization icon located on the map.
...
The Default map view option is now available through the visualization icon on the map.
...
- The Visualization isn't written in Python, rather this is a JSON file
...
The Visualization is built on top of [DeckGL](https://deck.gl/docs), a JavaScript front-end framework build for large dataset visualizations.
