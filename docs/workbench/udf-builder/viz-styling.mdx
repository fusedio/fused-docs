...
*Note: Numerical columns used for building height/elevation in 3D view.*
...
### Vector Layer Not Colorizing?

**Common issues:**

1. **Missing column**: Ensure the column referenced in `attr` or `property` exists in your DataFrame
2. **Wrong data type**: Cast categorical columns to `int` or `float` for continuous coloring:
   ```python
   df['numeric_column'] = df['string_column'].astype(int)
   ```
3. **Domain mismatch**: Verify your data values fall within the specified `domain` range
4. **Missing geometry**: GeoJsonLayer requires a geometry column in your GeoDataFrame

### Layer Not Appearing?

- Check that `pickable: true` is set for interactivity
- Verify your UDF returns the expected data type (GeoDataFrame for vector, array for raster)
- Ensure zoom levels (`minZoom`, `maxZoom`) include your current map zoom level
