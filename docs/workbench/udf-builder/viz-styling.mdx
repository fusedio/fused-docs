...
Fused automatically determines which layer to use based on your UDF output and data size:

**Automatic Layer Selection:**
- **Vector data** (GeoDataFrame): Uses `vectorLayer` with [`GeoJsonLayer`](#vector-geojsonlayer)
- **Raster data** (PNG, arrays): Uses `rasterLayer` with [`BitmapLayer`](#raster-bitmaplayer)
- **H3 data** (DataFrame with H3 indices): Uses `vectorLayer` with [`H3HexagonLayer`](#vector-h3hexagonlayer)

### JSON Configuration Format

**Important**: Visualizations use JSON configuration, not Python code. While your UDF is written in Python, the styling is defined through JSON properties that reference your DataFrame columns.

```javascript
// Visualization is JSON, not Python
{
  "vectorLayer": {
    "@@type": "GeoJsonLayer",
    "getFillColor": "@@=properties.column_name * 50"  // References Python DataFrame column
  }
}
```
...
Fused automatically selects the appropriate visualization layer based on what your UDF returns:
...
**Key Point**: You define all three layers in your JSON, but Fused uses only the appropriate one based on your data type. The unused layers are ignored.
...
### 1. Hardcoded Colors (`getFillColor`)
...
### 2. Property-Based Colors (`getFillColor`)
...
### 3. Conditional Colors with `hasProp` (`getFillColor`)
...
### 4. Category Colors (`getFillColor`)
...
### 5. Continuous Color Scales (`getFillColor`)
