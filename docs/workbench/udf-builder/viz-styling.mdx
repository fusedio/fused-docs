...
The Map builder displays data from your UDF on the map. You can customize the visual representation using the visualization icon located on the map.

:::tip Building standalone maps in Canvas?
If you're using `map_utils` to build standalone maps in Canvas, see [Standalone Maps](/examples/standalone-html-maps) instead. This page covers styling for Map Builder layers.
:::

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<LazyReactPlayer url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/visualization_open_edit7.mp4" />
...
|------------|-----------|------------|----------|
| `GeoDataFrame` | `vectorLayer` | [`GeoJsonLayer`](#vector-custom-map-mapbox-gl) | Polygons, lines, points with geometry |
...
- `source-layer`: Name of the tile layer (always `udf` for Fused vector tiles)
...
Use a custom map UDF that renders deck.gl `H3HexagonLayer` with tile-based data loading. The UDF consumes JSON tiles with H3 indices and values and returns an HTML map with configurable styling.

Live example rendered from a UDF (HTML output):

<iframe
  id="h3_tile_demo_preview"
  loading="lazy"
  src="https://www.fused.io/server/v1/realtime-shared/UDF_H3_Hexagon_Mapbox_Tile_With_Config/run/file?dtype_out_raster=png&dtype_out_vector=parquet"
  height="300px"
  width="100%"
  scrolling="no"
></iframe>
<p style={{marginTop: 6}}>
  <a href="https://www.fused.io/server/v1/realtime-shared/UDF_H3_Hexagon_Mapbox_Tile_With_Config/run/file?dtype_out_raster=png&dtype_out_vector=parquet" target="_blank" rel="noreferrer">Open demo ↗</a>
</p>

**Expected data schema:**
- **hex**: H3 index string (e.g., `8a2a1072b59ffff`)
- **metric**: numeric value used for color/elevation

**Key features:**
- **Tile-based loading**: Efficiently loads H3 data as map tiles
- **Configurable styling**: JSON-based configuration for colors, elevation, and styling
- **Interactive**: Hover to see hex details and metric values
        ...
        layers: [
          new TileLayer({
            ...
            renderSubLayers: (props) => {
              ...
              // Fallback: draw boundaries with PolygonLayer if geo-layers not present
...
**Example UDF**
- [Ookla Download Speeds UDF](https://www.fused.io/workbench/udf/catalog/Ookla_Download_Speeds-12a8bc7e-5baf-4eac-b695-a4434cff0554)
...
<LazyReactPlayer loop url="https://fused-magic.s3.us-west-2.amazonaws.com/workbench-walkthrough-videos/docs_rewrite/debug_layer10.mp4" />
