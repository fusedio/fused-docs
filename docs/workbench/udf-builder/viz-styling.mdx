...
The Fused UDF builder map supports the following layer types:
...
A UDF that returns a `DataFrame` can dynamically set visualization properties, such as line color (`getLineColor`) and line width (`getLineWidth`), based on its output properties.  Setting the property [`lineWidthUnits`](https://deck.gl/docs/api-reference/layers/geojson-layer#linewidthunits) to `pixels` helps maintain consistency across zoom levels.
