---
id: udf-catalog
title: UDF Catalog
tags: [catalog, udf]
sidebar_position: 3
---



The UDF Catalog offers a searchable collection of UDFs that can be imported into the [UDF Builder](/workbench/udf-builder/) for editing. It facilitates sharing and discovering UDFs within teams and the broader Fused community.

Data teams frequently need to share and reuse code snippets across different projects. Traditional notebooks pose challenges in version control, sharing individual utility functions, managing dependencies, and reproducing results in new environments. Additionally, generated assets are often disconnected from the code. UDFs, however, encapsulate modules, outputs, and code, addressing issuess related to lineage and reproducibility.


- Create a [new UDF](/workbench/udf-catalog/#new-udf)
- [Search](/workbench/udf-catalog/#search) the catalog across [UDF categories](/workbench/udf-catalog/#udf-categories)
- View the detailed [profile](/workbench/udf-catalog/#udf-profile) for each UDF
- [Add UDFs](/workbench/udf-catalog/#add-udf) to the UDF Builder

import ImgFe4 from '@site/static/img/image-fe-4.png';

<div style={{textAlign: 'center'}}>
<img src={ImgFe4} alt="File" style={{width: 600}} />
</div>

## New UDF

Create a new UDF by clicking the "New UDF" button. This opens a UDF with a base template.


## Search


Open the UDF Catalog by Clicking "Add UDF". Search UDFs by name, sort them, and toggle between gallery and list views. Click UDF cards to view their profiles or add them to the UDF Builder.

![Alt text](https://fused-magic.s3.us-west-2.amazonaws.com/docs_assets/image-20.png)





### UDF categories

- **Public UDFs:** Verified and accessible to all users
- **Community UDFs:** Shared by the community and accessible to all users
- **Team UDFs:** Shared privately within a team in a [GitHub](/core-concepts/content-management/git/) repo
- **Saved UDFs:** Personal UDFs saved by the user


## UDF profile

Select a UDF to view its profile which includes the [`README.md` description](/workbench/udf-builder/code-editor/#description), last update date, author, link to its code on GitHub, and code preview.
