...
The UDF Explorer offers a searchable collection of [User Defined Functions (UDFs)](/guide/getting-started/first-udf-basics) that can be imported into [Workbench](/workbench/overview) for editing. It facilitates sharing and discovering UDFs within teams and the broader Fused community.
...
Open the UDF Explorer by clicking the menu in the top-left corner and selecting `Quick Actions` and then `UDF Explorer`, or use the shortcut `Option + X`. Search UDFs by name, sort them, and toggle between gallery and list views. Click UDF cards to view their profiles or add them to Workbench.
