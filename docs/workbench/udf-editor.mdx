...
```json
{
  ...
  "vectorLayer": {
    ...
    "pickable": true,
    "getHexagon": "@@=hex",
    "getFillColor": "@@=[count+400, count+50, count]",
    "getElevation": "@@=count",
    "elevationScale": 4
  }
}
...
import GlsTile from "/img/gls_tile.png";

<div style={{ textAlign: "center" }}>
  <img src={GlsTile} alt="Gls" style={{ width: 400 }} />
</div>
...
#### c. Custom loading and error

Other layers that can be set are the loading and error layers. These are used to show the user that the UDF is still processing or that an error occurred.
