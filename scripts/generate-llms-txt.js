...
function filterHiddenParameters(content) {
  // For each function/decorator in HIDDEN_PARAMS, remove the specified parameters
  for (const [funcName, params] of Object.entries(HIDDEN_PARAMS)) {
    for (const param of params) {
      // Match parameter lines like "- name (Optional[str])" with optional trailing content
      // This handles both simple "- param (type)" and "- param (type) - description" formats
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        items.push(
          ...walkDirectory(fullPath, relativePath, isFullVersion, excludeDirs),
        );
        ...
        if (!isFullVersion && (frontmatter.unlisted || frontmatter.draft))
          continue;
        ...
        const isIndexFile =
          filename.replace(/\.mdx?$/, "").toLowerCase() === "index";
        ...
        if (isIndexFile) {
          ...
          urlParts = parts.map((part, index) => {
            ...
            if (isLastPart) {
              ...
              return part.replace(/\.mdx?$/, ""); // Remove extension only, keep underscores and case
            } else {
...
`;
    ...
    for (const [sectionTitle, sectionPath] of Object.entries(SECTIONS)) {
      ...
      const excludeDirs =
        sectionPath === "tutorials" ? ["Geospatial with Fused"] : [];
      const items = walkDirectory(
        sectionDir,
        sectionPath,
        isFullVersion,
        excludeDirs,
    ...
    content += `\n---\n\nGenerated automatically from Fused documentation. Last updated: ${
    ...
    console.log(
      `✅ Generated full-text llms.txt with ${totalSections} complete sections`,
    );
...
`;
    ...
    for (const [sectionTitle, sectionPath] of Object.entries(SECTIONS)) {
      ...
      [...topLevel, ...nested]
        .filter((item) => item.title.toLowerCase() !== "index")
        .forEach((item) => {
...
`;
  ...
  const filteredItems = items.filter(
    (item) => !item.path.includes("changelog.mdx"),
  );
  ...
  content += `\n---\n\nGenerated automatically from Fused Python SDK documentation. Last updated: ${
    new Date().toISOString().split("T")[0]
  ...
  fs.writeFileSync(outputFile, content, "utf8");
  console.log(
    `✅ Generated Python SDK llms.txt with ${filteredItems.length} pages`,
  );
  console.log(`📝 File saved to: ${outputFile}`);
  console.log(`📊 File size: ${Math.round(content.length / 1024)} KB`);
